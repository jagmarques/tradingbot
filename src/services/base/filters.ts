...
// Check token info (name, symbol, supply)
export async function checkTokenInfo(tokenAddress: string): Promise<FilterResult & { symbol?: string; decimals?: number }> {
  try {
    ...
    // Check total supply (very large supply is suspicious)
...
// Basic honeypot check (can we call transfer?)
export async function checkHoneypot(tokenAddress: string): Promise<FilterResult> {
  try {
    const token = new ethers.Contract(tokenAddress, ERC20_ABI, getProvider());

    // Try to estimate gas for a transfer (if this fails, might be honeypot)
    // We use a static call which doesn't actually execute
    try {
      // Check if there's an owner function (potential for owner manipulation)
