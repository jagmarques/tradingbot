...
// Minimum liquidity in AVAX
const MIN_LIQUIDITY_AVAX = 10.0;
...
function getProvider(): ethers.JsonRpcProvider {
  if (!provider) {
    provider = new ethers.JsonRpcProvider(AVAX_RPC_URL);
  }
  return provider;
}
...
export async function checkLiquidity(pair: NewPair): Promise<FilterResult> {
  try {
    ...
    const isToken0Wavax = token0.toLowerCase() === WAVAX.toLowerCase();
    const wavaxReserve = isToken0Wavax ? reserves[0] : reserves[1];
    const wavaxAmount = Number(wavaxReserve) / 1e18;

    if (wavaxAmount >= MIN_LIQUIDITY_AVAX) {
      return {
        passed: true,
        score: Math.min(100, (wavaxAmount / MIN_LIQUIDITY_AVAX) * 50),
      };
    }

    return {
      passed: false,
      reason: `Liquidity too low: ${wavaxAmount.toFixed(4)} AVAX (min: ${MIN_LIQUIDITY_AVAX})`,
      score: (wavaxAmount / MIN_LIQUIDITY_AVAX) * 50,
    };
  } catch (err) {
    return {
      passed: false,
