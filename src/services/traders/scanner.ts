  ...
  for (const wallet of insiders) {
    ...
    for (const hit of hits) {
      ...
      try {
        ...
        if ((pnl.status === "sold" || pnl.status === "transferred") && wallet.score >= 60) {
...
export async function runInsiderScan(): Promise<InsiderScanResult> {
  ...
  try {
    ...
    const deleted = deleteInsiderWalletsBelow(60);
    if (deleted > 0) {
      console.log(`[InsiderScanner] Removed ${deleted} wallets below score 60`);
    }
  ...
  try {
    ...
    for (const gem of heldGems) {
      ...
      // Process if: no score yet, OR scored >= 60 but no paper trade exists
      if (!cached || (cached.score >= 60 && !getGemPaperTrade(gem.tokenSymbol, gem.chain))) {
