// Elder Force Index(2) entry + Chandelier m6 exit
...
const TRADE_TYPE = "elder-chan" as const;
const LEVERAGE = 10;
const FORCE_PERIOD = 2;
const ATR_PERIOD = 14;
const CHAN_MULT = 6;
...
// Elder Force Index = EMA(period) of (close - prevClose) * volume
function computeForceIndex(candles: OhlcvCandle[]): number[] {
  const raw: number[] = [0];
  for (let i = 1; i < candles.length; i++) {
    raw.push((candles[i].close - candles[i - 1].close) * (candles[i].volume ?? 1));
  }
  return EMA.calculate({ period: FORCE_PERIOD, values: raw });
}

interface ElderSignal {
  pair: string;
...
async function analyzeSignal(pair: string): Promise<ElderSignal | null> {
  const cs = await fetchCandles(pair, "1h", 80);
  if (cs.length < 20) return null;

  const fi = computeForceIndex(cs);
  const atrVals = computeAtr(cs);
  if (fi.length < 3 || atrVals.length < 2) return null;

  const last = cs.length - 1;
  const fiOff = cs.length - fi.length;
  const atrOff = cs.length - atrVals.length;
  const atr = atrVals[last - 1 - atrOff];
  if (atr === undefined) return null;

  const fiLast = fi[last - 1 - fiOff];
  const fiPrev = fi[last - 2 - fiOff];
  if (fiLast === undefined || fiPrev === undefined) return null;

  const crossUp = fiLast > 0 && fiPrev <= 0;
  const crossDn = fiLast < 0 && fiPrev >= 0;
...
async function updateChanStops(): Promise<void> {
  ...
  for (const pos of positions) {
    try {
      ...
      console.error(`[Elder] Stop update error ${pos.pair}: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
}

export async function runElderChanCycle(): Promise<number> {
  ...
  for (const pos of myPositions) {
    ...
    if (holdMs >= MAX_HOLD_MS) {
      console.log(`[Elder] Max hold exit: ${pos.pair} ${pos.direction}`);
  ...
  for (const pair of QUANT_TRADING_PAIRS) {
    ...
    try {
      ...
      if (position) {
        executed++;
        openPairs.add(pair);
        console.log(`[Elder] Opened ${pair} ${signal.direction} $${QUANT_FIXED_POSITION_SIZE_USD} @${signal.entryPrice.toFixed(2)} SL=${signal.stopLoss.toFixed(4)}`);
      }
    } catch (err) {
      console.error(`[Elder] Error ${pair}: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
  return executed;
}
