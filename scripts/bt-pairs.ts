...
interface T{pair:string;dir:"long"|"short";pnl:number}
...
for(const t of st){const cl=new Set<string>();
  for(const[p,ps]of pos){const d=pdm.get(p)!;const bi=d.tm.get(t)??-1;if(bi<0)continue;const b=d.cs[bi];const sp=SP[p]??4e-4;let xp=0;
    ...
    if(xp>0){const raw=ps.dir==="long"?(xp/ps.ep-1)*SZ*LEV:(ps.ep/xp-1)*SZ*LEV;trades.push({pair:p,dir:ps.dir,pnl:raw-SZ*LEV*FEE*2});pos.delete(p);cl.add(p)}}
  for(const[p,d]of pdm){if(p==="BTCUSDT"||pos.has(p)||cl.has(p))continue;const bi=d.tm.get(t)??-1;if(bi<60)continue;
    ...
    pos.set(p,{pair:p,dir,ep:ent,et:t,sl:dir==="long"?ent*0.97:ent*1.03,tp:dir==="long"?ent*1.10:ent*0.90,pk:0})}}

// Per-pair stats
const pairStats=new Map<string,{n:number;w:number;pnl:number;longs:number;shorts:number}>();
for(const t of trades){const s=pairStats.get(t.pair)||{n:0,w:0,pnl:0,longs:0,shorts:0};s.n++;if(t.pnl>0)s.w++;s.pnl+=t.pnl;if(t.dir==="long")s.longs++;else s.shorts++;pairStats.set(t.pair,s)}

console.log("Pair     Trades  Longs  Shorts  WR%    PnL       $/trade");
console.log("-".repeat(65));
const sorted=[...pairStats.entries()].sort((a,b)=>b[1].n-a[1].n);
for(const[p,s]of sorted){
  console.log(`${p.replace("USDT","").padEnd(8)} ${String(s.n).padStart(5)}  ${String(s.longs).padStart(5)}  ${String(s.shorts).padStart(6)}  ${(s.w/s.n*100).toFixed(1).padStart(5)}  ${(s.pnl>=0?"+":"")}$${s.pnl.toFixed(0).padStart(5)}  ${(s.pnl>=0?"+":"")}$${(s.pnl/s.n).toFixed(2)}`)}
console.log(`\nTotal: ${trades.length} trades over ${days.toFixed(0)} days = ${(trades.length/days).toFixed(1)}/day`);
