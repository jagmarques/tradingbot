...
const CD="/tmp/bt-pair-cache",H=3600000,DAY=86400000,FEE=0.00035;
...
function sim(sz:number,pdm:Map<string,D>,btc:D,s:number,e:number):T[]{
  ...
  for(const t of st){const cl=new Set<string>();
    for(const[p,ps]of pos){const d=pdm.get(p)!;const bi=d.tm.get(t)??-1;if(bi<0)continue;const b=d.cs[bi];const sp=SP[p]??4e-4;let xp=0;
      ...
      if(xp>0){const raw=ps.dir==="long"?(xp/ps.ep-1)*sz*10:(ps.ep/xp-1)*sz*10;tr.push({pnl:raw-sz*10*FEE*2,et:ps.et,xt:t});pos.delete(p);cl.add(p)}}
...
for(const sz of [10,20,30,40,50]){
  const tr=sim(sz,pdm,btc,S,E);const pnl=tr.reduce((s,t)=>s+t.pnl,0);const w=tr.filter(t=>t.pnl>0).length;
  ...
  const sh=std>0?(avg/std)*Math.sqrt(252):0;const worst=dr.length>0?Math.min(...dr):0;
  const trn=sim(sz,pdm,btc,S,M);const tst=sim(sz,pdm,btc,M,E);
  const trnD=trn.reduce((a,t)=>a+t.pnl,0)/((M-S)/DAY);const tstD=tst.reduce((a,t)=>a+t.pnl,0)/((E-M)/DAY);
  console.log(`$${sz}/trade: $${(pnl/days).toFixed(2)}/day  WR=${(w/tr.length*100).toFixed(1)}%  Sharpe=${sh.toFixed(2)}  MaxDD=$${dd.toFixed(0)}  WorstDay=$${worst.toFixed(0)}  Train=$${trnD.toFixed(2)} Test=$${tstD.toFixed(2)}`);
}
