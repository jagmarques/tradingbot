// ZLEMA v3: Calmar-sorted Phase 1+2 (train), top-30 tested, ADX up to 30.
// Run: npx tsx scripts/tune-zlema-v3.ts
...
function runBacktest(candles4h: Candle[], pre4h: H4PreInd, dailyCandles: Candle[], preDaily: DailyPreInd, idxDailyAt: number[], p: Params, startIdx = 100, endIdx?: number): BacktestResult {
  ...
  for (let i = startIdx; i < end; i++) {
    ...
    if (pos !== null) {
      ...
      const currFast = fastArr[i], prevFast = fastArr[i - 1], currSlow = slowArr[i], prevSlow = slowArr[i - 1];
...
async function main() {
  ...
  console.log(`=== tune-zlema-v3.ts: ZLEMA v3 (ADX 14-30, Calmar-sorted), ${PAIRS.length} pairs, ${DAYS}d ===`);
  ...
  for (const params of PHASE1_GRID) {
    let total = 0, maxDD = 0;
    for (const pair of pairs) { const { h4, pre4h, dailyCandles, preDaily, idxDailyAt, trainEnd } = candleMap[pair]; const r = runBacktest(h4, pre4h, dailyCandles, preDaily, idxDailyAt, params, 100, trainEnd); total += r.totalReturn; maxDD = Math.max(maxDD, r.maxDrawdown); }
  ...
  for (const [adx, results] of byAdx) {
    ...
    console.log(`  ADX=${adx} best: sma=${w.params.dailySmaPeriod} ZLEMA(${w.params.zlemaFast}/${w.params.zlemaSlow}) | train: ${w.trainReturn.toFixed(2)}%`);
  }
  ...
  results.forEach((r, idx) => {
    ...
    console.log(`${String(idx + 1).padStart(2)}  ${r.profitPerDay >= 0 ? "+" : ""}${r.profitPerDay.toFixed(3)}  ${r.testReturn >= 0 ? "+" : ""}${r.testReturn.toFixed(2)}%  ${String(r.testTrades).padStart(6)}  ${r.testWinRate.toFixed(0)}%  ${r.testMaxDrawdown.toFixed(1)}%  ${calmar.toFixed(2)}   ${r.testSharpe.toFixed(2)}   sma=${r.params.dailySmaPeriod} adx=${r.params.dailyAdxMin} ZLEMA(${r.params.zlemaFast}/${r.params.zlemaSlow}) stop=${r.params.stopAtrMult} rr=${r.params.rrRatio} stag=${r.params.stagnationH * 4}h`);
  });
