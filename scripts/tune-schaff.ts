// Schaff Trend Cycle: MACD passed through a double stochastic for faster trend detection.
// Walk-forward: 240d train / ~125d test. 0.29% round-trip, 10x leverage.
// Run: npx tsx scripts/tune-schaff.ts
...
// Rolling stochastic of a dense series
...
function computeSTC(closes: number[], fast: number, slow: number, cycle: number, smooth: number): (number | null)[] {
  ...
  // Smooth stoch1 with EMA
  const d1 = smoothSeries(stoch1, smooth);
  // Step 3: stochastic of d1
  const stoch2 = computeStochOfSeries(d1, cycle);
  // Smooth stoch2 with EMA
  const stc = smoothSeries(stoch2, smooth);
  // Align to full close array
  const result: (number | null)[] = new Array(n).fill(null);
  // Total offset: (slow-1) + (cycle-1)*2 + (smooth-1)*2 bars of warmup (approximate)
  const warmup = n - stc.length;
  stc.forEach((v, i) => { result[warmup + i] = v; });
  return result;
}

const STC_SMOOTH = 3; // fixed
...
function precompute4h(candles: Candle[], fastVals: number[], slowVals: number[], cycleVals: number[]): H4PreInd {
  ...
  // Only compute unique combos where fast < slow
  const seen = new Set<string>();
  for (const fast of fastVals)
    for (const slow of slowVals)
      for (const cycle of cycleVals) {
...
function runBacktest(candles4h: Candle[], pre4h: H4PreInd, dailyCandles: Candle[], preDaily: DailyPreInd, idxDailyAt: number[], p: Params, startIdx = 80, endIdx?: number): BacktestResult {
  ...
  for (let i = startIdx; i < end; i++) {
    ...
    if (pos !== null) {
      ...
      const key = `${p.stcFast}_${p.stcSlow}_${p.stcCycle}`;
      const stcArr = pre4h.stc[key];
      if (!stcArr) continue;
      const curr = stcArr[i], prev = stcArr[i - 1];
...
// Phase 1: sma x adx x fast x slow x cycle x threshold = 4x4x2x2x3x2 = 384, only fast < slow
...
async function main() {
  ...
  console.log(`=== tune-schaff.ts: Schaff Trend Cycle, ${PAIRS.length} pairs, ${DAYS}d ===`);
  ...
  console.log(`  Best: sma=${best1.dailySmaPeriod} adx=${best1.dailyAdxMin} STC(${best1.stcFast},${best1.stcSlow},${best1.stcCycle}) thr=${best1.stcThreshold} | train: ${phase1[0].trainReturn.toFixed(2)}%`);
  ...
  results.forEach((r, i) => {
    const p = r.params;
    console.log(`${String(i + 1).padStart(2)}  ${r.profitPerDay >= 0 ? "+" : ""}${r.profitPerDay.toFixed(3)}  ${r.testReturn >= 0 ? "+" : ""}${r.testReturn.toFixed(2)}%  ${String(r.testTrades).padStart(6)}  ${r.testWinRate.toFixed(0)}%  ${r.testMaxDrawdown.toFixed(1)}%  ${r.testSharpe >= 0 ? " " : ""}${r.testSharpe.toFixed(2)}   sma=${p.dailySmaPeriod} adx=${p.dailyAdxMin} STC(${p.stcFast},${p.stcSlow},${p.stcCycle}) thr=${p.stcThreshold} stop=${p.stopAtrMult} rr=${p.rrRatio} stag=${p.stagnationH * 4}h`);
  });
