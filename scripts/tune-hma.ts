// Hull MA Cross: fast HMA crosses slow HMA with ADX + daily trend filter.
// HMA(n) = WMA(2*WMA(n/2) - WMA(n), sqrt(n)). Less lag than EMA.
// Walk-forward: 240d train / ~125d test. 0.29% round-trip, 10x leverage.
// Run: npx tsx scripts/tune-hma.ts
...
interface Params { dailySmaPeriod: number; dailyAdxMin: number; hmaFast: number; hmaSlow: number; stopAtrMult: number; rrRatio: number; stagnationH: number; }
...
interface H4PreInd { atr: (number | null)[]; hma: Record<number, (number | null)[]>; }
...
function computeHMA(closes: number[], period: number): (number | null)[] {
  ...
  // Compute 2*WMA(n/2) - WMA(n); both are valid from period-1 onward
  ...
  // Apply WMA(sqrt(n)) to the diff series
...
function precompute4h(candles: Candle[], hmaPeriods: number[]): H4PreInd {
  const n = candles.length;
  const highs = candles.map((c) => c.high);
  const lows = candles.map((c) => c.low);
  const closes = candles.map((c) => c.close);
  ...
  const hma: Record<number, (number | null)[]> = {};
  for (const period of hmaPeriods) { hma[period] = computeHMA(closes, period); }
  return { atr: atrArr, hma };
}
...
function runBacktest(candles4h: Candle[], pre4h: H4PreInd, dailyCandles: Candle[], preDaily: DailyPreInd, idxDailyAt: number[], p: Params, startIdx = 80, endIdx?: number): BacktestResult {
  ...
  const fastArr = pre4h.hma[p.hmaFast];
  const slowArr = pre4h.hma[p.hmaSlow];
  ...
  for (let i = startIdx; i < end; i++) {
    ...
    if (pos !== null) {
      ...
      // Long: fast HMA crosses above slow HMA in uptrend
      // Short: fast HMA crosses below slow HMA in downtrend
...
const HMA_FAST_VALS = [5, 8, 13];
const HMA_SLOW_VALS = [21, 34, 55];
const STOP_ATR_VALS = [1.5, 2.0, 2.5, 3.0];
const RR_VALS = [2.5, 3.0, 3.5, 4.0];
const STAG_VALS = [6, 9, 12];

const ALL_HMA_PERIODS = [...new Set([...HMA_FAST_VALS, ...HMA_SLOW_VALS])];
...
for (const dailySmaPeriod of DAILY_SMA_VALS)
  for (const dailyAdxMin of DAILY_ADX_VALS)
    for (const hmaFast of HMA_FAST_VALS)
      for (const hmaSlow of HMA_SLOW_VALS)
        PHASE1_GRID.push({ dailySmaPeriod, dailyAdxMin, hmaFast, hmaSlow, stopAtrMult: 2.0, rrRatio: 2.5, stagnationH: 9 });

async function main() {
  console.log(`=== tune-hma.ts: Hull MA Cross, ${PAIRS.length} pairs, ${DAYS}d ===`);
  ...
  for (const pair of PAIRS) {
    try {
      ...
      const pre4h = precompute4h(h4, ALL_HMA_PERIODS);
  ...
  console.log(`  Best Phase 1: sma=${best1.dailySmaPeriod} adx=${best1.dailyAdxMin} HMA(${best1.hmaFast}/${best1.hmaSlow}) | train: ${phase1[0].trainReturn.toFixed(2)}%`);
  ...
  results.forEach((r, i) => {
    const p = r.params;
    console.log(`${String(i + 1).padStart(2)}  ${r.profitPerDay >= 0 ? "+" : ""}${r.profitPerDay.toFixed(3)}  ${r.testReturn >= 0 ? "+" : ""}${r.testReturn.toFixed(2)}%    ${r.trainReturn >= 0 ? "+" : ""}${r.trainReturn.toFixed(2)}%   ${String(r.testTrades).padStart(6)}  ${r.testWinRate.toFixed(0)}%  ${r.testMaxDrawdown.toFixed(1)}%  ${r.testSharpe >= 0 ? " " : ""}${r.testSharpe.toFixed(2)}   sma=${p.dailySmaPeriod} adx=${p.dailyAdxMin} HMA(${p.hmaFast}/${p.hmaSlow}) stop=${p.stopAtrMult} rr=${p.rrRatio} stag=${p.stagnationH * 4}h`);
  });
  const best = results[0];
  console.log(`\nBest: ${best.profitPerDay >= 0 ? "+" : ""}${best.profitPerDay.toFixed(4)}%/day, Sharpe ${best.testSharpe.toFixed(2)}, trades ${best.testTrades}`);
  const p = best.params;
  console.log(`Optimal params: sma=${p.dailySmaPeriod} adx=${p.dailyAdxMin} HMA(${p.hmaFast}/${p.hmaSlow}) stop=${p.stopAtrMult} rr=${p.rrRatio} stag=${p.stagnationH * 4}h`);
