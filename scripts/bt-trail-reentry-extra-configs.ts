/**
 * Extra Trailing Stop + Re-entry Configs Backtest
 * True chronological event-driven simulation walking 1m bars.
 * Engine logic identical to bt-trail-reentry-independent.ts (audited).
 *
 * Configs tested:
 *   1. NO TRAIL (baseline)
 *   2. 35/5+RE
 ...
 *   5. 45/5+RE
 *   6. 45/7+RE
 *   7. 40/10+RE (reference)
 *
 * Run: NODE_OPTIONS="--max-old-space-size=8192" npx tsx scripts/bt-trail-reentry-extra-configs.ts
 */
...
// Engine B: Supertrend - fresh entry
...
// Engine D: Momentum Confirm - fresh entry
...
function runSim(
  ...
  // Close remaining open positions at simEnd
  for (let pi = openPositions.length - 1; pi >= 0; pi--) {
    const pos = openPositions[pi];
...
const configs: { label: string; act: number; dist: number; reentry: boolean }[] = [
  ...
  { label: "35/5+RE",    act: 35, dist: 5,  reentry: true  },
  { label: "40/3+RE",    act: 40, dist: 3,  reentry: true  },
  { label: "40/5+RE",    act: 40, dist: 5,  reentry: true  },
  { label: "45/5+RE",    act: 45, dist: 5,  reentry: true  },
  { label: "45/7+RE",    act: 45, dist: 7,  reentry: true  },
...
// --------------- print results ---------------
console.log("\n" + "=".repeat(165));
console.log("EXTRA TRAILING STOP + RE-ENTRY CONFIGS (Chronological 1m Event-Driven)");
...
for (const r of results) {
  const mark = r.label === "NO TRAIL" ? " <<<" : (r.label === "40/10+RE" ? " (ref)" : "");
  console.log([
    r.label.padEnd(14),
...
const ref = results.find(r => r.label === "40/10+RE")!;
const trailOnly = results.filter(r => r.label !== "NO TRAIL");
const bestPerDay = trailOnly.sort((a, b) => b.perDay - a.perDay)[0];
const bestOos = [...trailOnly].sort((a, b) => b.oosPerDay - a.oosPerDay)[0];
...
console.log(`40/10+RE (ref):$${ref.perDay.toFixed(2)}/day, MaxDD $${ref.maxDD.toFixed(0)}, Sharpe ${ref.sharpe.toFixed(2)}, OOS $${ref.oosPerDay.toFixed(2)}/day`);
console.log(`BEST $/day:    ${bestPerDay.label}: $${bestPerDay.perDay.toFixed(2)}/day, MaxDD $${bestPerDay.maxDD.toFixed(0)}, Sharpe ${bestPerDay.sharpe.toFixed(2)}, OOS $${bestPerDay.oosPerDay.toFixed(2)}/day`);
console.log(`BEST OOS:      ${bestOos.label}: OOS $${bestOos.oosPerDay.toFixed(2)}/day, PF ${bestOos.oosPf.toFixed(2)}`);

const diff = bestPerDay.perDay - noTrail.perDay;
const pctDiff = noTrail.perDay !== 0 ? ((diff / Math.abs(noTrail.perDay)) * 100) : 0;
console.log(`BEST vs BASELINE: ${diff >= 0 ? "+" : ""}$${diff.toFixed(2)}/day (${pctDiff >= 0 ? "+" : ""}${pctDiff.toFixed(0)}% vs no-trail)`);
