/**
 * GARCH v2 Z-Score Threshold Sweep
 * 2-engine setup: GARCH ($15) + Supertrend ($5), max positions variable.
 * Trail 20/3 + re-entry. Chronological 1m event-driven simulation.
 *
 * Run: NODE_OPTIONS="--max-old-space-size=8192" npx tsx scripts/bt-garch-thresholds.ts
 */
...
const SIZE_GARCH = 15;   // GARCH margin
const SIZE_ST    = 5;    // Supertrend margin
...
function aggregate(bars: C[], periodMs: number, minBars: number): C[] {
  // Single-pass: accumulate each bucket inline, no per-bucket sort needed
  // since input is already sorted by timestamp
  ...
  for (const b of bars) {
    ...
    if (!existing) {
      ...
      existing.c = b.c;  // last close wins (bars are sorted)
      existing.count++;
    }
  }
  const result: C[] = [];
...
// --------------- load all data (once) ---------------
// Strategy: load 1m data, aggregate to 1h and 4h for indicators.
// Keep full 1m arrays for SL/TP/trail simulation.
console.log("Loading data (this takes ~5 min, loading 1m data for 24 pairs)...");
...
for (const p of ["BTC", ...PAIRS]) {
  ...
  if (d.length > 0) {
    ...
    process.stdout.write(`  ${p}: ${d.length} 1m bars -> ${h1Data.get(p)!.length} 1h, ${h4Data.get(p)!.length} 4h\n`);
  } else {
...
// --------------- BTC 4h EMA(12) > EMA(21) filter ---------------
if (!h4Data.has("BTC")) {
  console.error("ERROR: BTC data not found in 1m cache.");
  process.exit(1);
}
...
// BTC h1 EMA(9) > EMA(21) for GARCH direction filter
...
// --------------- GARCH signals (parameterized thresholds) ---------------
interface GarchThresholds {
  longZ1h: number;   // 1h z > X for long
  longZ4h: number;   // 4h z > X for long
  shortZ1h: number;  // 1h z < X for short (negative)
  shortZ4h: number;  // 4h z < X for short (negative)
}
...
function runSim(
  ...
  // Supertrend exit check at 4h boundaries
  function checkSupertrendExit(
    pair: string, t: number, dir: "long" | "short", entryTime: number,
  ...
  // GARCH exit: 96h max hold + 7% TP (checked on 1m bar inline)
  let lastPct = -1;
  ...
  for (let t = simStart; t < simEnd; t += MIN_1) {
    ...
    // --- 2) Engine-specific exits at bar boundaries ---
    if (t % H4 === 0) {
      for (let pi = openPositions.length - 1; pi >= 0; pi--) {
        const pos = openPositions[pi];
        if (pos.engine === "ST") {
    ...
    if (pendingReentries.length > 0) {
      for (let ri = pendingReentries.length - 1; ri >= 0; ri--) {
        ...
        else if (re.engine === "GARCH") sig = checkGarchV2Reentry(re.pair, t, re.dir, thr);
...
const configs: SimConfig[] = [
  {
    label: "1. Current",
    note: "1h>4.5/4h>3.0 lng | 1h<-3.0/4h<-3.0 sht | max10",
    thr: { longZ1h: 4.5, longZ4h: 3.0, shortZ1h: -3.0, shortZ4h: -3.0 },
    maxPos: 10,
  },
  {
    label: "2. Moderate",
    note: "1h>3.5/4h>2.5 lng | 1h<-2.5/4h<-2.5 sht | max10",
    thr: { longZ1h: 3.5, longZ4h: 2.5, shortZ1h: -2.5, shortZ4h: -2.5 },
    maxPos: 10,
  },
  {
    label: "3. Relaxed",
    note: "1h>3.0/4h>2.0 lng | 1h<-2.0/4h<-2.0 sht | max10",
    thr: { longZ1h: 3.0, longZ4h: 2.0, shortZ1h: -2.0, shortZ4h: -2.0 },
    maxPos: 10,
  },
  {
    label: "4. Aggressive",
    note: "1h>2.5/4h>2.0 lng | 1h<-2.0/4h<-2.0 sht | max10",
    thr: { longZ1h: 2.5, longZ4h: 2.0, shortZ1h: -2.0, shortZ4h: -2.0 },
    maxPos: 10,
  },
  {
    label: "5. Asymmetric",
    note: "1h>3.5/4h>2.5 lng | 1h<-3.0/4h<-3.0 sht | max10",
    thr: { longZ1h: 3.5, longZ4h: 2.5, shortZ1h: -3.0, shortZ4h: -3.0 },
    maxPos: 10,
  },
  {
    label: "6. Cur+max15",
    note: "1h>4.5/4h>3.0 lng | 1h<-3.0/4h<-3.0 sht | max15",
    thr: { longZ1h: 4.5, longZ4h: 3.0, shortZ1h: -3.0, shortZ4h: -3.0 },
    maxPos: 15,
  },
  {
    label: "7. Cur+max20",
    note: "1h>4.5/4h>3.0 lng | 1h<-3.0/4h<-3.0 sht | max20",
    thr: { longZ1h: 4.5, longZ4h: 3.0, shortZ1h: -3.0, shortZ4h: -3.0 },
    maxPos: 20,
  },
];
...
// --------------- run all configs ---------------
console.log(`Running ${configs.length} configs (trail ${TRAIL_ACT}/${TRAIL_DIST} + re-entry, GARCH $${SIZE_GARCH} + ST $${SIZE_ST})...\n`);
...
for (const cfg of configs) {
  ...
  const { trades, reentries, blocked } = runSim(cfg.thr, cfg.maxPos, TRAIL_ACT, TRAIL_DIST, FULL_START, FULL_END);
  ...
  results.push({
    ...
    tradesPerDay: trades.length / fullDays,
    reentries,
    wr: m.wr,
    pf: m.pf,
    total: m.total,
    ...
    oosPf: oosM.pf,
    oosWr: oosM.wr,
    garchTrades: garchTrades.length,
    garchPerDay: garchTotal / fullDays,
    garchPf: garchGl > 0 ? garchGp / garchGl : 99,
    stTrades: stTrades.length,
    stPerDay: stTotal / fullDays,
...
const W = 170;
console.log("\n" + "=".repeat(W));
console.log("GARCH v2 Z-SCORE THRESHOLD SWEEP  |  GARCH $15 + ST $5  |  Trail 20/3 + re-entry  |  23 pairs  |  2023-01 to 2026-03");
console.log("OOS: 2025-09+");
...
for (const r of results) {
  console.log([
    ...
    r.garchPf.toFixed(2).padStart(8),
...
console.log("\nConfig notes:");
for (const r of results) {
  console.log(`  ${r.label.padEnd(14)} ${r.note}`);
}

// Summary: best by OOS $/day
