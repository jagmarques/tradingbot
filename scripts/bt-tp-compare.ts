...
const PAIRS=["OPUSDT","ARBUSDT","LDOUSDT","TRUMPUSDT","DOTUSDT","ENAUSDT","DOGEUSDT","APTUSDT","LINKUSDT","ADAUSDT","WLDUSDT","XRPUSDT","SOLUSDT","BNBUSDT","kSHIBUSDT","TIAUSDT","NEARUSDT","kBONKUSDT","ONDOUSDT","HYPEUSDT"];
...
function sim(tp:number,sl:number,pdm:Map<string,D>,btc:D,s:number,e:number):{tr:T[],finalEq:number,minEq:number}{
  ...
  for(const t of st){const cl=new Set<string>();
    for(const[p,ps]of pos){const d=pds.get(p)!;const bi=d.tm.get(t)??-1;if(bi<0)continue;const b=d.cs[bi];const sp=SP[p]??4e-4;let xp=0;
      ...
      if(xp>0){const raw=ps.dir==="long"?(xp/ps.ep-1)*ps.sz*LEV:(ps.ep/xp-1)*ps.sz*LEV;const pnl=raw-ps.sz*LEV*FEE*2;tr.push({pnl,et:ps.et,xt:t});equity+=pnl;if(equity<minEq)minEq=equity;pos.delete(p);cl.add(p)}}
    const sz=Math.max(10,Math.floor(equity*4/30));
    for(const[p,d]of pds){if(pos.has(p)||cl.has(p))continue;const bi=d.tm.get(t)??-1;if(bi<60)continue;
      ...
      const dc=[...pos.values()].filter(x=>x.dir===dir).length;if(dc>=6)continue;
      const en=d.cs[bi].c;const sp=SP[p]??4e-4;const ent=dir==="long"?en*(1+sp):en*(1-sp);
      pos.set(p,{pair:p,dir,ep:ent,et:t,sl:dir==="long"?ent*(1-sl):ent*(1+sl),tp:dir==="long"?ent*(1+tp):ent*(1-tp),sz})}}
...
const pdm=new Map<string,D>();for(const p of[...PAIRS,"BTCUSDT"]){const d=pre(p);if(d)pdm.set(p,d)}const btc=pdm.get("BTCUSDT")!;

console.log("TP%    SL%    Trades  T/day  WR%    FinalEq   MinEq  MaxDD%  $/day    Train    Test");
console.log("-".repeat(95));

for(const[tpPct,slPct]of[
  [0.01, 0.03], // 1% TP, 3% SL (what user thought)
  [0.02, 0.03], // 2% TP
  [0.03, 0.03], // 3% TP (1:1 R:R)
  [0.04, 0.03], // 4% TP
  [0.05, 0.03], // 5% TP
  [0.06, 0.03], // 6% TP
  [0.08, 0.03], // 8% TP
  [0.10, 0.03], // 10% TP (current)
  [0.12, 0.03], // 12% TP
  [0.15, 0.03], // 15% TP
  // Also test different SL with best TPs
  [0.03, 0.01], // 3% TP, 1% SL
  [0.03, 0.02], // 3% TP, 2% SL
  [0.05, 0.02], // 5% TP, 2% SL
  [0.05, 0.01], // 5% TP, 1% SL
]){
  const{tr,finalEq,minEq}=sim(tpPct,slPct,pdm,btc,S,E);
  if(tr.length<10)continue;
  const pnl=tr.reduce((s,t)=>s+t.pnl,0);const w=tr.filter(t=>t.pnl>0).length;
  let cum=200,pk=200,dd=0;
  for(const t of tr){cum+=t.pnl;if(cum>pk)pk=cum;if((pk-cum)/pk*100>dd)dd=(pk-cum)/pk*100}
  const trn=tr.filter(t=>t.xt<M);const tst=tr.filter(t=>t.xt>=M);
  const trnD=trn.reduce((a,t)=>a+t.pnl,0)/((M-S)/DAY);const tstD=tst.reduce((a,t)=>a+t.pnl,0)/((E-M)/DAY);
  console.log(
    `${(tpPct*100).toFixed(0).padStart(3)}%  ${(slPct*100).toFixed(0).padStart(3)}%  ${String(tr.length).padStart(6)}  ${(tr.length/days).toFixed(1).padStart(5)}  ${(w/tr.length*100).toFixed(1).padStart(5)}  $${finalEq.toFixed(0).padStart(7)}  $${minEq.toFixed(0).padStart(5)}  ${dd.toFixed(1).padStart(5)}%  $${(pnl/days).toFixed(2).padStart(7)}  $${trnD.toFixed(2).padStart(7)}  $${tstD.toFixed(2).padStart(7)}`
  );
}
