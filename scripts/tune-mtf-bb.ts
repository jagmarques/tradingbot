// MTF + BB Squeeze pre-filter: only enter when BB width is in bottom 30th percentile (squeeze).
// Compare results vs tune-mtf-deep.ts baseline (+0.271%/day, Sharpe 4.39).
// Walk-forward: 240d train / ~125d test. 0.29% round-trip, 10x leverage.
// Run: npx tsx scripts/tune-mtf-bb.ts
...
interface Params { dailySmaPeriod: number; dailyAdxMin: number; rsiPullbackLow: number; rsiPullbackHigh: number; rsiTurnDelta: number; stopAtrMult: number; rrRatio: number; stagnationH: number; }
...
interface H4PreInd { atr: (number | null)[]; rsi: (number | null)[]; bbWidths: (number | null)[]; }
...
function precompute4h(candles: Candle[]): H4PreInd {
  ...
  return { atr: atrArr, rsi: rsiArr, bbWidths: computeBBWidth(candles) };
}

const LEV = 10, FEE_RATE = 0.00045 * 2, SLIPPAGE_RATE = 0.001 * 2, TOTAL_COST = FEE_RATE + SLIPPAGE_RATE, STARTING_BALANCE = 100;

function runBacktest(candles4h: Candle[], pre4h: H4PreInd, dailyCandles: Candle[], preDaily: DailyPreInd, idxDailyAt: number[], p: Params, startIdx = 50, endIdx?: number): BacktestResult {
  ...
  for (let i = startIdx; i < end; i++) {
    ...
    if (pos !== null) {
      ...
      const rsi4h = pre4h.rsi[i], rsiPrev = i > 0 ? pre4h.rsi[i - 1] : null;
      if (rsi4h === null || rsiPrev === null) continue;
      const dailyUptrend = dailyClose > dailySma;
      const dailyDowntrend = dailyClose < dailySma;
      let dir: "long" | "short" | null = null;
      if (dailyUptrend && rsi4h >= p.rsiPullbackLow && rsi4h <= p.rsiPullbackHigh && rsi4h > rsiPrev + p.rsiTurnDelta) dir = "long";
      const shortLow = 100 - p.rsiPullbackHigh, shortHigh = 100 - p.rsiPullbackLow;
      if (dailyDowntrend && rsi4h >= shortLow && rsi4h <= shortHigh && rsi4h < rsiPrev - p.rsiTurnDelta) dir = "short";
      if (dir !== null && i + 1 < end) {
...
const RSI_PB_LOW_VALS = [25, 30, 35, 40];
const RSI_PB_HIGH_VALS = [45, 50, 55, 60];
const RSI_TURN_VALS = [1, 2, 3];
  ...
  for (const dailyAdxMin of DAILY_ADX_VALS)
    for (const rsiPullbackLow of RSI_PB_LOW_VALS)
      for (const rsiPullbackHigh of RSI_PB_HIGH_VALS)
        if (rsiPullbackLow < rsiPullbackHigh - 5)
          for (const rsiTurnDelta of RSI_TURN_VALS)
            PHASE1_GRID.push({ dailySmaPeriod, dailyAdxMin, rsiPullbackLow, rsiPullbackHigh, rsiTurnDelta, stopAtrMult: 2.0, rrRatio: 3.0, stagnationH: 12 });

async function main() {
  const p2Count = STOP_ATR_VALS.length * RR_VALS.length * STAG_VALS.length;
  console.log(`=== tune-mtf-bb.ts: MTF + BB Squeeze filter, ${PAIRS.length} pairs, ${DAYS}d ===`);
  console.log(`Baseline (no filter): +0.271%/day, Sharpe 4.39`);
  ...
  console.log(`  Best Phase 1: sma=${best1.dailySmaPeriod} adx=${best1.dailyAdxMin} rsiLow=${best1.rsiPullbackLow} rsiHigh=${best1.rsiPullbackHigh} turn=${best1.rsiTurnDelta} | train: ${phase1[0].trainReturn.toFixed(2)}%`);
  ...
  results.forEach((r, i) => {
    const p = r.params;
    console.log(`${String(i + 1).padStart(2)}  ${r.profitPerDay >= 0 ? "+" : ""}${r.profitPerDay.toFixed(3)}  ${r.testReturn >= 0 ? "+" : ""}${r.testReturn.toFixed(2)}%    ${r.trainReturn >= 0 ? "+" : ""}${r.trainReturn.toFixed(2)}%   ${String(r.testTrades).padStart(6)}  ${r.testWinRate.toFixed(0)}%  ${r.testMaxDrawdown.toFixed(1)}%  ${r.testSharpe >= 0 ? " " : ""}${r.testSharpe.toFixed(2)}   sma=${p.dailySmaPeriod} adx=${p.dailyAdxMin} rsiLow=${p.rsiPullbackLow} rsiHigh=${p.rsiPullbackHigh} turn=${p.rsiTurnDelta} stop=${p.stopAtrMult} rr=${p.rrRatio} stag=${p.stagnationH * 4}h`);
  });
