/**
 * Deep trail optimization around the winning tight trail config
 * Also test SL variants, different activation points, and hybrid approaches
 */
...
function runSim(pairs: PD[], trail: {a:number;d:number}[], beAt: number, slPct: number, label: string) {
  ...
  console.log(`  ${label.padEnd(50)} ${String(sorted.length).padStart(5)} ${tpd.toFixed(1).padStart(5)} ${(wr.toFixed(1)+"%").padStart(6)} ${pf.toFixed(2).padStart(5)} ${fmtPnl(pd).padStart(8)} $${maxDD.toFixed(0).padStart(3)} ${String(slN).padStart(5)} ${String(beN).padStart(4)} ${String(trN).padStart(5)}`);
}

function main() {
  console.log("=".repeat(110));
  console.log("  DEEP TRAIL + SL OPTIMIZATION - $5 margin, real leverage");
  console.log("=".repeat(110));
  console.log("\n  Loading...");
  const pairs: PD[] = [];
  for (const n of ALL) {
  ...
  const hdr = `  ${"Config".padEnd(50)} ${"Trd".padStart(5)} ${"T/d".padStart(5)} ${"WR%".padStart(6)} ${"PF".padStart(5)} ${"$/day".padStart(8)} ${"MDD".padStart(4)} ${"SL".padStart(5)} ${"BE".padStart(4)} ${"Trl".padStart(5)}`;

  // ─── Baseline + previous winner ───
  console.log("--- BASELINES ---\n");
  console.log(hdr); console.log("  " + "-".repeat(105));
  runSim(pairs, [{a:10,d:5},{a:15,d:4},{a:20,d:3},{a:25,d:2},{a:35,d:1.5},{a:50,d:1}], 2, 0.005, "OLD: 10/5 6-stage BE2% SL0.5%");
  runSim(pairs, [{a:3,d:1.5},{a:5,d:1},{a:10,d:0.5},{a:20,d:0.5}], 0, 0.005, "PREV WINNER: 3/1.5->5/1->10/0.5->20/0.5 noBE");

  // ─── Tight trail variants around winner ───
  console.log("\n--- TIGHT TRAIL VARIANTS (no BE, SL 0.5%) ---\n");
  console.log(hdr); console.log("  " + "-".repeat(105));

  // Vary the first step activation
  runSim(pairs, [{a:2,d:1},{a:4,d:0.75},{a:8,d:0.5},{a:15,d:0.5}], 0, 0.005, "2/1->4/0.75->8/0.5->15/0.5");
  runSim(pairs, [{a:2,d:1.5},{a:5,d:1},{a:10,d:0.5},{a:20,d:0.5}], 0, 0.005, "2/1.5->5/1->10/0.5->20/0.5");
  runSim(pairs, [{a:2,d:1},{a:5,d:0.75},{a:10,d:0.5},{a:20,d:0.5}], 0, 0.005, "2/1->5/0.75->10/0.5->20/0.5");
  runSim(pairs, [{a:3,d:1},{a:5,d:0.75},{a:10,d:0.5},{a:20,d:0.5}], 0, 0.005, "3/1->5/0.75->10/0.5->20/0.5");
  runSim(pairs, [{a:3,d:1.5},{a:6,d:1},{a:10,d:0.5},{a:20,d:0.5}], 0, 0.005, "3/1.5->6/1->10/0.5->20/0.5");
  runSim(pairs, [{a:3,d:1.5},{a:5,d:1},{a:8,d:0.5},{a:15,d:0.5}], 0, 0.005, "3/1.5->5/1->8/0.5->15/0.5");
  runSim(pairs, [{a:3,d:2},{a:5,d:1},{a:10,d:0.5},{a:20,d:0.5}], 0, 0.005, "3/2->5/1->10/0.5->20/0.5");
  runSim(pairs, [{a:4,d:1.5},{a:7,d:1},{a:12,d:0.5},{a:25,d:0.5}], 0, 0.005, "4/1.5->7/1->12/0.5->25/0.5");
  runSim(pairs, [{a:4,d:2},{a:8,d:1},{a:15,d:0.5},{a:30,d:0.5}], 0, 0.005, "4/2->8/1->15/0.5->30/0.5");

  // More stages
  runSim(pairs, [{a:2,d:1.5},{a:3,d:1},{a:5,d:0.75},{a:8,d:0.5},{a:15,d:0.5},{a:25,d:0.5}], 0, 0.005, "6-stage: 2/1.5->3/1->5/0.75->8/0.5->15/0.5->25/0.5");
  runSim(pairs, [{a:3,d:1.5},{a:5,d:1},{a:7,d:0.75},{a:10,d:0.5},{a:15,d:0.5},{a:25,d:0.5}], 0, 0.005, "6-stage: 3/1.5->5/1->7/0.75->10/0.5->15/0.5->25/0.5");
  runSim(pairs, [{a:2,d:1},{a:4,d:0.75},{a:6,d:0.5},{a:10,d:0.5},{a:20,d:0.5}], 0, 0.005, "5-stage: 2/1->4/0.75->6/0.5->10/0.5->20/0.5");

  // ─── SL variants with tight trail ───
  console.log("\n--- SL VARIANTS WITH TIGHT TRAIL (no BE) ---\n");
  console.log(hdr); console.log("  " + "-".repeat(105));

  const bestTrail = [{a:3,d:1.5},{a:5,d:1},{a:10,d:0.5},{a:20,d:0.5}];
  for (const sl of [0.003, 0.004, 0.005, 0.006, 0.007, 0.008, 0.01]) {
    runSim(pairs, bestTrail, 0, sl, `SL ${(sl*100).toFixed(1)}% + tight trail`);
  }

  // ─── Tight trail + small BE (best of both?) ───
  console.log("\n--- TIGHT TRAIL + LOW BE ---\n");
  console.log(hdr); console.log("  " + "-".repeat(105));

  runSim(pairs, bestTrail, 1, 0.005, "Tight trail + BE 1%");
  runSim(pairs, bestTrail, 1.5, 0.005, "Tight trail + BE 1.5%");
  runSim(pairs, bestTrail, 2, 0.005, "Tight trail + BE 2%");
  runSim(pairs, [{a:2,d:1},{a:4,d:0.75},{a:8,d:0.5},{a:15,d:0.5}], 1, 0.005, "2/1->4/0.75->8/0.5->15/0.5 + BE 1%");

  // ─── Max hold variants ───
  console.log("\n--- MAX HOLD VARIANTS (tight trail, no BE) ---\n");
  console.log(hdr); console.log("  " + "-".repeat(105));

  // Override MAX_HOLD by using the sim as-is (72h is hardcoded, would need param)
  // Instead test different trail tightening at later stages
  runSim(pairs, [{a:3,d:1.5},{a:5,d:1},{a:10,d:0.5},{a:20,d:0.25}], 0, 0.005, "Tighter late: 3/1.5->5/1->10/0.5->20/0.25");
  runSim(pairs, [{a:3,d:1.5},{a:5,d:1},{a:10,d:0.5},{a:20,d:0.5},{a:40,d:0.25}], 0, 0.005, "+40/0.25 stage");
  runSim(pairs, [{a:3,d:1.5},{a:5,d:1},{a:10,d:0.5},{a:20,d:0.5},{a:50,d:0.25}], 0, 0.005, "+50/0.25 stage");

  // ─── Cooldown variants with best trail ───
  console.log("\n--- COOLDOWN WITH BEST TRAIL ---\n");
  console.log(hdr); console.log("  " + "-".repeat(105));
  // Note: CD_H is hardcoded at 1, would need param for sweep
  // Just test the best configs we have
  runSim(pairs, bestTrail, 0, 0.005, "Best tight trail (CD 1h baseline)");
}
