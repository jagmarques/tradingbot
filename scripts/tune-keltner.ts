// Keltner Channel Breakout: enter when price breaks outside EMA ± ATR*mult bands.
// Similar to BBSqueeze but uses ATR-based channel instead of standard deviation.
// Walk-forward: 240d train / ~125d test. 0.29% round-trip, 10x leverage.
// Run: npx tsx scripts/tune-keltner.ts
...
interface Params { dailySmaPeriod: number; dailyAdxMin: number; emaPeriod: number; atrPeriod: number; atrMult: number; stopAtrMult: number; rrRatio: number; stagnationH: number; }
...
interface KeltnerBands { upper: (number | null)[]; lower: (number | null)[]; }
interface H4PreInd { atr14: (number | null)[]; keltner: Record<string, KeltnerBands>; }

function computeKeltner(candles: Candle[], emaPeriod: number, atrPeriod: number, atrMult: number): KeltnerBands {
  ...
  const atrRaw = ATR.calculate({ high: highs, low: lows, close: closes, period: atrPeriod });
  const upper: (number | null)[] = new Array(n).fill(null);
  const lower: (number | null)[] = new Array(n).fill(null);
  const emaOffset = n - emaRaw.length, atrOffset = n - atrRaw.length;
  for (let i = Math.max(emaOffset, atrOffset); i < n; i++) {
    const e = emaRaw[i - emaOffset], a = atrRaw[i - atrOffset];
    if (e !== undefined && a !== undefined) { upper[i] = e + atrMult * a; lower[i] = e - atrMult * a; }
  }
  return { upper, lower };
}
...
function precompute4h(candles: Candle[], emaPeriods: number[], atrPeriods: number[], atrMults: number[]): H4PreInd {
  const n = candles.length;
  const closes = candles.map(c => c.close), highs = candles.map(c => c.high), lows = candles.map(c => c.low);
  ...
  const keltner: Record<string, KeltnerBands> = {};
  for (const e of emaPeriods) for (const a of atrPeriods) for (const m of atrMults)
    keltner[`${e}_${a}_${m}`] = computeKeltner(candles, e, a, m);
  return { atr14, keltner };
}
...
function runBacktest(candles4h: Candle[], pre4h: H4PreInd, dailyCandles: Candle[], preDaily: DailyPreInd, idxDailyAt: number[], p: Params, startIdx = 80, endIdx?: number): BacktestResult {
  ...
  const kelt = pre4h.keltner[`${p.emaPeriod}_${p.atrPeriod}_${p.atrMult}`];
  if (!kelt) return { trades: 0, wins: 0, totalReturn: 0, maxDrawdown: 0, tradePnlPcts: [], days: 0 };
  ...
  for (let i = startIdx; i < end; i++) {
    ...
    if (pos !== null) {
      ...
      const upper = kelt.upper[i], prevUpper = kelt.upper[i - 1];
      const lower = kelt.lower[i], prevLower = kelt.lower[i - 1];
      if (upper === null || prevUpper === null || lower === null || prevLower === null) continue;
      const dailyUptrend = dailyClose > dailySma, dailyDowntrend = dailyClose < dailySma;
      let dir: "long" | "short" | null = null;
      // Breakout above upper channel = long
      if (dailyUptrend && candles4h[i - 1].close <= (prevUpper ?? 0) && c.close > upper) dir = "long";
      // Breakdown below lower channel = short
      if (dailyDowntrend && candles4h[i - 1].close >= (prevLower ?? 0) && c.close < lower) dir = "short";
      if (dir !== null && i + 1 < end) {
...
const EMA_PERIOD_VALS = [10, 15, 20, 25];
const ATR_PERIOD_VALS = [10, 14, 20];
const ATR_MULT_VALS = [1.5, 2.0, 2.5, 3.0];
...
for (const dailySmaPeriod of DAILY_SMA_VALS)
  for (const dailyAdxMin of DAILY_ADX_VALS)
    for (const emaPeriod of EMA_PERIOD_VALS)
      for (const atrPeriod of ATR_PERIOD_VALS)
        for (const atrMult of ATR_MULT_VALS)
          PHASE1_GRID.push({ dailySmaPeriod, dailyAdxMin, emaPeriod, atrPeriod, atrMult, stopAtrMult: 2.5, rrRatio: 3.0, stagnationH: 12 });

async function main() {
  const p2Count = STOP_ATR_VALS.length * RR_VALS.length * STAG_VALS.length;
  console.log(`=== tune-keltner.ts: Keltner Channel Breakout, ${PAIRS.length} pairs, ${DAYS}d ===`);
  ...
  for (const pair of PAIRS) {
    try {
      ...
      const pre4h = precompute4h(h4, EMA_PERIOD_VALS, ATR_PERIOD_VALS, ATR_MULT_VALS);
  ...
  console.log(`  Best: sma=${best1.dailySmaPeriod} adx=${best1.dailyAdxMin} EMA(${best1.emaPeriod}) ATR(${best1.atrPeriod},${best1.atrMult}) | train: ${phase1[0].trainReturn.toFixed(2)}%`);
  ...
  results.forEach((r, i) => {
    const p = r.params;
    console.log(`${String(i + 1).padStart(2)}  ${r.profitPerDay >= 0 ? "+" : ""}${r.profitPerDay.toFixed(3)}  ${r.testReturn >= 0 ? "+" : ""}${r.testReturn.toFixed(2)}%  ${String(r.testTrades).padStart(6)}  ${r.testWinRate.toFixed(0)}%  ${r.testMaxDrawdown.toFixed(1)}%  ${r.testSharpe >= 0 ? " " : ""}${r.testSharpe.toFixed(2)}   sma=${p.dailySmaPeriod} adx=${p.dailyAdxMin} EMA(${p.emaPeriod}) ATR(${p.atrPeriod},${p.atrMult}) stop=${p.stopAtrMult} rr=${p.rrRatio} stag=${p.stagnationH * 4}h`);
  });
