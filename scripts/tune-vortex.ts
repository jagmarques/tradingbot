// Vortex: V+ and V- crossover measures positive vs negative trend movement.
// Walk-forward: 240d train / ~125d test. 0.29% round-trip, 10x leverage.
// Run: npx tsx scripts/tune-vortex.ts
...
interface Params { dailySmaPeriod: number; dailyAdxMin: number; vortexPeriod: number; stopAtrMult: number; rrRatio: number; stagnationH: number; }
...
interface H4PreInd { atr: (number | null)[]; vortex: Record<number, { vip: (number | null)[]; vim: (number | null)[] }>; }
...
// VI+[i] = sum(VM+, period) / sum(TR, period), VM+[i] = abs(high[i] - low[i-1])
function computeVortex(candles: Candle[], period: number): { vip: (number | null)[]; vim: (number | null)[] } {
  const n = candles.length;
  const vip: (number | null)[] = new Array(n).fill(null);
  const vim: (number | null)[] = new Array(n).fill(null);
  for (let i = period; i < n; i++) {
    let sumVmp = 0, sumVmm = 0, sumTr = 0;
    for (let k = i - period + 1; k <= i; k++) {
      sumVmp += Math.abs(candles[k].high - candles[k - 1].low);
      sumVmm += Math.abs(candles[k].low - candles[k - 1].high);
      const tr = Math.max(
    ...
    if (sumTr > 0) { vip[i] = sumVmp / sumTr; vim[i] = sumVmm / sumTr; }
  }
  return { vip, vim };
}

function precompute4h(candles: Candle[], vortexPeriods: number[]): H4PreInd {
  const n = candles.length;
  ...
  const vortex: Record<number, { vip: (number | null)[]; vim: (number | null)[] }> = {};
  for (const period of vortexPeriods) vortex[period] = computeVortex(candles, period);
  return { atr, vortex };
}
...
function runBacktest(candles4h: Candle[], pre4h: H4PreInd, dailyCandles: Candle[], preDaily: DailyPreInd, idxDailyAt: number[], p: Params, startIdx = 80, endIdx?: number): BacktestResult {
  ...
  for (let i = startIdx; i < end; i++) {
    ...
    if (pos !== null) {
      ...
      const vx = pre4h.vortex[p.vortexPeriod];
      if (!vx) continue;
      const currVip = vx.vip[i], prevVip = vx.vip[i - 1], currVim = vx.vim[i], prevVim = vx.vim[i - 1];
      if (currVip === null || prevVip === null || currVim === null || prevVim === null) continue;
      const dailyUptrend = dailyClose > dailySma, dailyDowntrend = dailyClose < dailySma;
      let dir: "long" | "short" | null = null;
      if (dailyUptrend && prevVip <= prevVim && currVip > currVim) dir = "long";
      if (dailyDowntrend && prevVim <= prevVip && currVim > currVip) dir = "short";
      if (dir !== null && i + 1 < end) {
...
const VORTEX_PERIOD_VALS = [7, 10, 14, 21, 28];

interface TrainResult { params: Params; trainReturn: number; }

// Phase 1: sma x adx x vortexPeriod = 4x4x5 = 80 combos
const PHASE1_GRID: Params[] = [];
for (const dailySmaPeriod of DAILY_SMA_VALS)
  for (const dailyAdxMin of DAILY_ADX_VALS)
    for (const vortexPeriod of VORTEX_PERIOD_VALS)
      PHASE1_GRID.push({ dailySmaPeriod, dailyAdxMin, vortexPeriod, stopAtrMult: 2.5, rrRatio: 3.0, stagnationH: 12 });

async function main() {
  const p2Count = STOP_ATR_VALS.length * RR_VALS.length * STAG_VALS.length;
  console.log(`=== tune-vortex.ts: Vortex Indicator, ${PAIRS.length} pairs, ${DAYS}d ===`);
  ...
  for (const pair of PAIRS) {
    try {
      ...
      const pre4h = precompute4h(h4, VORTEX_PERIOD_VALS);
  ...
  console.log(`  Best: sma=${best1.dailySmaPeriod} adx=${best1.dailyAdxMin} vortex=${best1.vortexPeriod} | train: ${phase1[0].trainReturn.toFixed(2)}%`);
  ...
  results.forEach((r, i) => {
    const p = r.params;
    console.log(`${String(i + 1).padStart(2)}  ${r.profitPerDay >= 0 ? "+" : ""}${r.profitPerDay.toFixed(3)}  ${r.testReturn >= 0 ? "+" : ""}${r.testReturn.toFixed(2)}%  ${String(r.testTrades).padStart(6)}  ${r.testWinRate.toFixed(0)}%  ${r.testMaxDrawdown.toFixed(1)}%  ${r.testSharpe >= 0 ? " " : ""}${r.testSharpe.toFixed(2)}   sma=${p.dailySmaPeriod} adx=${p.dailyAdxMin} vortex=${p.vortexPeriod} stop=${p.stopAtrMult} rr=${p.rrRatio} stag=${p.stagnationH * 4}h`);
  });
