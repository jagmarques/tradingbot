// HMA High-Quality: picks top winner per ADX level so high-ADX params get Phase 2 coverage.
// Walk-forward: 240d train / ~125d test. 0.29% round-trip, 10x leverage.
// Run: npx tsx scripts/tune-hma-hq.ts
...
function precompute4h(candles: Candle[], hmaPeriods: number[]): H4PreInd {
  ...
  for (const p of hmaPeriods) hma[p] = computeHMA(closes, p);
...
function runBacktest(candles4h: Candle[], pre4h: H4PreInd, dailyCandles: Candle[], preDaily: DailyPreInd, idxDailyAt: number[], p: Params, startIdx = 100, endIdx?: number): BacktestResult {
  ...
  const fastArr = pre4h.hma[p.hmaFast], slowArr = pre4h.hma[p.hmaSlow];
...
const HMA_FAST_VALS = [4, 6, 8, 10, 13];
const HMA_SLOW_VALS = [13, 21, 34, 55];
  ...
  for (const dailyAdxMin of DAILY_ADX_VALS)
    for (const hmaFast of HMA_FAST_VALS)
      for (const hmaSlow of HMA_SLOW_VALS)
        if (hmaFast < hmaSlow)
          PHASE1_GRID.push({ dailySmaPeriod, dailyAdxMin, hmaFast, hmaSlow, stopAtrMult: 2.5, rrRatio: 3.0, stagnationH: 12 });

async function main() {
  const p2Count = STOP_ATR_VALS.length * RR_VALS.length * STAG_VALS.length;
  console.log(`=== tune-hma-hq.ts: HMA High-Quality (ADX 14-25), ${PAIRS.length} pairs, ${DAYS}d ===`);
  ...
  // Pick top-1 per ADX level
  const byAdx = new Map<number, TrainResult[]>();
  for (const r of phase1) {
    if (!byAdx.has(r.params.dailyAdxMin)) byAdx.set(r.params.dailyAdxMin, []);
    byAdx.get(r.params.dailyAdxMin)!.push(r);
  }
  const phase1Winners: TrainResult[] = [];
  for (const [adx, results] of byAdx) {
    results.sort((a, b) => b.trainReturn - a.trainReturn);
    const winner = results[0];
    phase1Winners.push(winner);
    console.log(`  ADX=${adx} best: sma=${winner.params.dailySmaPeriod} HMA(${winner.params.hmaFast}/${winner.params.hmaSlow}) | train: ${winner.trainReturn.toFixed(2)}%`);
  }
  ...
  const viabilityThreshold = 0.30;
  results.forEach((r, idx) => {
    const calmar = r.profitPerDay / (r.testMaxDrawdown || 1);
    console.log(
      `${String(idx + 1).padStart(2)}  ${r.profitPerDay >= 0 ? "+" : ""}${r.profitPerDay.toFixed(3)}  ${r.testReturn >= 0 ? "+" : ""}${r.testReturn.toFixed(2)}%  ${String(r.testTrades).padStart(6)}  ${r.testWinRate.toFixed(0)}%  ${r.testMaxDrawdown.toFixed(1)}%  ${calmar.toFixed(2)}   ${r.testSharpe.toFixed(2)}   sma=${r.params.dailySmaPeriod} adx=${r.params.dailyAdxMin} HMA(${r.params.hmaFast}/${r.params.hmaSlow}) stop=${r.params.stopAtrMult} rr=${r.params.rrRatio} stag=${r.params.stagnationH * 4}h`
    );
  });
  ...
  console.log(best.profitPerDay >= viabilityThreshold ? "VIABLE" : "NOT VIABLE");
}
