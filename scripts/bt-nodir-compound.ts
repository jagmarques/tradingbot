...
// 16 without losers
const P16=["OPUSDT","WIFUSDT","ARBUSDT","TRUMPUSDT","DOTUSDT","ENAUSDT","APTUSDT","LINKUSDT","ADAUSDT","WLDUSDT","XRPUSDT","SOLUSDT","BNBUSDT","NEARUSDT","kBONKUSDT","HYPEUSDT"];

function report(name:string,pairs:string[],maxDir:number){
  const{tr,finalEq,minEq}=sim(pairs,maxDir,4,pdm,btc,S,E);
  ...
  const trnPnl=trnTr.reduce((s,t)=>s+t.pnl,0)/((M-S)/DAY);
  const tstPnl=tstTr.reduce((s,t)=>s+t.pnl,0)/((E-M)/DAY);
  const cap=maxDir>0?String(maxDir):"none";
  console.log(`${name.padEnd(22)} ${cap.padStart(5)}  $${finalEq.toFixed(0).padStart(7)}  $${minEq.toFixed(0).padStart(5)}  ${dd.toFixed(1).padStart(5)}%  ${String(tr.length).padStart(5)}  ${(tr.length/days).toFixed(1).padStart(4)}  ${(w/tr.length*100).toFixed(1).padStart(5)}  $${trnPnl.toFixed(2).padStart(6)}  $${tstPnl.toFixed(2).padStart(7)}`);

  // Per-pair if no cap
  if(maxDir===0){
    const pairData=new Map<string,{n:number;w:number;pnl:number;gl:number}>();
    for(const t of tr){const s=pairData.get(t.pair)||{n:0,w:0,pnl:0,gl:0};s.n++;s.pnl+=t.pnl;if(t.pnl>0)s.w++;else s.gl+=Math.abs(t.pnl);pairData.set(t.pair,s)}
    const sorted=[...pairData.entries()].sort((a,b)=>b[1].pnl-a[1].pnl);
    console.log("  Per-pair:");
    for(const[p,s]of sorted){
      const v=s.pnl<0?" <-- LOSE":"";
      console.log(`    ${p.replace("USDT","").padEnd(8)} ${String(s.n).padStart(4)}tr WR=${(s.w/s.n*100).toFixed(0)}% ${(s.pnl>=0?"+$":"-$")+Math.abs(s.pnl).toFixed(0).padStart(6)}${v}`);
    }
  }
}

console.log("\nSet                    MaxD  FinalEq  MinEq  MaxDD  Trades T/day  WR%    Train    Test");
console.log("-".repeat(105));

report("21 pairs",P21,4);
report("21 pairs",P21,6);
report("21 pairs",P21,8);
report("21 pairs NO CAP",P21,0);
console.log("");
report("16 pairs (no losers)",P16,4);
report("16 pairs (no losers)",P16,6);
report("16 pairs (no losers)",P16,8);
report("16 pairs NO CAP",P16,0);
