// Elder Impulse: EMA direction + MACD histogram direction agreement.
// Walk-forward: 240d train / ~125d test. 0.29% round-trip, 10x leverage.
// Run: npx tsx scripts/tune-elder-impulse.ts
...
interface H4PreInd {
  ...
  macd: Record<string, { line: number | null; hist: number | null }[]>;
}
...
function runBacktest(candles4h: Candle[], pre4h: H4PreInd, dailyCandles: Candle[], preDaily: DailyPreInd, idxDailyAt: number[], p: Params, startIdx = 80, endIdx?: number): BacktestResult {
  ...
  for (let i = startIdx; i < end; i++) {
    ...
    if (pos !== null) {
      ...
      const isBullish = emaRising && histRising;
      const isBearish = emaFalling && histFalling;
      // Check prev bar state
...
async function main() {
  ...
  console.log(`=== tune-elder-impulse.ts: Elder Impulse System, ${PAIRS.length} pairs, ${DAYS}d ===`);
  ...
  console.log(`  Best: sma=${best1.dailySmaPeriod} adx=${best1.dailyAdxMin} ema(${best1.emaPeriod}) macd(${best1.macdFast},${best1.macdSlow},${best1.macdSignal}) | train: ${phase1[0].trainReturn.toFixed(2)}%`);
  ...
  results.forEach((r, i) => {
    const p = r.params;
    console.log(`${String(i + 1).padStart(2)}  ${r.profitPerDay >= 0 ? "+" : ""}${r.profitPerDay.toFixed(3)}  ${r.testReturn >= 0 ? "+" : ""}${r.testReturn.toFixed(2)}%  ${String(r.testTrades).padStart(6)}  ${r.testWinRate.toFixed(0)}%  ${r.testMaxDrawdown.toFixed(1)}%  ${r.testSharpe >= 0 ? " " : ""}${r.testSharpe.toFixed(2)}   sma=${p.dailySmaPeriod} adx=${p.dailyAdxMin} ema(${p.emaPeriod}) macd(${p.macdFast},${p.macdSlow},${p.macdSignal}) stop=${p.stopAtrMult} rr=${p.rrRatio} stag=${p.stagnationH * 4}h`);
  });
