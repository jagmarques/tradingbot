...
function sim(pairs:string[],maxDir:number,pdm:Map<string,D>,btc:D,s:number,e:number):T[]{
...
const P15=["OPUSDT","WIFUSDT","ARBUSDT","LDOUSDT","TRUMPUSDT","DASHUSDT","DOTUSDT","ENAUSDT","DOGEUSDT","APTUSDT","LINKUSDT","ADAUSDT","WLDUSDT","XRPUSDT","UNIUSDT"];
...
console.log("\nSet                           MaxDir  Trades  T/day  WR%    PnL       $/day  Sharpe  MaxDD  Train   Test");
console.log("-".repeat(110));

for(const[name,pairs]of[["15 pairs",P15],["21 pairs",P21]]as[string,string[]][]){
  for(const maxDir of [4,5,6,7,8,10,99]){
    const tr=sim(pairs,maxDir,pdm,btc,S,E);const pnl=tr.reduce((s,t)=>s+t.pnl,0);const w=tr.filter(t=>t.pnl>0).length;
    ...
    const trn=sim(pairs,maxDir,pdm,btc,S,M);const tst=sim(pairs,maxDir,pdm,btc,M,E);
    const trnD=trn.reduce((a,t)=>a+t.pnl,0)/((M-S)/DAY);const tstD=tst.reduce((a,t)=>a+t.pnl,0)/((E-M)/DAY);
    const cap=maxDir===99?"none":String(maxDir);
    console.log(
      `${name.padEnd(29)} ${cap.padStart(7)}  ${String(tr.length).padStart(6)}  ${(tr.length/days).toFixed(1).padStart(5)}  ${(w/tr.length*100).toFixed(1).padStart(5)}  ${(pnl>=0?"+$":"-$")+(Math.abs(pnl).toFixed(0)).padStart(5)}  ${("$"+(pnl/days).toFixed(2)).padStart(6)}  ${sh.toFixed(2).padStart(6)}  ${("$"+dd.toFixed(0)).padStart(5)}  $${trnD.toFixed(2).padStart(5)}  $${tstD.toFixed(2).padStart(5)}`
    );
  }
  console.log("");
}
