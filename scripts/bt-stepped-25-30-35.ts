 ...
 * No trail | flat 30/3 | stepped 25/6->30/3->35/1 | stepped 22/6->28/2->33/1
 ...
 *
 * Run: NODE_OPTIONS="--max-old-space-size=8192" npx tsx scripts/bt-stepped-25-30-35.ts
 */
...
// --------------- trail modes ---------------
type TrailMode = "none" | "flat30_3" | "stepped25" | "stepped22";
...
// stepped22: >22->6%, >28->2%, >33->1%
function steppedTrailDist22(peakPct: number): number {
    ...
    if (is5m && openPositions.length > 0) {
      for (let pi = openPositions.length - 1; pi >= 0; pi--) {
        ...
        if (trailMode === "flat30_3") {
          ...
          const dist = steppedTrailDist22(pos.peakPnlPct);
          if (dist > 0) {
            const currPct = pos.dir === "long"
              ? (bar.c / pos.entryPrice - 1) * LEV * 100
              : (pos.entryPrice / bar.c - 1) * LEV * 100;
...
for (const r of results.slice(1)) {
  ...
  console.log(
    `  ${r.label.padEnd(33)}  $/day ${dPerDay >= 0 ? "+" : ""}${dPerDay.toFixed(3)}` +
