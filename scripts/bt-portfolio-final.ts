/**
 * PORTFOLIO FINAL — combine breakthrough GARCH tight-SL + ExtRev (uncorrelated).
 *
 * Also test more ExtRev variations, more tight SL combos.
 *
 * Target: $5/day MDD < $20 on $60 account.
 * Current bests:
 *   - GARCH z4/6_22 SL0.08% m$20 = $1.71/day MDD $19 PF 2.01
 *   - ExtRev 3sigma SL0.1% m$10 = $1.65/day MDD $38 PF 1.58
 *
 * Run: cd "/Users/jagmarques/Library/CloudStorage/OneDrive/Work/.TradingBot" && NODE_OPTIONS="--max-old-space-size=12288" npx tsx scripts/bt-portfolio-final.ts
 */
  ...
  for (const ts of timepoints) {
    ...
    for (const eng of engines) {
      for (const p of pairs) {
        ...
        // Prevent same engine+pair from opening twice
...
function main() {
  ...
  console.log("  PORTFOLIO FINAL — GARCH tight-SL + ExtRev stacked");
  console.log("=".repeat(140));
  ...
  for (const n of ALL_PAIRS) {
    ...
    pairs.push({
      name: n,
      ind: { h1, h4, m5, h1Map, h4Map, z1, z4, ret1h, stdev1h_168 },
      sp: SP[n] ?? DSP, lev,
    });
  }
  ...
  // Engine definitions
  const GARCH_BEST = { id: "G", kind: "garch" as const, margin: 20, slPct: 0.0008, zL1: 4, zS1: -6, zL4: 2, zS4: -2, maxHoldH: 72 };
  const GARCH_SAFE = { id: "G", kind: "garch" as const, margin: 15, slPct: 0.0008, zL1: 4, zS1: -6, zL4: 2, zS4: -2, maxHoldH: 72 };
  const GARCH_SMALL = { id: "G", kind: "garch" as const, margin: 10, slPct: 0.0008, zL1: 4, zS1: -6, zL4: 2, zS4: -2, maxHoldH: 72 };
  const GARCH_Z34 = { id: "G", kind: "garch" as const, margin: 20, slPct: 0.0008, zL1: 4, zS1: -6, zL4: 3, zS4: -3, maxHoldH: 72 };

  const EXTREV_3S_M10 = { id: "X", kind: "extrev" as const, margin: 10, slPct: 0.001, sigmaMult: 3, maxHoldH: 12 };
  const EXTREV_3S_M15 = { id: "X", kind: "extrev" as const, margin: 15, slPct: 0.001, sigmaMult: 3, maxHoldH: 12 };
  const EXTREV_3S_M20 = { id: "X", kind: "extrev" as const, margin: 20, slPct: 0.001, sigmaMult: 3, maxHoldH: 12 };
  const EXTREV_4S_M10 = { id: "X", kind: "extrev" as const, margin: 10, slPct: 0.001, sigmaMult: 4, maxHoldH: 12 };
  const EXTREV_4S_M15 = { id: "X", kind: "extrev" as const, margin: 15, slPct: 0.001, sigmaMult: 4, maxHoldH: 12 };
  const EXTREV_5S_M10 = { id: "X", kind: "extrev" as const, margin: 10, slPct: 0.001, sigmaMult: 5, maxHoldH: 12 };
  const EXTREV_5S_M20 = { id: "X", kind: "extrev" as const, margin: 20, slPct: 0.001, sigmaMult: 5, maxHoldH: 12 };

  const tests: Array<{ label: string; engines: EngineCfg[] }> = [
    // Singles (baselines)
    { label: "GARCH best alone m$20", engines: [GARCH_BEST] },
    { label: "GARCH safe alone m$15", engines: [GARCH_SAFE] },
    { label: "GARCH small alone m$10", engines: [GARCH_SMALL] },
    { label: "GARCH z34 alone m$20", engines: [GARCH_Z34] },
    { label: "ExtRev 3s alone m$10", engines: [EXTREV_3S_M10] },
    { label: "ExtRev 3s alone m$15", engines: [EXTREV_3S_M15] },
    { label: "ExtRev 3s alone m$20", engines: [EXTREV_3S_M20] },
    { label: "ExtRev 4s alone m$10", engines: [EXTREV_4S_M10] },
    { label: "ExtRev 5s alone m$10", engines: [EXTREV_5S_M10] },

    // Portfolios
    { label: "PF1: GARCH m$20 + ExtRev3s m$10", engines: [GARCH_BEST, EXTREV_3S_M10] },
    { label: "PF2: GARCH m$15 + ExtRev3s m$15", engines: [GARCH_SAFE, EXTREV_3S_M15] },
    { label: "PF3: GARCH m$10 + ExtRev3s m$10", engines: [GARCH_SMALL, EXTREV_3S_M10] },
    { label: "PF4: GARCH m$10 + ExtRev5s m$10", engines: [GARCH_SMALL, EXTREV_5S_M10] },
    { label: "PF5: GARCH m$15 + ExtRev4s m$10", engines: [GARCH_SAFE, EXTREV_4S_M10] },
    { label: "PF6: GARCH m$20 + ExtRev5s m$20", engines: [GARCH_BEST, EXTREV_5S_M20] },
    { label: "PF7: GARCH z34 m$20 + ExtRev3s m$15", engines: [GARCH_Z34, EXTREV_3S_M15] },
    { label: "PF8: GARCH m$10 + ExtRev3s m$15", engines: [GARCH_SMALL, EXTREV_3S_M15] },
    { label: "PF9: GARCH m$15 + ExtRev3s m$20", engines: [GARCH_SAFE, EXTREV_3S_M20] },
    { label: "PF10: GARCH m$20 + ExtRev3s m$15", engines: [GARCH_BEST, EXTREV_3S_M15] },
  ];

  const hdr = `${"Config".padEnd(45)} ${"$/day".padStart(9)} ${"MDD".padStart(8)} ${"PF".padStart(5)} ${"WR%".padStart(6)} ${"MaxL".padStart(8)} ${"N".padStart(6)}`;
  console.log("\n" + hdr);
  console.log("-".repeat(140));
  ...
  for (const t of tests) {
    ...
    console.log(`${t.label.padEnd(45).slice(0, 45)} ${fmtD(res.dollarsPerDay).padStart(9)} ${("$" + res.maxDD.toFixed(0)).padStart(8)} ${res.pf.toFixed(2).padStart(5)} ${res.wr.toFixed(1).padStart(6)} ${fmtD(res.maxSingleLoss).padStart(8)} ${String(res.numTrades).padStart(6)}`);
  }
  ...
  for (const r of byDay.slice(0, 15)) {
    console.log(`${r.label.padEnd(45).slice(0, 45)} ${fmtD(r.res.dollarsPerDay).padStart(9)} ${("$" + r.res.maxDD.toFixed(0)).padStart(8)} ${r.res.pf.toFixed(2).padStart(5)} ${r.res.wr.toFixed(1).padStart(6)} ${fmtD(r.res.maxSingleLoss).padStart(8)} ${String(r.res.numTrades).padStart(6)}`);
  }
  ...
  console.log("\n" + "=".repeat(140));
  console.log("TARGET CHECK ($/day >= 5 AND MDD < $20)");
  console.log("=".repeat(140));
