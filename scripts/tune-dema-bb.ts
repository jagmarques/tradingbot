// DEMA Cross + BB Squeeze pre-filter: only enter when BB width is in bottom 30th percentile (squeeze).
// Compare results vs tune-dema-deep.ts baseline (+0.415%/day, Sharpe 5.77).
// Walk-forward: 240d train / ~125d test. 0.29% round-trip, 10x leverage.
// Run: npx tsx scripts/tune-dema-bb.ts
...
interface Params { dailySmaPeriod: number; dailyAdxMin: number; demaFast: number; demaSlow: number; stopAtrMult: number; rrRatio: number; stagnationH: number; }
...
// BB Squeeze filter params (same as production BBSqueeze engine)
const BB_SQ_PERIOD = 15, BB_SQ_STDDEV = 1.5, BB_SQ_WINDOW = 50, BB_SQ_THRESH = 0.30;

function computeBBWidth(candles: Candle[]): (number | null)[] {
  const n = candles.length;
  const closes = candles.map((c) => c.close);
  const result: (number | null)[] = new Array(n).fill(null);
  for (let i = BB_SQ_PERIOD - 1; i < n; i++) {
    const sl = closes.slice(i - BB_SQ_PERIOD + 1, i + 1);
    const mean = sl.reduce((s, v) => s + v, 0) / BB_SQ_PERIOD;
    const std = Math.sqrt(sl.reduce((s, v) => s + (v - mean) ** 2, 0) / BB_SQ_PERIOD);
    result[i] = mean === 0 ? 0 : (std * 2 * BB_SQ_STDDEV) / mean;
  }
  return result;
}

function inSqueeze(bbWidths: (number | null)[], i: number): boolean {
  const curr = bbWidths[i];
  if (curr === null) return false;
  const lb: number[] = [];
  for (let j = Math.max(0, i - BB_SQ_WINDOW + 1); j <= i; j++) { const w = bbWidths[j]; if (w !== null) lb.push(w); }
  if (lb.length < 10) return false;
  const sorted = [...lb].sort((a, b) => a - b);
  return curr <= sorted[Math.floor(sorted.length * BB_SQ_THRESH)];
}

interface DailyPreInd { sma: Record<number, (number | null)[]>; adx: (number | null)[]; }
interface H4PreInd { atr: (number | null)[]; dema: Record<number, (number | null)[]>; bbWidths: (number | null)[]; }
...
function precompute4h(candles: Candle[], demaPeriods: number[]): H4PreInd {
  const n = candles.length;
  const closes = candles.map((c) => c.close);
  const highs = candles.map((c) => c.high);
  const lows = candles.map((c) => c.low);
  ...
  const dema: Record<number, (number | null)[]> = {};
  for (const period of demaPeriods) { dema[period] = computeDEMA(closes, period); }
  return { atr: atrArr, dema, bbWidths: computeBBWidth(candles) };
}
...
function runBacktest(candles4h: Candle[], pre4h: H4PreInd, dailyCandles: Candle[], preDaily: DailyPreInd, idxDailyAt: number[], p: Params, startIdx = 80, endIdx?: number): BacktestResult {
  ...
  const fastArr = pre4h.dema[p.demaFast];
  const slowArr = pre4h.dema[p.demaSlow];
  if (!fastArr || !slowArr) return { trades: 0, wins: 0, totalReturn: 0, maxDrawdown: 0, tradePnlPcts: [], days: 0 };
  ...
  for (let i = startIdx; i < end; i++) {
    ...
    if (pos !== null) {
      ...
      if (!inSqueeze(pre4h.bbWidths, i)) continue; // BB Squeeze pre-filter
      const currFast = fastArr[i], prevFast = fastArr[i - 1];
      const currSlow = slowArr[i], prevSlow = slowArr[i - 1];
      if (currFast === null || prevFast === null || currSlow === null || prevSlow === null) continue;
      const dailyUptrend = dailyClose > dailySma;
      const dailyDowntrend = dailyClose < dailySma;
      let dir: "long" | "short" | null = null;
      if (dailyUptrend && prevFast <= prevSlow && currFast > currSlow) dir = "long";
      if (dailyDowntrend && prevFast >= prevSlow && currFast < currSlow) dir = "short";
      if (dir !== null && i + 1 < end) {
...
const DEMA_FAST_VALS = [3, 5, 8, 13];
const DEMA_SLOW_VALS = [13, 21, 34, 55, 89];
...
for (const dailySmaPeriod of DAILY_SMA_VALS)
  for (const dailyAdxMin of DAILY_ADX_VALS)
    for (const demaFast of DEMA_FAST_VALS)
      for (const demaSlow of DEMA_SLOW_VALS)
        if (demaFast < demaSlow)
          PHASE1_GRID.push({ dailySmaPeriod, dailyAdxMin, demaFast, demaSlow, stopAtrMult: 2.5, rrRatio: 2.5, stagnationH: 12 });

async function main() {
  console.log(`=== tune-dema-bb.ts: DEMA Cross + BB Squeeze filter, ${PAIRS.length} pairs, ${DAYS}d ===`);
  console.log(`Baseline (no filter): +0.415%/day, Sharpe 5.77`);
  console.log(`BB filter: period=${BB_SQ_PERIOD} stddev=${BB_SQ_STDDEV} window=${BB_SQ_WINDOW} thresh=${BB_SQ_THRESH * 100}th pct`);
  ...
  for (const pair of PAIRS) {
    try {
      ...
      const pre4h = precompute4h(h4, ALL_DEMA_PERIODS);
  ...
  console.log(`  Best Phase 1: sma=${best1.dailySmaPeriod} adx=${best1.dailyAdxMin} DEMA(${best1.demaFast}/${best1.demaSlow}) | train: ${phase1[0].trainReturn.toFixed(2)}%`);

  console.log(`\nPhase 2 (${STOP_ATR_VALS.length * RR_VALS.length * STAG_VALS.length} combos)...`);
  ...
  results.forEach((r, i) => {
    const p = r.params;
    console.log(`${String(i + 1).padStart(2)}  ${r.profitPerDay >= 0 ? "+" : ""}${r.profitPerDay.toFixed(3)}  ${r.testReturn >= 0 ? "+" : ""}${r.testReturn.toFixed(2)}%    ${r.trainReturn >= 0 ? "+" : ""}${r.trainReturn.toFixed(2)}%   ${String(r.testTrades).padStart(6)}  ${r.testWinRate.toFixed(0)}%  ${r.testMaxDrawdown.toFixed(1)}%  ${r.testSharpe >= 0 ? " " : ""}${r.testSharpe.toFixed(2)}   sma=${p.dailySmaPeriod} adx=${p.dailyAdxMin} DEMA(${p.demaFast}/${p.demaSlow}) stop=${p.stopAtrMult} rr=${p.rrRatio} stag=${p.stagnationH * 4}h`);
  });
  const best = results[0];
  const prev = 0.415;
  console.log(`\nBest WITH BB filter: ${best.profitPerDay >= 0 ? "+" : ""}${best.profitPerDay.toFixed(4)}%/day, Sharpe ${best.testSharpe.toFixed(2)}, trades ${best.testTrades}`);
  console.log(`Baseline WITHOUT filter: +${prev}%/day`);
  if (best.testSharpe > 1.0 && best.testTrades > 30) {
    console.log(`VIABLE. Filter ${best.profitPerDay > prev ? `IMPROVES (+${(best.profitPerDay - prev).toFixed(3)}%/day)` : `HURTS (${(best.profitPerDay - prev).toFixed(3)}%/day)`}`);
  } else { console.log("NOT VIABLE (too few trades)."); }
}
