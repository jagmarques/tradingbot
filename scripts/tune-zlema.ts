// ZLEMA Cross: Zero-lag EMA crossover with lag correction.
// Walk-forward: 240d train / ~125d test. 0.29% round-trip, 10x leverage.
// Run: npx tsx scripts/tune-zlema.ts
...
interface Params { dailySmaPeriod: number; dailyAdxMin: number; zlemaFast: number; zlemaSlow: number; stopAtrMult: number; rrRatio: number; stagnationH: number; }
...
interface H4PreInd { atr: (number | null)[]; zlema: Record<number, (number | null)[]>; }

function computeZLEMA(closes: number[], period: number): (number | null)[] {
  const n = closes.length;
  const lagOffset = Math.floor((period - 1) / 2);
  const corrected = closes.map((v, i) => i >= lagOffset ? v + (v - closes[i - lagOffset]) : v);
  const emaRaw = EMA.calculate({ values: corrected, period });
  const result: (number | null)[] = new Array(n).fill(null);
  emaRaw.forEach((v, i) => { result[n - emaRaw.length + i] = v; });
  return result;
}
...
function precompute4h(candles: Candle[], zlemaPeriods: number[]): H4PreInd {
  const n = candles.length;
  const closes = candles.map(c => c.close), highs = candles.map(c => c.high), lows = candles.map(c => c.low);
  ...
  const zlema: Record<number, (number | null)[]> = {};
  for (const p of zlemaPeriods) zlema[p] = computeZLEMA(closes, p);
  return { atr: atrArr, zlema };
}

const LEV = 10, FEE_RATE = 0.00045 * 2, SLIPPAGE_RATE = 0.001 * 2, TOTAL_COST = FEE_RATE + SLIPPAGE_RATE, STARTING_BALANCE = 100;

function runBacktest(candles4h: Candle[], pre4h: H4PreInd, dailyCandles: Candle[], preDaily: DailyPreInd, idxDailyAt: number[], p: Params, startIdx = 100, endIdx?: number): BacktestResult {
  ...
  const fastArr = pre4h.zlema[p.zlemaFast], slowArr = pre4h.zlema[p.zlemaSlow];
...
const ZLEMA_FAST_VALS = [3, 5, 8, 10, 13, 21];
const ZLEMA_SLOW_VALS = [13, 21, 34, 55, 89];
const STOP_ATR_VALS = [0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 4.0];
const RR_VALS = [1.5, 2.0, 2.5, 3.0, 3.5, 4.0, 5.0];
const STAG_VALS = [4, 6, 9, 12, 16, 24];
const ALL_ZLEMA_PERIODS = [...new Set([...ZLEMA_FAST_VALS, ...ZLEMA_SLOW_VALS])];
...
for (const dailySmaPeriod of DAILY_SMA_VALS)
  for (const dailyAdxMin of DAILY_ADX_VALS)
    for (const zlemaFast of ZLEMA_FAST_VALS)
      for (const zlemaSlow of ZLEMA_SLOW_VALS)
        if (zlemaFast < zlemaSlow)
          PHASE1_GRID.push({ dailySmaPeriod, dailyAdxMin, zlemaFast, zlemaSlow, stopAtrMult: 2.5, rrRatio: 3.0, stagnationH: 12 });

async function main() {
  const p2Count = STOP_ATR_VALS.length * RR_VALS.length * STAG_VALS.length;
  console.log(`=== tune-zlema.ts: ZLEMA Cross, ${PAIRS.length} pairs, ${DAYS}d ===`);
  ...
  for (const pair of PAIRS) {
    try {
      ...
      const pre4h = precompute4h(h4, ALL_ZLEMA_PERIODS);
  ...
  for (const params of PHASE1_GRID) {
    let total = 0;
    for (const pair of pairs) { const { h4, pre4h, dailyCandles, preDaily, idxDailyAt, trainEnd } = candleMap[pair]; total += runBacktest(h4, pre4h, dailyCandles, preDaily, idxDailyAt, params, 100, trainEnd).totalReturn; }
    phase1.push({ params, trainReturn: total / pairs.length });
  }
  phase1.sort((a, b) => b.trainReturn - a.trainReturn);
  const best1 = phase1[0].params;
  console.log(`  Best: sma=${best1.dailySmaPeriod} adx=${best1.dailyAdxMin} ZLEMA(${best1.zlemaFast}/${best1.zlemaSlow}) | train: ${phase1[0].trainReturn.toFixed(2)}%`);
  ...
  results.forEach((r, i) => {
    const p = r.params;
    console.log(`${String(i + 1).padStart(2)}  ${r.profitPerDay >= 0 ? "+" : ""}${r.profitPerDay.toFixed(3)}  ${r.testReturn >= 0 ? "+" : ""}${r.testReturn.toFixed(2)}%  ${String(r.testTrades).padStart(6)}  ${r.testWinRate.toFixed(0)}%  ${r.testMaxDrawdown.toFixed(1)}%  ${r.testSharpe >= 0 ? " " : ""}${r.testSharpe.toFixed(2)}   sma=${p.dailySmaPeriod} adx=${p.dailyAdxMin} ZLEMA(${p.zlemaFast}/${p.zlemaSlow}) stop=${p.stopAtrMult} rr=${p.rrRatio} stag=${p.stagnationH * 4}h`);
  });
