/**
 * Deep sweep: untested levers on the winning trail (2/1.5)
 * Tests: MOM_LB, VOL_WIN, SL widths, max hold, mc, block hours
 * Fixed: $15 margin, cd2h
 */
...
// Precompute z-scores for all lb/vw combos
const lbValues = [1, 2, 3, 5];
const vwValues = [15, 20, 30, 40, 50];
...
const rawH1: Map<string, Candle[]> = new Map();
const rawH4: Map<string, Candle[]> = new Map();
  ...
  for (const ts of timepoints) {
    ...
    for (let i = openPositions.length - 1; i >= 0; i--) {
      ...
      if (!exitPrice && isH1Boundary) {
        ...
        if (pos.peakLevPnlPct >= TRAIL[0]!.a && currentLevPnl <= pos.peakLevPnlPct - TRAIL[0]!.d) {
      ...
      if (exitPrice > 0) {
        ...
        if (reason === "sl") cooldowns.set(`${pos.pair}:long`, ts + CD_H * H);
      }
    }
    ...
    if (cfg.blockHours.has(new Date(ts).getUTCHours())) continue;
    ...
    for (const p of pairs) {
      ...
      if (h1Idx === undefined || h1Idx < cfg.vw + cfg.lb + 2) continue;

      const z1hArr = zCache1h.get(`${p.name}:${cfg.lb}:${cfg.vw}`)!;
      const z1 = z1hArr[h1Idx - 1]!;
      if (z1 <= cfg.z1h) continue;

      const z4hArr = zCache4h.get(`${p.name}:${cfg.lb}:${cfg.vw}`)!;
...
// Sweep dimensions
const mcValues = [3, 5, 7, 10, 15, 999];
const slConfigs = [
...
const zConfigs = [
  { z1h: 1.0, z4h: 0.5, label: "z1.0/0.5" },
  { z1h: 1.5, z4h: 1.0, label: "z1.5/1.0" },
  { z1h: 2.0, z4h: 1.5, label: "z2.0/1.5" },
  { z1h: 2.5, z4h: 2.0, label: "z2.5/2.0" },
];
const maxHoldValues = [24, 48, 72, 120, 999];
const blockConfigs = [
  { hours: new Set([22, 23]), label: "b22-23" },
  { hours: new Set<number>(), label: "noblock" },
];

const total = lbValues.length * vwValues.length * mcValues.length * slConfigs.length * zConfigs.length * maxHoldValues.length * blockConfigs.length;
    ...
    for (const mc of mcValues) {
      for (const sl of slConfigs) {
        for (const zc of zConfigs) {
          for (const mh of maxHoldValues) {
            for (const bc of blockConfigs) {
              count++;
              if (count % 100 === 0) process.stdout.write(`\r${count}/${total}`);
              const r = simulate({ lb, vw, mc, slLow: sl.low, slHigh: sl.high, z1h: zc.z1h, z4h: zc.z4h, maxHoldH: mh, blockHours: bc.hours });
              if (r.trades < 50) continue;
              results.push({
                label: `lb${lb}/vw${vw} mc${mc} ${sl.label} ${zc.label} mh${mh} ${bc.label}`,
                ...r,
              });
            }
          }
