/**
 * TIGHT SL SCALING: the breakthrough finding was SL 0.10% + trail 9/0.5 = $0.46/day PF 2.03 MDD $8.
 * Scale this across margin levels, test other tight SL values, combine with looser z-thresholds
 * to hit $5/day target.
 *
 * Also tests the cross-sectional momentum signal from research agent as an uncorrelated add-on.
 *
 * Run: cd "/Users/jagmarques/Library/CloudStorage/OneDrive/Work/.TradingBot" && NODE_OPTIONS="--max-old-space-size=12288" npx tsx scripts/bt-tight-sl-scale.ts
 */
...
function buildConfigs(): Cfg[] {
  ...
  // Tight SL scaling across margins and z thresholds
  const slValues = [0.0008, 0.001, 0.0012, 0.0015, 0.002];
  const margins = [10, 15, 20, 25, 30, 40, 50];
  const zCombos = [
    { l1: 4, s1: -6, l4: 3, s4: -3, name: "z4/6_34" },
    { l1: 4, s1: -6, l4: 2, s4: -2, name: "z4/6_22" },
    { l1: 5, s1: -6, l4: 2, s4: -2, name: "z5/6_22" },
    { l1: 3, s1: -6, l4: 2, s4: -2, name: "z3/6_22" },
    { l1: 3, s1: -5, l4: 2, s4: -2, name: "z3/5_22" },
    { l1: 2, s1: -5, l4: 1.5, s4: -1.5, name: "z2/5_1515" },
  ];
  ...
  for (const z of zCombos) {
    for (const sl of slValues) {
      for (const m of margins) {
        out.push({
          label: `${z.name} SL${(sl * 100).toFixed(2)}% m$${m}`,
          margin: m,
          slPct: sl,
          trailAct: 9, trailDist: 0.5,
          zLong1h: z.l1, zShort1h: z.s1, zLong4h: z.l4, zShort4h: z.s4,
          beAt: 7,
        });
      }
    }
...
function main() {
  ...
  console.log("  TIGHT SL SCALING — exploit PF 2.03 breakthrough from tight SL");
  console.log(`  Target: $5/day MDD<$20 on $60 account. Current best: $0.46/day MDD $8 PF 2.03`);
  console.log("=".repeat(140));
  ...
  // Target check
  console.log("\n" + "=".repeat(140));
  console.log("CONFIGS MEETING TARGET ($/day >= 5, MDD < $20)");
  console.log("=".repeat(140));
  console.log(hdr);
  ...
  if (winners.length === 0) {
    ...
    for (const r of winners) {
      console.log(`${r.cfg.label.padEnd(45).slice(0, 45)} ${fmtD(r.res.dollarsPerDay).padStart(9)} ${("$" + r.res.maxDD.toFixed(0)).padStart(8)} ${r.res.pf.toFixed(2).padStart(5)} ${r.res.wr.toFixed(1).padStart(6)} ${fmtD(r.res.maxSingleLoss).padStart(8)} ${String(r.res.numTrades).padStart(6)}`);
    }
  }

  // TOP 20 by $/day
  console.log("\n" + "=".repeat(140));
  console.log("TOP 20 BY $/DAY (any DD)");
  console.log("=".repeat(140));
  console.log(hdr);
  ...
  // MDD < $20 TOP 20
  console.log("\n" + "=".repeat(140));
  console.log("SAFE (MDD < $20) TOP 20 by $/day");
  console.log("=".repeat(140));
  console.log(hdr);
