/**
 * Does Range Expansion add value to the 3-engine config, or is GARCH-auto-bear better?
 *
 * A. 3-engine (A+B+D at $7/$5/$7) - current config
 * B. 3-engine + Range Expansion (A+B+D+E at $7/$5/$7/$3)
 * C. 3-engine + GARCH auto-bear (A+B+D + C only when BTC 30d<-10% AND Fear<25 AND EMA bearish)
 *
 * Run: cd "/Users/jagmarques/Library/CloudStorage/OneDrive/Work/.TradingBot" && npx tsx scripts/bt-without-range.ts
 */
...
interface ComboConfig {
  ...
  garchBearOnly?: boolean; // C = true means GARCH only fires when bear regime active
}
...
// Fear proxy: realized volatility of BTC daily returns over 14 days
// Higher volatility in a downtrend = fear. Scale: 0-100 where lower = more fear.
// We use: if 14d realized vol > 3% AND BTC trending down => "fear < 25" equivalent
let btcDailyVolMap: Map<number, number>; // dayTs -> 14d realized vol (annualized pct)
...
function loadAllData() {
  ...
  // Precompute BTC 14-day realized vol as fear proxy
  btcDailyVolMap = new Map();
  for (let i = 14; i < btc.daily.length; i++) {
    const rets: number[] = [];
    for (let j = i - 13; j <= i; j++) {
      rets.push(Math.log(btc.daily[j].c / btc.daily[j - 1].c));
    }
    const mean = rets.reduce((a, b) => a + b, 0) / rets.length;
    const variance = rets.reduce((s, r) => s + (r - mean) ** 2, 0) / (rets.length - 1);
    const annVol = Math.sqrt(variance) * Math.sqrt(365) * 100; // annualized pct
    btcDailyVolMap.set(btc.daily[i].t, annVol);
  }
...
function runBacktest(config: ComboConfig, spreadMultiplier: number): Trade[] {
  ...
  const garchBearOnly = config.garchBearOnly ?? false;

  const positions = new Map<string, Position>();
  const trades: Trade[] = [];
  let lastCarryRebalance = 0;
  let garchActiveDays = 0;
  let totalDays = 0;
  ...
  // Bear regime gate for GARCH: BTC 30d < -10% AND Fear < 25 proxy AND EMA bearish
  function isBearRegime(t: number): boolean {
    const ret30d = btc30dRet(t);
    if (ret30d >= -0.10) return false; // need 30d return < -10%

    // EMA bearish: EMA20 < EMA50
    if (btcBullish(t)) return false; // must be bearish

    // Fear proxy: high realized vol in a downtrend = fear
    // We use 14d annualized vol > 60% as "Fear < 25" equivalent
    // (crypto Fear & Greed < 25 typically corresponds to high-vol sell-offs)
    const dayAligned = Math.floor(t / DAY) * DAY;
    const vol = btcDailyVolMap.get(dayAligned);
    if (vol === undefined) {
      // Try previous day
      const prevVol = btcDailyVolMap.get(dayAligned - DAY);
      if (prevVol === undefined || prevVol < 60) return false;
    } else if (vol < 60) {
      return false;
    }
  ...
  for (const dayT of dailyTimestamps) {
    ...
    // Track bear regime days for GARCH
    if (garchBearOnly && activeEngines.has("C") && isBearRegime(dayT)) {
      garchActiveDays++;
    }
    ...
    if (activeEngines.has("C")) {
      // If garchBearOnly, only fire on bear regime days
      const garchAllowed = !garchBearOnly || isBearRegime(dayT);
  ...
  if (garchBearOnly && activeEngines.has("C")) {
    console.log(`  [GARCH auto-bear] Active ${garchActiveDays}/${totalDays} days (${(garchActiveDays/totalDays*100).toFixed(1)}%)`);
  }
...
const COMBOS: ComboConfig[] = [
  ...
  {
    label: "B. 3-eng+RangeExp",
    engines: ["A","B","D","E"],
    maxPositions: 10,
    sizes: { A: 7, B: 5, D: 7, E: 3 },
  },
  {
    label: "C. 3-eng+GARCH-bear",
    engines: ["A","B","C","D"],
    maxPositions: 10,
    sizes: { A: 7, B: 5, C: 5, D: 7 },
    garchBearOnly: true,
  },
];

const ENGINE_NAMES: Record<string, string> = {
  A: "Donchian", B: "Supertrend", C: "GARCH", D: "Carry", E: "RangeExp",
};

// ─── Main ───────────────────────────────────────────────────────────
console.log("=".repeat(110));
console.log("  RANGE EXPANSION DEAD-WEIGHT TEST");
console.log("  A. 3-engine (A+B+D at $7/$5/$7) - current config");
console.log("  B. 3-engine + Range Expansion (A+B+D+E at $7/$5/$7/$3)");
console.log("  C. 3-engine + GARCH auto-bear (A+B+D+C, GARCH only when BTC 30d<-10% AND Fear<25 AND EMA bearish)");
...
for (const combo of COMBOS) {
  ...
  console.log(`  Sizes: ${combo.engines.map(e => e + "=$" + combo.sizes[e]).join(", ")}, MaxPos=${combo.maxPositions}`);
  if (combo.garchBearOnly) console.log(`  GARCH gate: bear-only (BTC 30d<-10%, Fear<25 proxy, EMA20<EMA50)`);
  console.log("#".repeat(110));
  ...
  results.push({ label: combo.label, fullStats, yearStats, trades });
}

// ─── RANKED COMPARISON TABLE ────────────────────────────────────────
console.log("\n\n" + "=".repeat(130));
console.log("  COMPARISON TABLE (sorted by $/day)");
...
const [configA, configB, configC] = [results[0], results[1], results[2]];

const rangeExpDelta = configB.fullStats.perDay - configA.fullStats.perDay;
const garchBearDelta = configC.fullStats.perDay - configA.fullStats.perDay;

console.log(`
Current 3-engine (A):       ${fmtPnl(configA.fullStats.perDay)}/day, PF ${configA.fullStats.pf}, Sharpe ${configA.fullStats.sharpe}, MaxDD $${configA.fullStats.maxDd.toFixed(0)}
+ Range Expansion (B):      ${fmtPnl(configB.fullStats.perDay)}/day, PF ${configB.fullStats.pf}, Sharpe ${configB.fullStats.sharpe}, MaxDD $${configB.fullStats.maxDd.toFixed(0)}
+ GARCH auto-bear (C):      ${fmtPnl(configC.fullStats.perDay)}/day, PF ${configC.fullStats.pf}, Sharpe ${configC.fullStats.sharpe}, MaxDD $${configC.fullStats.maxDd.toFixed(0)}

Range Expansion delta:      ${fmtPnl(rangeExpDelta)}/day (${rangeExpDelta > 0 ? "helps" : rangeExpDelta === 0 ? "neutral" : "hurts"})
GARCH auto-bear delta:      ${fmtPnl(garchBearDelta)}/day (${garchBearDelta > 0 ? "helps" : garchBearDelta === 0 ? "neutral" : "hurts"})
`);

const best = sorted[0];
if (best.label === configA.label) {
  console.log("RESULT: Current 3-engine config is optimal. Neither addition improves results.");
} else if (best.label === configB.label) {
  console.log("RESULT: Adding Range Expansion improves the portfolio.");
} else {
  console.log("RESULT: Adding GARCH in bear-only mode improves the portfolio.");
}

// Drawdown comparison
console.log(`\nDrawdown comparison:`);
console.log(`  A: $${configA.fullStats.maxDd.toFixed(0)} MaxDD, ${configA.fullStats.maxDdDuration} duration`);
console.log(`  B: $${configB.fullStats.maxDd.toFixed(0)} MaxDD, ${configB.fullStats.maxDdDuration} duration`);
console.log(`  C: $${configC.fullStats.maxDd.toFixed(0)} MaxDD, ${configC.fullStats.maxDdDuration} duration`);

// Risk-adjusted
const bestSharpe = sorted.reduce((best, r) => r.fullStats.sharpe > best.fullStats.sharpe ? r : best, sorted[0]);
console.log(`\nBest risk-adjusted (Sharpe): ${bestSharpe.label} -> ${bestSharpe.fullStats.sharpe}`);
