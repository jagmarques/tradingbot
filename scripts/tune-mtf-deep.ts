// MTF deep sweep: wider SMA, ADX, RSI pullback zone, and RSI turn delta search.
// Phase 1: 192 indicator combos. Phase 2: 60 risk/reward combos on best Phase-1 winner.
// Walk-forward: 240d train / ~125d test. 0.29% round-trip, 10x leverage.
// Run: npx tsx scripts/tune-mtf-deep.ts
...
interface H4PreInd { atr: (number | null)[]; rsi: (number | null)[]; }
...
async function main() {
  ...
  console.log(`=== tune-mtf-deep.ts: MTF deep sweep, ${PAIRS.length} pairs, ${DAYS}d ===`);
  ...
  if (best.testSharpe > 1.0 && best.testTrades > 30) {
    console.log(`VIABLE. vs prev MTF +${prev}%/day: ${best.profitPerDay > prev ? `BEATS IT (+${(best.profitPerDay - prev).toFixed(3)}%/day)` : "below"}`);
