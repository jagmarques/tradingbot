// PSAR v3: Calmar-sorted Phase 1+2 (train), top-30 tested, ADX up to 30.
// Run: npx tsx scripts/tune-psar-v3.ts
...
function precompute4h(candles: Candle[], stepVals: number[], maxVals: number[]): H4PreInd {
...
function runBacktest(candles4h: Candle[], pre4h: H4PreInd, dailyCandles: Candle[], preDaily: DailyPreInd, idxDailyAt: number[], p: Params, startIdx = 80, endIdx?: number): BacktestResult {
  ...
  for (let i = startIdx; i < end; i++) {
    ...
    if (pos !== null) {
      ...
      const key = `${p.psarStep}_${p.psarMax}`, psarArr = pre4h.psar[key];
      if (!psarArr) continue;
...
async function main() {
  ...
  console.log(`=== tune-psar-v3.ts: PSAR v3 (ADX 14-30, Calmar-sorted), ${PAIRS.length} pairs, ${DAYS}d ===`);
  ...
  for (const [adx, results] of byAdx) {
    ...
    console.log(`  ADX=${adx} best: sma=${w.params.dailySmaPeriod} psar(step=${w.params.psarStep},max=${w.params.psarMax}) | train: ${w.trainReturn.toFixed(2)}%`);
  }
  ...
  results.forEach((r, idx) => {
    ...
    console.log(`${String(idx + 1).padStart(2)}  ${r.profitPerDay >= 0 ? "+" : ""}${r.profitPerDay.toFixed(3)}  ${r.testReturn >= 0 ? "+" : ""}${r.testReturn.toFixed(2)}%  ${String(r.testTrades).padStart(6)}  ${r.testWinRate.toFixed(0)}%  ${r.testMaxDrawdown.toFixed(1)}%  ${calmar.toFixed(2)}   ${r.testSharpe.toFixed(2)}   sma=${r.params.dailySmaPeriod} adx=${r.params.dailyAdxMin} psar(step=${r.params.psarStep},max=${r.params.psarMax}) stop=${r.params.stopAtrMult} rr=${r.params.rrRatio} stag=${r.params.stagnationH * 4}h`);
  });
