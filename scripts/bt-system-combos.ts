/**
 * System Combo Backtest - Test 4-engine system running TOGETHER with different parameter combos
 *
 * Engines: A(Daily Donchian), B(4h Supertrend), C(GARCH v2 MTF), D(Carry Momentum)
 * All engines share the position pool, so interactions matter.
 * Run: cd "/Users/jagmarques/Library/CloudStorage/OneDrive/Work/.TradingBot" && npx tsx scripts/bt-system-combos.ts
 */
...
interface TrailConfig {
  // breakeven at bestPnlAtr >= breakevenAt
  breakevenAt: number;
  // trail distance = trailDist ATR when bestPnlAtr >= trailAt
  trailAt: number;
  trailDist: number;
  // tighter trail at bestPnlAtr >= tightTrailAt
  tightTrailAt: number;
  tightTrailDist: number;
}

interface SLConfig {
  atrMultAB: number;   // ATR multiplier for engines A/B
  pctC: number;        // SL % for engine C
  pctD: number;        // SL % for engine D
  maxSlPct: number;    // cap
}
...
interface SystemConfig {
  ...
  sl: SLConfig;
  trail: TrailConfig | null;   // null = no trailing
  tpC: number;                 // TP % for engine C (0.07 = 7%)
  tpAB: number;                // TP % for engines A/B (0 = none)
  maxPositions: number;
}
...
// ─── Run Backtest ───────────────────────────────────────────────────
function runBacktest(config: SystemConfig, spreadMultiplier: number): Trade[] {
  const MAX_POSITIONS = config.maxPositions;
  const sizes = config.sizes;
  const slCfg = config.sl;
  const trailCfg = config.trail;
  ...
  for (const dayT of dailyTimestamps) {
    ...
    for (const [key, pos] of [...positions.entries()]) {
      ...
      // ATR trailing ladder (configurable)
      if (trailCfg !== null && pos.atr > 0) {
        ...
        let newSl = pos.sl;
        if (pos.bestPnlAtr >= trailCfg.tightTrailAt) {
          const trailPrice = pos.dir === "long"
            ? pos.ep + (pos.bestPnlAtr - trailCfg.tightTrailDist) * pos.atr
            : pos.ep - (pos.bestPnlAtr - trailCfg.tightTrailDist) * pos.atr;
          newSl = pos.dir === "long" ? Math.max(pos.sl, trailPrice) : Math.min(pos.sl, trailPrice);
        } else if (pos.bestPnlAtr >= trailCfg.trailAt) {
          const trailPrice = pos.dir === "long"
            ? bar.h - trailCfg.trailDist * pos.atr
            : bar.l + trailCfg.trailDist * pos.atr;
          newSl = pos.dir === "long" ? Math.max(pos.sl, trailPrice) : Math.min(pos.sl, trailPrice);
        } else if (pos.bestPnlAtr >= trailCfg.breakevenAt) {
      ...
      if (pos.engine === "B") {
        ...
        if (eb) {
          // Check if supertrend flipped on any 4h bar within this day
          for (let h4Offset = 0; h4Offset < DAY; h4Offset += H4) {
            ...
            if (pos.dir === "long" && stNow === -1) {
              closePosition(key, bar.c, dayT); break;
            }
    ...
    for (const p of available) {
      ...
      let slDist = atrVal * slCfg.atrMultAB;
      if (slDist / ep > slCfg.maxSlPct) slDist = ep * slCfg.maxSlPct;
      const sl = dir === "long" ? ep - slDist : ep + slDist;

      let tp = 0;
      if (config.tpAB > 0) {
        tp = dir === "long" ? ep * (1 + config.tpAB) : ep * (1 - config.tpAB);
      }

      positions.set(key, {
        pair: p, engine: "A", dir, ep, et: dayT, sl, tp,
    ...
    for (let h4Offset = 0; h4Offset < DAY; h4Offset += H4) {
      ...
      for (const p of available) {
        ...
        positions.set(key, {
          pair: p, engine: "B", dir, ep, et: h4T, sl, tp,
    ...
    for (let h1Offset = 0; h1Offset < DAY; h1Offset += H1) {
      ...
      for (const p of available) {
        ...
        const slPct = slCfg.pctC;
        let slDist = ep * slPct;
        if (slDist / ep > slCfg.maxSlPct) slDist = ep * slCfg.maxSlPct;
        const sl = dir === "long" ? ep - slDist : ep + slDist;
        const tp = config.tpC > 0
          ? (dir === "long" ? ep * (1 + config.tpC) : ep * (1 - config.tpC))
          : 0;
...
const BASELINE_TRAIL: TrailConfig = {
  breakevenAt: 1,
  trailAt: 2,
  trailDist: 2,
  tightTrailAt: 3,
...
const TIGHTER_TRAIL: TrailConfig = {
  breakevenAt: 0.5,
  trailAt: 1.5,
  trailDist: 1.5,
  tightTrailAt: 2.5,
...
const LOOSER_TRAIL: TrailConfig = {
  breakevenAt: 2,
  trailAt: 3,
  trailDist: 2.5,
  tightTrailAt: 4,
...
const BASELINE_SL: SLConfig = { atrMultAB: 3, pctC: 0.03, pctD: 0.04, maxSlPct: 0.035 };

const COMBOS: SystemConfig[] = [
  {
    label: "1. Baseline",
  ...
  },
  {
    label: "5. Wider SL",
    sizes: { A: 7, B: 5, C: 3, D: 7 },
    sl: { atrMultAB: 4, pctC: 0.05, pctD: 0.05, maxSlPct: 0.05 },
    trail: BASELINE_TRAIL,
    tpC: 0.07, tpAB: 0,
    maxPositions: 20,
  },
  {
    label: "6. Tighter SL",
    sizes: { A: 7, B: 5, C: 3, D: 7 },
    sl: { atrMultAB: 2, pctC: 0.02, pctD: 0.03, maxSlPct: 0.035 },
    trail: BASELINE_TRAIL,
    tpC: 0.07, tpAB: 0,
    maxPositions: 20,
  },
  {
    label: "7. Equal sizing",
    sizes: { A: 5, B: 5, C: 5, D: 5 },
    sl: BASELINE_SL,
    trail: BASELINE_TRAIL,
    tpC: 0.07, tpAB: 0,
    maxPositions: 20,
  ...
  {
    label: "8. Supertrend heavy",
    sizes: { A: 3, B: 7, C: 3, D: 3 },
    sl: BASELINE_SL,
    trail: BASELINE_TRAIL,
    tpC: 0.07, tpAB: 0,
    maxPositions: 20,
...
console.log("  SYSTEM COMBO BACKTEST - 4-engine system with different parameter sets");
console.log("  18 pairs, 2023-01 to 2026-03, 5m candles, 20 max positions, 10x leverage");
...
for (const combo of COMBOS) {
  ...
  console.log(`  Sizes: A=$${combo.sizes["A"]}, B=$${combo.sizes["B"]}, C=$${combo.sizes["C"]}, D=$${combo.sizes["D"]}`);
  console.log(`  SL: ATR*${combo.sl.atrMultAB} (A/B), ${(combo.sl.pctC * 100).toFixed(0)}% (C), ${(combo.sl.pctD * 100).toFixed(0)}% (D) | cap ${(combo.sl.maxSlPct * 100).toFixed(1)}%`);
  if (combo.trail) {
    console.log(`  Trail: BE@${combo.trail.breakevenAt}x, ${combo.trail.trailDist}x@${combo.trail.trailAt}x, ${combo.trail.tightTrailDist}x@${combo.trail.tightTrailAt}x`);
  } else {
    console.log(`  Trail: NONE (SL + engine exit signals only)`);
  }
  console.log(`  TP: C=${(combo.tpC * 100).toFixed(0)}%, A/B=${combo.tpAB > 0 ? (combo.tpAB * 100).toFixed(0) + "%" : "none"}`);
  console.log("#".repeat(120));
  ...
  for (const eng of ["A", "B", "C", "D"]) {
    ...
    console.log(
      `${(eng + ": " + ENGINE_NAMES[eng]).padEnd(16)} ${pad(String(et.length), 6)} ${pad(epf.toFixed(2), 6)} ` +
  ...
  results.push({ label: combo.label, fullStats, yearStats, conservative: {} as Stats, trades, conservativeTrades: [] });
}

// ─── Run best combo with 2x spreads ─────────────────────────────────
const bestIdx = results.reduce((bi, r, i) => r.fullStats.perDay > results[bi].fullStats.perDay ? i : bi, 0);
const bestCombo = COMBOS[bestIdx];

console.log("\n\n" + "=".repeat(120));
console.log(`  ROBUSTNESS CHECK - Best combo (${bestCombo.label}) with 2x spreads`);
console.log("=".repeat(120));

const trades2x = runBacktest(bestCombo, 2.0);
const cons = computeStats(trades2x, FULL_START, FULL_END);
results[bestIdx].conservative = cons;
results[bestIdx].conservativeTrades = trades2x;

console.log("\n--- 2x Spread Full ---");
printHeader();
printStatsLine("2x Spread Full", cons);
for (const year of [2023, 2024, 2025, 2026]) {
  const ys = new Date(`${year}-01-01`).getTime();
  const ye = new Date(`${year + 1}-01-01`).getTime();
...
// Also run baseline (combo 1) with 2x for comparison
if (bestIdx !== 0) {
  console.log("\n--- Baseline (combo 1) with 2x Spread for comparison ---");
  const baselineTrades2x = runBacktest(COMBOS[0], 2.0);
  const baselineCons = computeStats(baselineTrades2x, FULL_START, FULL_END);
  results[0].conservative = baselineCons;
  results[0].conservativeTrades = baselineTrades2x;
  printHeader();
  printStatsLine("Baseline 2x", baselineCons);
  for (const year of [2023, 2024, 2025, 2026]) {
    const ys = new Date(`${year}-01-01`).getTime();
    const ye = new Date(`${year + 1}-01-01`).getTime();
    printStatsLine(`Baseline 2x ${year}`, computeStats(baselineTrades2x, ys, Math.min(ye, FULL_END)));
  }
}
...
Baseline:         ${baseline.label}  ->  ${fmtPnl(baseline.fullStats.perDay)}/day, PF ${baseline.fullStats.pf}, Sharpe ${baseline.fullStats.sharpe}

Delta vs baseline: ${fmtPnl(best.fullStats.perDay - baseline.fullStats.perDay)}/day (${baseline.fullStats.perDay !== 0 ? ((best.fullStats.perDay / baseline.fullStats.perDay - 1) * 100).toFixed(1) : "N/A"}%)

Year consistency (best):
  2023: ${fmtPnl(best.yearStats.get(2023)!.perDay)}/day  (PF ${best.yearStats.get(2023)!.pf})
  2024: ${fmtPnl(best.yearStats.get(2024)!.perDay)}/day  (PF ${best.yearStats.get(2024)!.pf})
  2025: ${fmtPnl(best.yearStats.get(2025)!.perDay)}/day  (PF ${best.yearStats.get(2025)!.pf})
  2026: ${fmtPnl(best.yearStats.get(2026)!.perDay)}/day  (PF ${best.yearStats.get(2026)!.pf})
`);

if (best.conservative.trades > 0) {
  console.log(`Best under 2x spread: ${fmtPnl(best.conservative.perDay)}/day, PF ${best.conservative.pf}`);
  if (best.conservative.perDay > 0) {
    console.log(`VERDICT: ${best.label} is robust (positive under 2x cost model).`);
  } else {
    console.log(`WARNING: Best config is NOT robust under doubled costs.`);
  }
}

// Check year consistency
const bestYears = [2023, 2024, 2025, 2026].map(y => best.yearStats.get(y)!.perDay);
const positiveYears = bestYears.filter(v => v > 0).length;
console.log(`Year consistency: ${positiveYears}/4 years positive`);
if (positiveYears === 4) {
  console.log("All years profitable - strong consistency.");
} else {
  console.log(`WARNING: ${4 - positiveYears} year(s) negative - watch for regime dependency.`);
}
