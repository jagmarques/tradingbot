// TRIX HQ: picks top winner per ADX level (14-25), Phase 2 for each.
// Run: npx tsx scripts/tune-trix-hq.ts
...
async function main() {
  ...
  console.log(`=== tune-trix-hq.ts: TRIX HQ (ADX 14-25), ${PAIRS.length} pairs, ${DAYS}d ===`);
  ...
  for (const [adx, results] of byAdx) {
    ...
    console.log(`  ADX=${adx} best: sma=${w.params.dailySmaPeriod} TRIX(${w.params.trixPeriod},sig=${w.params.signalPeriod}) | train: ${w.trainReturn.toFixed(2)}%`);
  }
