// TRIX v4: hybrid Phase 1 (raw return sort, top-3/ADX), extended signals+STAG, Phase 3 trail tuning.
// Run: npx tsx scripts/tune-trix-v4.ts
...
interface Params { dailySmaPeriod: number; dailyAdxMin: number; trixPeriod: number; signalPeriod: number; stopAtrMult: number; rrRatio: number; stagnationH: number; trailThreshold: number; trailRetrace: number; }
...
interface TrixSignal { trix: (number | null)[]; signal: (number | null)[]; }
interface H4PreInd { atr14: (number | null)[]; trixMap: Record<string, TrixSignal>; }

function computeTrix(closes: number[], trixPeriod: number, signalPeriod: number): TrixSignal {
  const n = closes.length;
  const trixArr: (number | null)[] = new Array(n).fill(null);
  const signalArr: (number | null)[] = new Array(n).fill(null);
  const ema1 = EMA.calculate({ values: closes, period: trixPeriod });
  const ema2 = EMA.calculate({ values: ema1, period: trixPeriod });
  const ema3 = EMA.calculate({ values: ema2, period: trixPeriod });
  const trixValues: number[] = [];
  const trixStartIdx: number[] = [];
  for (let i = 1; i < ema3.length; i++) {
    if (ema3[i - 1] !== 0) {
      const t = ((ema3[i] - ema3[i - 1]) / ema3[i - 1]) * 100;
      const closesIdx = n - ema3.length + i;
      trixArr[closesIdx] = t; trixValues.push(t); trixStartIdx.push(closesIdx);
    }
  }
  for (let i = signalPeriod - 1; i < trixValues.length; i++) {
    const mean = trixValues.slice(i - signalPeriod + 1, i + 1).reduce((s, v) => s + v, 0) / signalPeriod;
    signalArr[trixStartIdx[i]] = mean;
  }
  return { trix: trixArr, signal: signalArr };
}
...
function precompute4h(candles: Candle[], trixPeriods: number[], signalPeriods: number[]): H4PreInd {
  const n = candles.length, closes = candles.map(c => c.close), highs = candles.map(c => c.high), lows = candles.map(c => c.low);
  const atr14Raw = ATR.calculate({ high: highs, low: lows, close: closes, period: 14 });
  const atr14: (number | null)[] = new Array(n).fill(null);
  atr14Raw.forEach((v, i) => { atr14[n - atr14Raw.length + i] = v; });
  const trixMap: Record<string, TrixSignal> = {};
  for (const tp of trixPeriods) for (const sp of signalPeriods) trixMap[`${tp}_${sp}`] = computeTrix(closes, tp, sp);
  return { atr14, trixMap };
}
...
function runBacktest(candles4h: Candle[], pre4h: H4PreInd, dailyCandles: Candle[], preDaily: DailyPreInd, idxDailyAt: number[], p: Params, startIdx = 80, endIdx?: number): BacktestResult {
  ...
  const ts = pre4h.trixMap[`${p.trixPeriod}_${p.signalPeriod}`];
  if (!ts) return { trades: 0, wins: 0, totalReturn: 0, maxDrawdown: 0, tradePnlPcts: [], days: 0 };
  ...
  for (let i = startIdx; i < end; i++) {
    ...
    if (pos !== null) {
      ...
      const trailHit = pos.peak > p.trailThreshold && unrlPct <= pos.peak - p.trailRetrace;
      ...
      const currTrix = ts.trix[i], prevTrix = ts.trix[i - 1], currSignal = ts.signal[i], prevSignal = ts.signal[i - 1];
      if (currTrix === null || prevTrix === null || currSignal === null || prevSignal === null) continue;
      const dailyUptrend = dailyClose > dailySma, dailyDowntrend = dailyClose < dailySma;
      let dir: "long" | "short" | null = null;
      if (dailyUptrend && prevTrix <= prevSignal && currTrix > currSignal) dir = "long";
      if (dailyDowntrend && prevTrix >= prevSignal && currTrix < currSignal) dir = "short";
      if (dir !== null && i + 1 < end) {
        const entryPrice = candles4h[i + 1].open, atr = pre4h.atr14[i] ?? c.close * 0.02;
...
const TRIX_PERIOD_VALS = [5, 7, 8, 10, 12, 15, 18, 21];
const SIGNAL_PERIOD_VALS = [3, 5, 7, 9, 12, 14, 18];
const STOP_ATR_VALS = [0.5, 1.5, 2.0, 2.5, 3.0, 4.0];
const RR_VALS = [2.5, 3.0, 3.5, 4.0, 5.0];
const STAG_VALS = [6, 9, 12, 16, 24, 36, 48];
const TRAIL_THRESH_VALS = [3, 5, 7];
const TRAIL_RETRACE_VALS = [1.0, 1.5, 2.0];
...
for (const dailySmaPeriod of DAILY_SMA_VALS)
  for (const dailyAdxMin of DAILY_ADX_VALS)
    for (const trixPeriod of TRIX_PERIOD_VALS)
      for (const signalPeriod of SIGNAL_PERIOD_VALS)
        PHASE1_GRID.push({ dailySmaPeriod, dailyAdxMin, trixPeriod, signalPeriod, stopAtrMult: 2.5, rrRatio: 3.0, stagnationH: 12, trailThreshold: 5, trailRetrace: 2 });

async function main() {
  const p2Count = STOP_ATR_VALS.length * RR_VALS.length * STAG_VALS.length;
  const p3Count = TRAIL_THRESH_VALS.length * TRAIL_RETRACE_VALS.length;
  console.log(`=== tune-trix-v4.ts: TRIX v4 (hybrid: raw-return P1 top-3/ADX, Calmar P2, extended signals+STAG, Phase 3 trail), ${PAIRS.length} pairs, ${DAYS}d ===`);
  console.log(`Phase 1: ${PHASE1_GRID.length} combos (RAW return sort) | Phase 2: top-3/ADX × ${p2Count} risk combos (Calmar) | Phase 3: top-50 × ${p3Count} trail, top-20 tested`);
  ...
  for (const pair of PAIRS) {
    try {
      ...
      const pre4h = precompute4h(h4, TRIX_PERIOD_VALS, SIGNAL_PERIOD_VALS);
  ...
  console.log(`Phase 1 (${PHASE1_GRID.length} combos, RAW return sort, top-3 per ADX — hybrid)...`);
  ...
  for (const [adx, results] of byAdx) {
    results.sort((a, b) => b.trainReturn - a.trainReturn);  // RAW return sort (hybrid — Calmar P1 hurt TRIX in v3)
    const top3 = results.slice(0, 3);
    phase1Winners.push(...top3);
    const w = top3[0];
    console.log(`  ADX=${adx} #1: sma=${w.params.dailySmaPeriod} TRIX(${w.params.trixPeriod},sig=${w.params.signalPeriod}) | train: ${w.trainReturn.toFixed(2)}% MaxDD ${w.trainMaxDD.toFixed(1)}%`);
  }

  console.log(`\nPhase 2: ${phase1Winners.length} P1 winners × ${p2Count} risk combos (trail locked 5/2, Calmar-sorted, top-50)...`);
    ...
    for (const stopAtrMult of STOP_ATR_VALS)
      for (const rrRatio of RR_VALS)
        for (const stagnationH of STAG_VALS) {
          const params: Params = { ...best1, stopAtrMult, rrRatio, stagnationH, trailThreshold: 5, trailRetrace: 2 };
          let total = 0, maxDD = 0;
          for (const pair of pairs) { const { h4, pre4h, dailyCandles, preDaily, idxDailyAt, trainEnd } = candleMap[pair]; const r = runBacktest(h4, pre4h, dailyCandles, preDaily, idxDailyAt, params, 80, trainEnd); total += r.totalReturn; maxDD = Math.max(maxDD, r.maxDrawdown); }
          allPhase2.push({ params, trainReturn: total / pairs.length, trainMaxDD: maxDD });
        }

  const top50 = allPhase2
    .filter(r => r.trainReturn > 0 && r.trainMaxDD > 0)
    .sort((a, b) => (b.trainReturn / b.trainMaxDD) - (a.trainReturn / a.trainMaxDD))
    .slice(0, 50);
  if (top50.length === 0) { console.error("Phase 2: no positive results"); process.exit(1); }
  console.log(`  Phase 2 done. Top-50 #1: train ${top50[0].trainReturn.toFixed(2)}% MaxDD ${top50[0].trainMaxDD.toFixed(1)}%`);

  console.log(`\nPhase 3: ${top50.length} P2 winners × ${p3Count} trail combos (thresh=${TRAIL_THRESH_VALS.join('/')} retrace=${TRAIL_RETRACE_VALS.join('/')}), top-20 tested...`);
  const allPhase3: TrainResult[] = [];
  for (const { params: best2 } of top50)
    for (const trailThreshold of TRAIL_THRESH_VALS)
      for (const trailRetrace of TRAIL_RETRACE_VALS) {
        const params: Params = { ...best2, trailThreshold, trailRetrace };
        let total = 0, maxDD = 0;
        for (const pair of pairs) { const { h4, pre4h, dailyCandles, preDaily, idxDailyAt, trainEnd } = candleMap[pair]; const r = runBacktest(h4, pre4h, dailyCandles, preDaily, idxDailyAt, params, 80, trainEnd); total += r.totalReturn; maxDD = Math.max(maxDD, r.maxDrawdown); }
        allPhase3.push({ params, trainReturn: total / pairs.length, trainMaxDD: maxDD });
      }

  const top20 = allPhase3
    .filter(r => r.trainReturn > 0 && r.trainMaxDD > 0)
    .sort((a, b) => (b.trainReturn / b.trainMaxDD) - (a.trainReturn / a.trainMaxDD))
    .slice(0, 20);
  if (top20.length === 0) { console.error("Phase 3: no positive results"); process.exit(1); }
  console.log(`\nEvaluating top-20 (by train Calmar) on TEST set...`);
  const results: WalkForwardResult[] = [];
  for (const { params, trainReturn } of top20) {
  ...
  results.forEach((r, idx) => {
    ...
    console.log(`${String(idx + 1).padStart(2)}  ${r.profitPerDay >= 0 ? "+" : ""}${r.profitPerDay.toFixed(3)}  ${r.testReturn >= 0 ? "+" : ""}${r.testReturn.toFixed(2)}%  ${String(r.testTrades).padStart(6)}  ${r.testWinRate.toFixed(0)}%  ${r.testMaxDrawdown.toFixed(1)}%  ${calmar.toFixed(2)}   ${r.testSharpe.toFixed(2)}   sma=${r.params.dailySmaPeriod} adx=${r.params.dailyAdxMin} TRIX(${r.params.trixPeriod},sig=${r.params.signalPeriod}) stop=${r.params.stopAtrMult} rr=${r.params.rrRatio} stag=${r.params.stagnationH * 4}h trail=${r.params.trailThreshold}/${r.params.trailRetrace}`);
  });
  const best = results[0];
  if (!best) { console.log("No viable results"); return; }
