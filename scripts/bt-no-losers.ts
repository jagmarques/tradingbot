...
const ALL=["OPUSDT","WIFUSDT","ARBUSDT","LDOUSDT","AVAXUSDT","TRUMPUSDT","DASHUSDT","DOTUSDT","ENAUSDT","DOGEUSDT","APTUSDT","SEIUSDT","LINKUSDT","ADAUSDT","WLDUSDT","XRPUSDT","SUIUSDT","TONUSDT","UNIUSDT"];
const NO_LOSERS=ALL.filter(p=>p!=="SEIUSDT"&&p!=="SUIUSDT"); // 17 pairs
...
function sim(pairs:string[],pdm:Map<string,D>,btc:D,s:number,e:number):T[]{
  const ts=new Set<number>();for(const p of pairs){const d=pdm.get(p);if(d)for(const c of d.cs)if(c.t>=s&&c.t<e)ts.add(c.t)}
  ...
  for(const t of st){const cl=new Set<string>();
    ...
    for(const p of pairs){const d=pdm.get(p);if(!d||pos.has(p)||cl.has(p))continue;const bi=d.tm.get(t)??-1;if(bi<60)continue;
...
console.log("Loading...");const pdm=new Map<string,D>();for(const p of[...ALL,"BTCUSDT"]){const d=pre(p);if(d)pdm.set(p,d)}const btc=pdm.get("BTCUSDT")!;

for(const[name,pairs]of[["19 pairs (all)",ALL],["17 pairs (no SEI/SUI)",NO_LOSERS]]as[string,string[]][]){
  ...
  const posDays=dr.filter(d=>d>0).length;
