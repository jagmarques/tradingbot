// DEMA Cross deep sweep: wider fast/slow range, more SMA/ADX combos.
// Tries DEMA(3..13 fast, 13..89 slow) to find if shorter or longer periods beat (5/21).
// Walk-forward: 240d train / ~125d test. 0.29% round-trip, 10x leverage.
// Run: npx tsx scripts/tune-dema-deep.ts
...
// Wider search: fast [3,5,8,13], slow [13,21,34,55,89], SMA [50,70,100,150], ADX [14,18,22]
...
async function main() {
  console.log(`=== tune-dema-deep.ts: DEMA Cross deep sweep, ${PAIRS.length} pairs, ${DAYS}d ===`);
  ...
  if (best.testSharpe > 1.0 && best.testTrades > 30) {
    const prev = 0.415;
    console.log(`VIABLE. vs prev best DEMA +${prev}%/day: ${best.profitPerDay > prev ? `BEATS IT (+${(best.profitPerDay - prev).toFixed(3)}%/day)` : "below"}`);
