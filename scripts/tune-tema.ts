...
interface Params { dailySmaPeriod: number; dailyAdxMin: number; temaFast: number; temaSlow: number; stopAtrMult: number; rrRatio: number; stagnationH: number; }
...
interface H4PreInd { atr: (number | null)[]; tema: Record<number, (number | null)[]>; }
...
function computeTEMA(closes: number[], period: number): (number | null)[] {
  ...
  // ema3 starts at index (period-1)*3 in the original closes array
...
function precompute4h(candles: Candle[], temaPeriods: number[]): H4PreInd {
  const n = candles.length;
  const closes = candles.map(c => c.close), highs = candles.map(c => c.high), lows = candles.map(c => c.low);
  ...
  const tema: Record<number, (number | null)[]> = {};
  for (const p of temaPeriods) tema[p] = computeTEMA(closes, p);
  return { atr: atrArr, tema };
}
...
function runBacktest(candles4h: Candle[], pre4h: H4PreInd, dailyCandles: Candle[], preDaily: DailyPreInd, idxDailyAt: number[], p: Params, startIdx = 100, endIdx?: number): BacktestResult {
  ...
  const fastArr = pre4h.tema[p.temaFast], slowArr = pre4h.tema[p.temaSlow];
...
const TEMA_FAST_VALS = [3, 5, 8, 13];
const TEMA_SLOW_VALS = [13, 21, 34, 55, 89];
const STOP_ATR_VALS = [1.5, 2.0, 2.5, 3.0];
const RR_VALS = [2.0, 2.5, 3.0, 3.5, 4.0];
const STAG_VALS = [6, 9, 12, 16];
const ALL_TEMA_PERIODS = [...new Set([...TEMA_FAST_VALS, ...TEMA_SLOW_VALS])];
...
for (const dailySmaPeriod of DAILY_SMA_VALS)
  for (const dailyAdxMin of DAILY_ADX_VALS)
    for (const temaFast of TEMA_FAST_VALS)
      for (const temaSlow of TEMA_SLOW_VALS)
        if (temaFast < temaSlow)
          PHASE1_GRID.push({ dailySmaPeriod, dailyAdxMin, temaFast, temaSlow, stopAtrMult: 2.5, rrRatio: 3.0, stagnationH: 12 });

async function main() {
  const p2Count = STOP_ATR_VALS.length * RR_VALS.length * STAG_VALS.length;
  console.log(`=== tune-tema.ts: TEMA Cross, ${PAIRS.length} pairs, ${DAYS}d ===`);
  ...
  for (const pair of PAIRS) {
    try {
      ...
      const pre4h = precompute4h(h4, ALL_TEMA_PERIODS);
  ...
  console.log(`  Best: sma=${best1.dailySmaPeriod} adx=${best1.dailyAdxMin} TEMA(${best1.temaFast}/${best1.temaSlow}) | train: ${phase1[0].trainReturn.toFixed(2)}%`);
  ...
  results.forEach((r, i) => {
    const p = r.params;
    console.log(`${String(i + 1).padStart(2)}  ${r.profitPerDay >= 0 ? "+" : ""}${r.profitPerDay.toFixed(3)}  ${r.testReturn >= 0 ? "+" : ""}${r.testReturn.toFixed(2)}%  ${String(r.testTrades).padStart(6)}  ${r.testWinRate.toFixed(0)}%  ${r.testMaxDrawdown.toFixed(1)}%  ${r.testSharpe >= 0 ? " " : ""}${r.testSharpe.toFixed(2)}   sma=${p.dailySmaPeriod} adx=${p.dailyAdxMin} TEMA(${p.temaFast}/${p.temaSlow}) stop=${p.stopAtrMult} rr=${p.rrRatio} stag=${p.stagnationH * 4}h`);
  });
