// Elder low-MaxDD sweep: top-3 per ADX (Calmar), ADX up to 35, MaxDD cap <25% on Phase 2, Phase 3 trail.
// Run: npx tsx scripts/tune-elder-lowdd.ts
...
interface Params { dailySmaPeriod: number; dailyAdxMin: number; emaPeriod: number; macdFast: number; macdSlow: number; macdSignal: number; stopAtrMult: number; rrRatio: number; stagnationH: number; trailThreshold: number; trailRetrace: number; }
...
const DAILY_ADX_VALS = [14, 18, 22, 25, 30, 35];  // extended — higher ADX = stricter trend filter = lower MaxDD
...
const MAXDD_CAP = 25;  // only promote Phase 2 combos with training MaxDD below this
  ...
  for (const dailyAdxMin of DAILY_ADX_VALS)
    for (const emaPeriod of EMA_PERIOD_VALS)
      for (const { fast, slow, signal } of MACD_COMBOS)
        PHASE1_GRID.push({ dailySmaPeriod, dailyAdxMin, emaPeriod, macdFast: fast, macdSlow: slow, macdSignal: signal, stopAtrMult: 2.5, rrRatio: 3.0, stagnationH: 12, trailThreshold: 5, trailRetrace: 2 });
...
async function main() {
  ...
  console.log(`=== tune-elder-lowdd.ts: Elder low-MaxDD (top-3/ADX Calmar, ADX 14-35, DD cap ${MAXDD_CAP}%, Phase 3 trail), ${PAIRS.length} pairs, ${DAYS}d ===`);
  ...
  for (const [adx, results] of byAdx) {
    ...
    const w = top3[0];
    console.log(`  ADX=${adx} #1: sma=${w.params.dailySmaPeriod} EMA(${w.params.emaPeriod}) MACD(${w.params.macdFast},${w.params.macdSlow},${w.params.macdSignal}) | train: ${w.trainReturn.toFixed(2)}% MaxDD ${w.trainMaxDD.toFixed(1)}%`);
  }
  ...
  results.forEach((r, idx) => {
    ...
    console.log(`${String(idx + 1).padStart(2)}  ${r.profitPerDay >= 0 ? "+" : ""}${r.profitPerDay.toFixed(3)}  ${r.testReturn >= 0 ? "+" : ""}${r.testReturn.toFixed(2)}%  ${String(r.testTrades).padStart(6)}  ${r.testWinRate.toFixed(0)}%  ${r.testMaxDrawdown.toFixed(1)}%  ${calmar.toFixed(2)}   ${r.testSharpe.toFixed(2)}   sma=${r.params.dailySmaPeriod} adx=${r.params.dailyAdxMin} EMA(${r.params.emaPeriod}) MACD(${r.params.macdFast},${r.params.macdSlow},${r.params.macdSignal}) stop=${r.params.stopAtrMult} rr=${r.params.rrRatio} stag=${r.params.stagnationH * 4}h trail=${r.params.trailThreshold}/${r.params.trailRetrace}`);
  });
  ...
  console.log(`Deployed: +0.438%/day MaxDD 33.3%`);
  console.log(best.testMaxDrawdown < 33.3 ? `IMPROVEMENT — MaxDD reduced by ${(33.3 - best.testMaxDrawdown).toFixed(1)}pp` : "No MaxDD improvement");
}
