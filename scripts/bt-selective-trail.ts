/**
 * Selective per-engine trailing stop backtest.
 * Architecture identical to bt-trail-reentry-independent.ts (audited).
 * Extends runSim with a trailEngines set controlling which engines get trail.
 *
 * Run: NODE_OPTIONS="--max-old-space-size=8192" npx tsx scripts/bt-selective-trail.ts
 */
...
function checkMomentumConfirm(pair: string, t: number): SignalResult | null {
  ...
  for (let j = prev - 20; j <= prev; j++) {
    if (j >= 0) closes2.push(cs[j].c);
  }
  if (closes2.length < 20) return null;
  const cMean = closes2.reduce((s, v) => s + v, 0) / closes2.length;
  const cStd = Math.sqrt(closes2.reduce((s, v) => s + (v - cMean) ** 2, 0) / closes2.length);
  const priceZ = cStd > 0 ? (closes2[closes2.length - 1] - cMean) / cStd : 0;
...
// --------------- simulation ---------------
// trailEngines: set of engine letters that receive trailing stop treatment.
// If empty, no engines trail (baseline).
// act/dist: trail activation % and trail distance % (leveraged P&L).
function runSim(
  act: number,
  dist: number,
  doReentry: boolean,
  trailEngines: Set<string>,
  startTs: number,
  endTs: number,
  ...
  for (let t = simStart; t < simEnd; t += MIN_1) {
    ...
    for (let pi = openPositions.length - 1; pi >= 0; pi--) {
      ...
      // Peak tracking (always, in case engine later added to trail set)
      ...
      // Trailing stop: only apply if this engine is in trailEngines
      if (act > 0 && trailEngines.has(pos.engine) && pos.peakPnlPct >= act) {
...
interface Metrics {
  ...
  oosPerDay: number;
  oosPf: number;
  engBreakdown: Record<string, { trades: number; perDay: number; pf: number; wr: number }>;
}
...
function engineBreakdown(trades: ClosedTrade[], days: number): Record<string, { trades: number; perDay: number; pf: number; wr: number }> {
  const result: Record<string, { trades: number; perDay: number; pf: number; wr: number }> = {};
  for (const eng of ["A", "B", "C", "D"]) {
    ...
    result[eng] = {
      trades: et.length,
      perDay: et.length > 0 ? et.reduce((s, t) => s + t.pnl, 0) / days : 0,
      pf: gl > 0 ? gp / gl : 99,
...
// --------------- configs ---------------
// trailEngines: which engines get trail
// act=0 means no trail for all (trailEngines ignored when act=0)
const ALL_ENGINES = new Set(["A", "B", "C", "D"]);
const SELECTIVE_CD = new Set(["C", "D"]);   // GARCH + Momentum only
const GARCH_ONLY = new Set(["C"]);
...
const configs: {
  ...
  { label: "BASELINE",        act: 0,  dist: 0, reentry: false, trailEngines: new Set() },
  { label: "ALL 40/3+RE",     act: 40, dist: 3, reentry: true,  trailEngines: ALL_ENGINES },
  { label: "SEL-CD 30/3+RE",  act: 30, dist: 3, reentry: true,  trailEngines: SELECTIVE_CD },
  { label: "SEL-CD 40/3+RE",  act: 40, dist: 3, reentry: true,  trailEngines: SELECTIVE_CD },
  { label: "SEL-CD 40/5+RE",  act: 40, dist: 5, reentry: true,  trailEngines: SELECTIVE_CD },
  { label: "SEL-CD 45/5+RE",  act: 45, dist: 5, reentry: true,  trailEngines: SELECTIVE_CD },
  { label: "GARCH-ONLY 40/3", act: 40, dist: 3, reentry: true,  trailEngines: GARCH_ONLY },
];

console.log("Running selective trail simulation...\n");
console.log("Configs:");
console.log("  1. BASELINE         - no trail on any engine");
console.log("  2. ALL 40/3+RE      - all engines trail 40/3 (reference)");
console.log("  3. SEL-CD 30/3+RE   - GARCH+Momentum trail 30/3, Donchian+Supertrend no trail");
console.log("  4. SEL-CD 40/3+RE   - GARCH+Momentum trail 40/3, Donchian+Supertrend no trail");
console.log("  5. SEL-CD 40/5+RE   - GARCH+Momentum trail 40/5, Donchian+Supertrend no trail");
console.log("  6. SEL-CD 45/5+RE   - GARCH+Momentum trail 45/5, Donchian+Supertrend no trail");
console.log("  7. GARCH-ONLY 40/3  - GARCH only trail 40/3, no trail on others\n");
...
for (const cfg of configs) {
  ...
  const full = runSim(cfg.act, cfg.dist, cfg.reentry, cfg.trailEngines, FULL_START, FULL_END);
  ...
  const eng = engineBreakdown(full.trades, fullDays);
  console.log(` ${full.trades.length} trades (${full.reentries} re-ent), $${fm.perDay.toFixed(2)}/day, DD $${fm.maxDD.toFixed(0)}, PF ${fm.pf.toFixed(2)}`);
  results.push({
    label: cfg.label,
...
const sep = "-".repeat(170);
console.log("\n" + SEP);
console.log("SELECTIVE PER-ENGINE TRAIL BACKTEST");
console.log("Engines: A=$2 Donchian SMA20/50 | B=$3 Supertrend 14/1.75 | C=$9 GARCH v2 | D=$3 Momentum");
console.log("BTC filter: 4h EMA(12)>EMA(21) | 23 pairs | Max 20 pool | Kelly sizing");
console.log("Full: 2023-01 to 2026-03-26 | OOS: 2025-09+");
console.log(SEP);
...
console.log("\n" + SEP);
console.log("PER-ENGINE $/DAY BREAKDOWN (full period)");
console.log(SEP);

const engHdr = [
  "Config".padEnd(18),
  "A-Donch$/d".padStart(12),
  "A-PF".padStart(6),
  "B-ST$/d".padStart(10),
  "B-PF".padStart(6),
  "C-GARCH$/d".padStart(12),
  "C-PF".padStart(6),
  "D-Mom$/d".padStart(10),
  "D-PF".padStart(6),
  "C-WR%".padStart(7),
...
for (const r of results) {
  const A = r.engBreakdown["A"];
  const B = r.engBreakdown["B"];
  const C = r.engBreakdown["C"];
  const Dm = r.engBreakdown["D"];
  console.log([
    r.label.padEnd(18),
    ("$" + A.perDay.toFixed(2)).padStart(12),
    A.pf.toFixed(2).padStart(6),
    ("$" + B.perDay.toFixed(2)).padStart(10),
    B.pf.toFixed(2).padStart(6),
    ("$" + C.perDay.toFixed(2)).padStart(12),
    C.pf.toFixed(2).padStart(6),
    ("$" + Dm.perDay.toFixed(2)).padStart(10),
    Dm.pf.toFixed(2).padStart(6),
    C.wr.toFixed(1).padStart(6) + "%",
    Dm.wr.toFixed(1).padStart(6) + "%",
  ].join("  "));
}
...
console.log(`BASELINE:  $${baseline.perDay.toFixed(2)}/day  MaxDD $${baseline.maxDD.toFixed(0)}  Sharpe ${baseline.sharpe.toFixed(2)}  OOS $${baseline.oosPerDay.toFixed(2)}/day`);
const best = results.slice(1).sort((a, b) => b.perDay - a.perDay)[0];
if (best) {
  const diff = best.perDay - baseline.perDay;
  const pct = baseline.perDay !== 0 ? (diff / Math.abs(baseline.perDay)) * 100 : 0;
  console.log(`BEST:      ${best.label}: $${best.perDay.toFixed(2)}/day  MaxDD $${best.maxDD.toFixed(0)}  Sharpe ${best.sharpe.toFixed(2)}  OOS $${best.oosPerDay.toFixed(2)}/day`);
  console.log(`DIFF:      ${diff >= 0 ? "+" : ""}$${diff.toFixed(2)}/day (${pct >= 0 ? "+" : ""}${pct.toFixed(0)}% vs baseline)`);
}

// Selective vs ALL comparison
const allTrail = results.find(r => r.label === "ALL 40/3+RE");
const selBest = results.filter(r => r.label.startsWith("SEL")).sort((a, b) => b.perDay - a.perDay)[0];
if (allTrail && selBest) {
  const diff = selBest.perDay - allTrail.perDay;
  console.log(`\nSELECTIVE vs ALL-TRAIL: ${selBest.label} vs ALL 40/3+RE`);
  console.log(`  Selective: $${selBest.perDay.toFixed(2)}/day  MaxDD $${selBest.maxDD.toFixed(0)}  OOS $${selBest.oosPerDay.toFixed(2)}/day`);
  console.log(`  All-trail: $${allTrail.perDay.toFixed(2)}/day  MaxDD $${allTrail.maxDD.toFixed(0)}  OOS $${allTrail.oosPerDay.toFixed(2)}/day`);
  console.log(`  Delta: ${diff >= 0 ? "+" : ""}$${diff.toFixed(2)}/day`);
}
