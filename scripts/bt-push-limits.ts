/**
 * Push Limits Backtest - Test every optimization to squeeze max $/day
 *
 * Engines: A(Donchian), B(Supertrend), C(GARCH), D(Carry), E(Momentum/RangeExp)
 * Run: cd "/Users/jagmarques/Library/CloudStorage/OneDrive/Work/.TradingBot" && npx tsx scripts/bt-push-limits.ts
 */
...
// Build indicators for a given config (supports parameterization)
function buildIndicators(config: OptConfig) {
  const engAMap = new Map<string, EngAData>();
  const engBMap = new Map<string, EngBData>();
  const engB2Map = new Map<string, EngBData>();
  const engCMap = new Map<string, EngCData>();
  const engEMap = new Map<string, EngEData>();

  const donFast = config.donSmaFast ?? 20;
  const donSlow = config.donSmaSlow ?? 50;
  const stAtr = config.stAtrPeriod ?? 14;
  const stMult = config.stMult ?? 1.75;
  ...
  for (const p of available) {
    ...
    engAMap.set(p, {
      smaFast: sma(dc, donFast), smaSlow: sma(dc, donSlow),
      donLo15: donchianLow(dc, 15), donHi15: donchianHigh(dc, 15),
      atr14: atrFn(pd.daily, 14),
    });

    engBMap.set(p, {
      st: supertrend(pd.h4, stAtr, stMult).trend,
      atr14: atrFn(pd.h4, 14),
    });

    // Build second Supertrend if configured
    if (config.st2AtrPeriod !== undefined && config.st2Mult !== undefined) {
      engB2Map.set(p, {
        st: supertrend(pd.h4, config.st2AtrPeriod, config.st2Mult).trend,
        atr14: atrFn(pd.h4, 14),
      });
    }
  ...
  return { engAMap, engBMap, engB2Map, engCMap, engEMap };
}

// ─── Run Backtest ───────────────────────────────────────────────────
function runBacktest(config: OptConfig): Trade[] {
  const MAX_POSITIONS = config.maxPositions;
  const activeEngines = new Set(config.engines);
  const sizes = config.sizes;
  const donSlowPeriod = config.donSmaSlow ?? 50;

  const { engAMap, engBMap, engB2Map, engCMap, engEMap } = buildIndicators(config);

  const carryLookback = config.carryLookback ?? 5;
  ...
  for (const dayT of dailyTimestamps) {
    ...
    if (activeEngines.has("A")) {
      for (const p of available) {
        ...
        const di = pd.dailyMap.get(dayT);
        if (di === undefined || di < donSlowPeriod + 1) continue;

        const bar = pd.daily[di];
        const smaFastNow = ea.smaFast[di - 1], smaSlowNow = ea.smaSlow[di - 1];
        const smaFastPrev = ea.smaFast[di - 2], smaSlowPrev = ea.smaSlow[di - 2];
        if (smaFastNow === null || smaSlowNow === null || smaFastPrev === null || smaSlowPrev === null) continue;

        let dir: Dir | null = null;
        if (smaFastPrev <= smaSlowPrev && smaFastNow > smaSlowNow) {
    ...
    // ─── ENGINE B2: Second Supertrend (different params) ────────────
      ...
      for (let h4Offset = 0; h4Offset < DAY; h4Offset += H4) {
        ...
        for (const p of available) {
          ...
          positions.set(key, {
            ...
            margin: sizes["B2"] ?? 5, lev: 10, maxHold: 60 * DAY, atr: atrVal, bestPnlAtr: 0,
          });
        }
      }
    }
    ...
    if (activeEngines.has("D")) {
      if (dayT - lastCarryRebalance >= 7 * DAY) {
        ...
        for (const p of available) {
          const pd = pairData.get(p)!;
          const di = pd.dailyMap.get(dayT);
          if (di === undefined || di < carryLookback + 1) continue;

          let fundSum = 0;
          for (let j = di - carryLookback; j < di; j++) {
            if (j >= 0) {
              fundSum += (pd.daily[j].c - pd.daily[j].o) / pd.daily[j].c * 0.05;
            }
          }
          const avgFunding = fundSum / carryLookback;
          const momentum = pd.daily[di].c / pd.daily[di - carryLookback].c - 1;
...
const ENGINE_NAMES: Record<string, string> = {
  A: "Donchian", B: "Supertrend", B2: "ST-Fast", C: "GARCH", D: "Carry", E: "Momentum",
};
...
const CONFIGS: OptConfig[] = [
  ...
  // 2. Bigger sizes on winners
  {
    label: "2. BIGGER WINNERS",
    engines: ["A","B","C","D","E"],
    maxPositions: 20,
    sizes: { A: 10, B: 7, C: 3, D: 10, E: 5 },
  },
  // 3. Max 25 positions
  {
    label: "3. MAX 25 POS",
  ...
  // 6. Supertrend faster: ST(10, 1.5)
  {
    label: "6. ST FASTER",
    engines: ["A","B","C","D","E"],
    maxPositions: 20,
    sizes: { A: 7, B: 5, C: 3, D: 7, E: 3 },
    stAtrPeriod: 10, stMult: 1.5,
  },
  // 7. Double Supertrend: B=ST(14,1.75) + B2=ST(10,1.5)
  {
    label: "7. DOUBLE ST",
    engines: ["A","B","B2","C","D","E"],
    maxPositions: 20,
    sizes: { A: 7, B: 5, B2: 5, C: 3, D: 7, E: 3 },
    stAtrPeriod: 14, stMult: 1.75,
    st2AtrPeriod: 10, st2Mult: 1.5,
  },
  // 8. Donchian faster: SMA(10/30)
  {
    label: "8. DON FASTER",
    engines: ["A","B","C","D","E"],
    maxPositions: 20,
    sizes: { A: 7, B: 5, C: 3, D: 7, E: 3 },
    donSmaFast: 10, donSmaSlow: 30,
  },
  // 9. Carry 3-day lookback
  {
    label: "9. CARRY 3d",
    engines: ["A","B","C","D","E"],
    maxPositions: 20,
  ...
  // 10. Combined best (placeholder - will pick top 3 improvements after running 1-9)
  // We'll construct this after seeing results, but pre-define a likely combo:
  // Bigger sizes + no GARCH + max 25 + faster carry
  {
    label: "10. COMBINED BEST",
    engines: ["A","B","D","E"],        // no GARCH
    maxPositions: 25,                    // more positions
    sizes: { A: 10, B: 7, D: 10, E: 5 }, // bigger sizes
    carryLookback: 3,                    // faster carry
  },
];

// ─── Main ───────────────────────────────────────────────────────────
console.log("=".repeat(120));
console.log("  PUSH LIMITS BACKTEST - Squeeze Maximum Performance");
...
console.log("  NO trailing stops. Baseline: Don $7, ST $5, GARCH $3, Carry $7, Mom $3");
console.log("=".repeat(120));
...
for (const cfg of CONFIGS) {
  ...
  const engineDesc = cfg.engines.map(e => `${e}:${ENGINE_NAMES[e]}=$${cfg.sizes[e] ?? "?"}`).join(", ");
  console.log(`  Engines: ${engineDesc}`);
  console.log(`  MaxPos: ${cfg.maxPositions}` +
    (cfg.stAtrPeriod ? `, ST(${cfg.stAtrPeriod},${cfg.stMult})` : "") +
    (cfg.st2AtrPeriod ? `, ST2(${cfg.st2AtrPeriod},${cfg.st2Mult})` : "") +
    (cfg.donSmaFast ? `, Don SMA(${cfg.donSmaFast}/${cfg.donSmaSlow})` : "") +
    (cfg.carryLookback ? `, Carry LB=${cfg.carryLookback}d` : "")
  );
  console.log("#".repeat(120));
  ...
  for (const eng of cfg.engines) {
    ...
    console.log(
      `${(eng + ": " + ENGINE_NAMES[eng]).padEnd(18)} ${pad(String(et.length), 6)} ${pad(epf.toFixed(2), 6)} ` +
...
// ─── RANKED TABLE ───────────────────────────────────────────────────
console.log("\n\n" + "=".repeat(140));
console.log("  RANKED TABLE - All Optimizations (sorted by $/day)");
console.log("=".repeat(140));

const baseline = results[0].fullStats.perDay;

console.log(
  `${"#".padEnd(3)} ${"Config".padEnd(24)} ${pad("Trades", 6)} ${pad("PF", 6)} ${pad("Sharpe", 7)} ` +
  `${pad("$/day", 9)} ${pad("vs BL", 8)} ${pad("WR", 7)} ${pad("MaxDD", 7)} ` +
  `${pad("DDdur", 6)} ${pad("Total PnL", 11)} ` +
...
for (let i = 0; i < sorted.length; i++) {
  ...
  const delta = s.perDay - baseline;
  const deltaStr = (delta >= 0 ? "+" : "") + "$" + delta.toFixed(2);

  console.log(
    `${String(i + 1).padEnd(3)} ${r.label.padEnd(24)} ${pad(String(s.trades), 6)} ${pad(String(s.pf), 6)} ${pad(String(s.sharpe), 7)} ` +
    `${pad(fmtPnl(s.perDay), 9)} ${pad(deltaStr, 8)} ${pad(fmtPct(s.wr), 7)} ${pad("$" + s.maxDd.toFixed(0), 7)} ` +
    `${pad(s.maxDdDuration, 6)} ${pad(fmtPnl(s.totalPnl), 11)} ` +
    `${pad(fmtPnl(y23.perDay), 8)} ${pad(fmtPnl(y24.perDay), 8)} ${pad(fmtPnl(y25.perDay), 8)} ${pad(fmtPnl(y26.perDay), 8)}`
  );
}
...
console.log(
  `${"Optimization".padEnd(28)} ${pad("$/day", 9)} ${pad("Delta", 9)} ${pad("Pct", 8)} ${pad("PF", 6)} ${pad("Sharpe", 7)} ${pad("MaxDD", 7)}`
);
console.log("-".repeat(90));
...
for (const r of results) {
  ...
  console.log(
    `${r.label.padEnd(28)} ${pad(fmtPnl(s.perDay), 9)} ${pad((delta >= 0 ? "+" : "") + "$" + delta.toFixed(2), 9)} ` +
    `${pad((pctChange >= 0 ? "+" : "") + pctChange.toFixed(1) + "%", 8)} ` +
    `${pad(String(s.pf), 6)} ${pad(String(s.sharpe), 7)} ${pad("$" + s.maxDd.toFixed(0), 7)}`
  );
}
