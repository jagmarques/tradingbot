// HMA v3: Calmar-sorted Phase 1+2, top-30 tested, ADX up to 30.
// Run: npx tsx scripts/tune-hma-v3.ts
...
async function main() {
  ...
  console.log(`=== tune-hma-v3.ts: HMA v3 (ADX 14-30, Calmar-sorted P1+P2), ${PAIRS.length} pairs, ${DAYS}d ===`);
  ...
  for (const [adx, results] of byAdx) {
    ...
    console.log(`  ADX=${adx} best: sma=${w.params.dailySmaPeriod} HMA(${w.params.hmaFast}/${w.params.hmaSlow}) | train: ${w.trainReturn.toFixed(2)}% MaxDD ${w.trainMaxDD.toFixed(1)}%`);
  }
