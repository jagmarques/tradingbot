// Dual DEMA crossover: DEMA(fast) crosses DEMA(slow) with ADX + daily trend filter.
// DEMA(n) = 2*EMA(n) - EMA(EMA(n)). Less lag than standard EMA crossover.
// Walk-forward: 240d train / ~125d test. 0.29% round-trip, 10x leverage.
// Run: npx tsx scripts/tune-dema-cross.ts
...
interface H4PreInd { atr: (number | null)[]; dema: Record<number, (number | null)[]>; }
...
function computeDEMA(closes: number[], period: number): (number | null)[] {
  ...
  // ema1Raw aligns to [period-1 .. n-1], length = n - period + 1
  // ema2Raw aligns within ema1Raw at [period-1..], so back-aligns to [2*(period-1) .. n-1] in closes
  const result: (number | null)[] = new Array(n).fill(null);
  ema2Raw.forEach((e2, i) => {
    const e1Idx = i + (period - 1); // index in ema1Raw
    const closesIdx = e1Idx + (period - 1); // index in closes
...
function runBacktest(candles4h: Candle[], pre4h: H4PreInd, dailyCandles: Candle[], preDaily: DailyPreInd, idxDailyAt: number[], p: Params, startIdx = 80, endIdx?: number): BacktestResult {
  ...
  for (let i = startIdx; i < end; i++) {
    ...
    if (pos !== null) {
      ...
      // Long: fast DEMA crosses above slow DEMA in uptrend
      // Short: fast DEMA crosses below slow DEMA in downtrend
  ...
  for (const dailyAdxMin of DAILY_ADX_VALS)
    for (const demaFast of DEMA_FAST_VALS)
      for (const demaSlow of DEMA_SLOW_VALS)
        PHASE1_GRID.push({ dailySmaPeriod, dailyAdxMin, demaFast, demaSlow, stopAtrMult: 2.0, rrRatio: 2.5, stagnationH: 9 });

async function main() {
  console.log(`=== tune-dema-cross.ts: Dual DEMA Crossover, ${PAIRS.length} pairs, ${DAYS}d ===`);
  ...
  if (best.testSharpe > 1.0 && best.testTrades > 30) {
    console.log(`VIABLE. vs MTF +0.293%/day: ${best.profitPerDay > 0.293 ? "BEATS IT" : "below"}. vs EmaRibbon +0.180%/day: ${best.profitPerDay > 0.180 ? "BEATS IT" : "below"}.`);
  } else { console.log("NOT VIABLE (Sharpe <1.0 or <30 trades)."); }
}
