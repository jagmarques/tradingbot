/**
 * PARTIAL CLOSE + MULTI-STAGE TRAIL VARIATIONS
 *
 * Baseline:
 *   Engine A — GARCH long-only loose (z1h>2.0 AND z4h>1.5, LONGS ONLY)
 *   Engine B — Range Expansion mult=2.0
 *   Shared: SL 0.15%, trail 9/0.5, m$15 each, vol regime ON
 *   OOS baseline: $2.41/day, MDD $15, PF 2.00
 *
 * TEST variations (applied symmetrically to both engines):
 *   Partial close configs:
 *     1. Close 50% @ +5% lev pnl, runner trail 9/0.5
 *     2. Close 50% @ +3% lev pnl, runner trail 9/0.5
 *     3. Close 50% @ +5%, runner trail 12/0.5
 *     4. Close 33% @ +3%, 33% @ +7%, runner trail 15/1
 *     5. Close 25% @ +2%, 25% @ +5%, 50% runner trail 9/0.5
 *   Multi-stage trail (no partial):
 *     6. Trail 5/1 -> 10/0.5 -> 20/0.5
 *     7. Trail 7/2 -> 15/1 -> 30/0.5
 *     8. Trail 3/1 -> 9/0.5 -> 20/0.5
 *   Breakeven variants:
 *     9. BE @ +3% peak
 *    10. BE @ +5% peak
 *    11. BE @ +3% + trail 9/0.5
...
// Partial step: close `frac` of REMAINING notional at peak >= trig (leveraged %)
interface PartialStep { trig: number; frac: number; }
// Trail stage: activate when peak >= act, exit when peak - cur >= dist
interface TrailStage { act: number; dist: number; }

interface ExitCfg {
  label: string;
  partials: PartialStep[];        // applied in order, only once each
  trailStages: TrailStage[];      // highest-active stage applies
  beAt?: number;                  // move SL to entry (no fee) once peak >= beAt
}
...
interface OpenPos {
  ...
  origNot: number;
  maxHoldH: number;
  partialsTaken: number;  // count of partial steps already executed
  beMoved: boolean;
}
...
function simulate(pairs: PD[], cfg: Cfg, startTs: number, endTs: number, days: number): Res {
  ...
  for (const ts of timepoints) {
    ...
    for (let i = openPositions.length - 1; i >= 0; i--) {
      ...
      // Update peak first (use bar extremes)
      ...
      if (!xp) {
        while (pos.partialsTaken < cfg.exit.partials.length) {
          ...
          if (pos.pk < step.trig) break;
          // Close `step.frac` of CURRENT notional at trigger price
          // (price that would yield step.trig leveraged %)
          const trigPrice = pos.dir === "long"
            ? pos.ep * (1 + step.trig / 100 / pos.lev)
            : pos.ep * (1 - step.trig / 100 / pos.lev);
      ...
      if (xp > 0 && pos.not > 0.01) {
        ...
        // Fees: on final close, we only pay close-side fee on remaining notional,
        // plus the open-side fee was on full origNot (already accounted once).
        // Simpler: fee = 2x FEE on remaining, plus we already billed 2x FEE on each partial's slice.
        // That slightly double-bills entry fees on partials. To stay conservative we accept that.
...
function main() {
  log("=".repeat(130));
  log("  PARTIAL CLOSE + MULTI-STAGE TRAIL — GARCH long-only + REX portfolio");
  log("  Baseline: SL 0.15%, trail 9/0.5, m$15 each engine. OOS ref: $2.41/day, MDD $15, PF 2.00");
  ...
  const exitConfigs: ExitCfg[] = [
    // Baseline
    {
      label: "BASELINE: trail 9/0.5",
      partials: [],
      trailStages: [{ act: 9, dist: 0.5 }],
    },
    // Partial close configs
    {
      label: "1. Close 50% @ +5%, runner trail 9/0.5",
      partials: [{ trig: 5, frac: 0.5 }],
      trailStages: [{ act: 9, dist: 0.5 }],
    },
    {
      label: "2. Close 50% @ +3%, runner trail 9/0.5",
      partials: [{ trig: 3, frac: 0.5 }],
      trailStages: [{ act: 9, dist: 0.5 }],
    },
    {
      label: "3. Close 50% @ +5%, runner trail 12/0.5",
      partials: [{ trig: 5, frac: 0.5 }],
      trailStages: [{ act: 12, dist: 0.5 }],
    },
    {
      label: "4. Close 33% @ +3%, 33% @ +7%, runner trail 15/1",
      // Frac is of remaining; to remove 1/3 of original then 1/3 of original from 2/3 remaining → frac2 = 0.5
      partials: [{ trig: 3, frac: 0.3333 }, { trig: 7, frac: 0.5 }],
      trailStages: [{ act: 15, dist: 1 }],
    },
    {
      label: "5. Close 25% @ +2%, 25% @ +5%, runner trail 9/0.5",
      // 25% of original, then 25% of original from remaining 75% → frac2 = 0.3333
      partials: [{ trig: 2, frac: 0.25 }, { trig: 5, frac: 0.3333 }],
      trailStages: [{ act: 9, dist: 0.5 }],
    },
    // Multi-stage trail (no partial)
    {
      label: "6. Trail 5/1 -> 10/0.5 -> 20/0.5",
      partials: [],
      trailStages: [{ act: 5, dist: 1 }, { act: 10, dist: 0.5 }, { act: 20, dist: 0.5 }],
    },
    {
      label: "7. Trail 7/2 -> 15/1 -> 30/0.5",
      partials: [],
      trailStages: [{ act: 7, dist: 2 }, { act: 15, dist: 1 }, { act: 30, dist: 0.5 }],
    },
    {
      label: "8. Trail 3/1 -> 9/0.5 -> 20/0.5",
      partials: [],
      trailStages: [{ act: 3, dist: 1 }, { act: 9, dist: 0.5 }, { act: 20, dist: 0.5 }],
    },
    // Breakeven variants
    {
      label: "9. BE @ +3% peak (no trail)",
      partials: [],
      trailStages: [],
      beAt: 3,
    },
    {
      label: "10. BE @ +5% peak (no trail)",
      partials: [],
      trailStages: [],
      beAt: 5,
    },
    {
      label: "11. BE @ +3% + trail 9/0.5",
      partials: [],
      trailStages: [{ act: 9, dist: 0.5 }],
      beAt: 3,
    },
  ...
  for (const exit of exitConfigs) {
    ...
    log(`  IS  $/day ${fmtD(is.dollarsPerDay).padStart(8)}  MDD $${is.maxDD.toFixed(2).padStart(6)}  PF ${is.pf.toFixed(2)}  WR ${is.wr.toFixed(1)}%  N=${is.numTrades}  A:${is.byEngine.A.n} B:${is.byEngine.B.n}`);
    log(`  OOS $/day ${fmtD(oos.dollarsPerDay).padStart(8)}  MDD $${oos.maxDD.toFixed(2).padStart(6)}  PF ${oos.pf.toFixed(2)}  WR ${oos.wr.toFixed(1)}%  N=${oos.numTrades}  A:${oos.byEngine.A.n} B:${oos.byEngine.B.n}`);
    log(`  OOS A: ${fmtD(oos.byEngine.A.pnl)} (${oos.byEngine.A.n} trades)  B: ${fmtD(oos.byEngine.B.pnl)} (${oos.byEngine.B.n} trades)`);
    log(`  OOS maxSingleLoss: ${fmtD(oos.maxSingleLoss)}`);
  ...
  log("=".repeat(130));
  log(`${"Config".padEnd(50)} ${"$/day".padStart(9)} ${"MDD".padStart(8)} ${"PF".padStart(6)} ${"WR%".padStart(6)} ${"N".padStart(6)} ${"MaxL".padStart(9)}`);
  log("-".repeat(130));
  ...
  log("-".repeat(130));
  log(`${"Config".padEnd(50)} ${"$/day".padStart(9)} ${"MDD".padStart(8)} ${"PF".padStart(6)} ${"WR%".padStart(6)} ${"N".padStart(6)}`);
  log("-".repeat(130));
  ...
  // IS / OOS deltas — robustness check
  log("\n" + "=".repeat(130));
  log("  IS vs OOS CONSISTENCY (watch for overfitting)");
  log("=".repeat(130));
  log(`${"Config".padEnd(50)} ${"IS$/d".padStart(9)} ${"OOS$/d".padStart(9)} ${"ISMDD".padStart(8)} ${"OOSMDD".padStart(8)} ${"ISPF".padStart(6)} ${"OOSPF".padStart(6)}`);
  log("-".repeat(130));
  for (const r of recs) {
    log(
      `${r.label.padEnd(50)} ` +
  ...
  // VERDICT
  log("\n" + "=".repeat(130));
  log("  VERDICT — vs baseline OOS $2.41/day, MDD $15");
  log("=".repeat(130));

  const baseline = recs[0]!; // first is baseline
  log(`\n  Baseline (in-run): $/day ${fmtD(baseline.oos.dollarsPerDay)}  MDD $${baseline.oos.maxDD.toFixed(2)}  PF ${baseline.oos.pf.toFixed(2)}  N=${baseline.oos.numTrades}`);

  // Passing MDD < 15
  const passMDD = recs.filter(r => r.oos.maxDD < 15 && r.label !== baseline.label);
  passMDD.sort((a, b) => b.oos.dollarsPerDay - a.oos.dollarsPerDay);
  log(`\n  Configs with OOS MDD < $15 (${passMDD.length}):`);
  for (const r of passMDD) {
    const dpnl = r.oos.dollarsPerDay - baseline.oos.dollarsPerDay;
    const dmdd = r.oos.maxDD - baseline.oos.maxDD;
    log(`    ${r.label.padEnd(50)} $/day ${fmtD(r.oos.dollarsPerDay)} (Δ${fmtD(dpnl)})  MDD $${r.oos.maxDD.toFixed(0)} (Δ$${dmdd.toFixed(0)})  PF ${r.oos.pf.toFixed(2)}`);
  }

  // Beating baseline on $/day
  ...
  for (const r of beatPnl) {
    const dpnl = r.oos.dollarsPerDay - baseline.oos.dollarsPerDay;
    const dmdd = r.oos.maxDD - baseline.oos.maxDD;
    log(`    ${r.label.padEnd(50)} $/day ${fmtD(r.oos.dollarsPerDay)} (+${fmtD(dpnl)})  MDD $${r.oos.maxDD.toFixed(0)} (Δ$${dmdd.toFixed(0)})  PF ${r.oos.pf.toFixed(2)}`);
  }

  // Best combined
  log("\n  BEST BY RISK-ADJ ($/day / MDD ratio):");
  const ranked = [...recs]
    .filter(r => r.oos.maxDD > 0 && r.oos.dollarsPerDay > 0)
    .map(r => ({ r, score: r.oos.dollarsPerDay / r.oos.maxDD }))
    .sort((a, b) => b.score - a.score);
  for (const { r, score } of ranked.slice(0, 5)) {
    log(`    ${r.label.padEnd(50)} score ${score.toFixed(4)}  $/day ${fmtD(r.oos.dollarsPerDay)}  MDD $${r.oos.maxDD.toFixed(0)}  PF ${r.oos.pf.toFixed(2)}`);
  }

  const outDir = "/Users/jagmarques/Library/CloudStorage/OneDrive/Work/.TradingBot/.company/backtester";
  if (!fs.existsSync(outDir)) fs.mkdirSync(outDir, { recursive: true });
  fs.writeFileSync(path.join(outDir, "partial-trail.txt"), lines.join("\n") + "\n");
  log(`\nSaved to ${path.join(outDir, "partial-trail.txt")}`);
}
