// Vortex v3: Calmar-sorted Phase 1+2, top-30 tested, ADX up to 30.
// Run: npx tsx scripts/tune-vortex-v3.ts
...
function computeVortex(candles: Candle[], period: number): { vip: (number | null)[]; vim: (number | null)[] } {
  ...
  const vip: (number | null)[] = new Array(n).fill(null), vim: (number | null)[] = new Array(n).fill(null);
  for (let i = period; i < n; i++) {
    ...
    for (let k = i - period + 1; k <= i; k++) {
      ...
      sumTr += Math.max(candles[k].high - candles[k].low, Math.abs(candles[k].high - candles[k - 1].close), Math.abs(candles[k].low - candles[k - 1].close));
    }
...
function runBacktest(candles4h: Candle[], pre4h: H4PreInd, dailyCandles: Candle[], preDaily: DailyPreInd, idxDailyAt: number[], p: Params, startIdx = 80, endIdx?: number): BacktestResult {
  ...
  for (let i = startIdx; i < end; i++) {
    ...
    if (pos !== null) {
      ...
      const vx = pre4h.vortex[p.vortexPeriod]; if (!vx) continue;
...
async function main() {
  ...
  console.log(`=== tune-vortex-v3.ts: Vortex v3 (ADX 14-30, Calmar-sorted P1+P2), ${PAIRS.length} pairs, ${DAYS}d ===`);
  ...
  for (const [adx, results] of byAdx) {
    ...
    console.log(`  ADX=${adx} best: sma=${w.params.dailySmaPeriod} Vortex(${w.params.vortexPeriod}) | train: ${w.trainReturn.toFixed(2)}% MaxDD ${w.trainMaxDD.toFixed(1)}%`);
  }
  ...
  results.forEach((r, idx) => {
    ...
    console.log(`${String(idx + 1).padStart(2)}  ${r.profitPerDay >= 0 ? "+" : ""}${r.profitPerDay.toFixed(3)}  ${r.testReturn >= 0 ? "+" : ""}${r.testReturn.toFixed(2)}%  ${String(r.testTrades).padStart(6)}  ${r.testWinRate.toFixed(0)}%  ${r.testMaxDrawdown.toFixed(1)}%  ${calmar.toFixed(2)}   ${r.testSharpe.toFixed(2)}   sma=${r.params.dailySmaPeriod} adx=${r.params.dailyAdxMin} Vortex(${r.params.vortexPeriod}) stop=${r.params.stopAtrMult} rr=${r.params.rrRatio} stag=${r.params.stagnationH * 4}h`);
  });
