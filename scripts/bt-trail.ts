...
interface Cfg{name:string;tp:number;trailA:number;trailD:number;mh:number}
...
function sim(c:Cfg,pdm:Map<string,D>,btc:D,s:number,e:number):T[]{
  ...
  for(const t of st){const cl=new Set<string>();
    for(const[p,ps]of pos){const d=pdm.get(p)!;const bi=d.tm.get(t)??-1;if(bi<0)continue;const b=d.cs[bi];const sp=SP[p]??4e-4;let xp=0,reason="";
      ...
      // Trailing stop
      if(!reason&&c.trailA>0){
        const pnlPct=ps.dir==="long"?(b.c-ps.ep)/ps.ep*LEV*100:(ps.ep-b.c)/ps.ep*LEV*100;
        if(pnlPct>ps.pk)ps.pk=pnlPct;
        if(ps.pk>=c.trailA&&pnlPct<=ps.pk-c.trailD){xp=ps.dir==="long"?b.c*(1-sp):b.c*(1+sp);reason="trail"}}
      // Max hold
      if(!reason&&t-ps.et>=c.mh){xp=ps.dir==="long"?b.c*(1-sp):b.c*(1+sp);reason="mh"}
    ...
    for(const[p,d]of pdm){if(p==="BTCUSDT"||pos.has(p)||cl.has(p))continue;const bi=d.tm.get(t)??-1;if(bi<60)continue;
      ...
      pos.set(p,{pair:p,dir,ep:ent,et:t,sl:dir==="long"?ent*0.97:ent*1.03,tp:c.tp>0?(dir==="long"?ent*(1+c.tp):ent*(1-c.tp)):0,pk:0})}}
  return tr}
...
const cfgs:Cfg[]=[
  // Baseline: no trail, TP 10%
  {name:"NO-trail-TP10",tp:0.10,trailA:0,trailD:0,mh:48*H},
  // No trail, TP 12%
  {name:"NO-trail-TP12",tp:0.12,trailA:0,trailD:0,mh:48*H},
  // No trail, TP 15%
  {name:"NO-trail-TP15",tp:0.15,trailA:0,trailD:0,mh:48*H},
  // Trail only, no TP (let trail capture gains)
  {name:"Trail-15/5-noTP",tp:0,trailA:15,trailD:5,mh:48*H},
  {name:"Trail-20/7-noTP",tp:0,trailA:20,trailD:7,mh:48*H},
  {name:"Trail-25/8-noTP",tp:0,trailA:25,trailD:8,mh:48*H},
  {name:"Trail-30/10-noTP",tp:0,trailA:30,trailD:10,mh:48*H},
  {name:"Trail-40/15-noTP",tp:0,trailA:40,trailD:15,mh:72*H},
  // TP + Trail combo (TP as ceiling, trail as floor)
  {name:"TP10+Trail15/5",tp:0.10,trailA:15,trailD:5,mh:48*H},
  {name:"TP12+Trail20/7",tp:0.12,trailA:20,trailD:7,mh:48*H},
  {name:"TP15+Trail20/7",tp:0.15,trailA:20,trailD:7,mh:48*H},
  {name:"TP15+Trail25/8",tp:0.15,trailA:25,trailD:8,mh:48*H},
  {name:"TP20+Trail30/10",tp:0.20,trailA:30,trailD:10,mh:72*H},
  // Wide trail for big runners
  {name:"Trail-50/15-noTP",tp:0,trailA:50,trailD:15,mh:72*H},
  {name:"TP25+Trail40/12",tp:0.25,trailA:40,trailD:12,mh:72*H},
];

console.log(`\nz=4.5/3.0, ADX 30/25, SL 3%, $20/trade, max 4/dir\n`);
console.log("Strategy              Trades  T/day  WR%    PnL       $/day  Sharpe  MaxDD   Train   Test");
console.log("-".repeat(105));

for(const c of cfgs){
  const tr=sim(c,pdm,btc,S,E);const pnl=tr.reduce((s,t)=>s+t.pnl,0);const w=tr.filter(t=>t.pnl>0).length;
  ...
  const trn=sim(c,pdm,btc,S,M);const tst=sim(c,pdm,btc,M,E);
  ...
  console.log(`${c.name.padEnd(21)} ${String(tr.length).padStart(6)}  ${(tr.length/days).toFixed(1).padStart(5)}  ${(tr.length>0?w/tr.length*100:0).toFixed(1).padStart(5)}  ${p.padStart(8)}  ${("$"+(pnl/days).toFixed(2)).padStart(6)}  ${sh.toFixed(2).padStart(6)}  ${("$"+dd.toFixed(0)).padStart(6)}  $${trnD.toFixed(2).padStart(5)}  $${tstD.toFixed(2).padStart(5)}`);
}
