// TRIX v3: Calmar-sorted Phase 1+2, top-30 tested, ADX up to 30.
// Run: npx tsx scripts/tune-trix-v3.ts
...
async function main() {
  ...
  console.log(`=== tune-trix-v3.ts: TRIX v3 (ADX 14-30, Calmar-sorted P1+P2), ${PAIRS.length} pairs, ${DAYS}d ===`);
  ...
  for (const [adx, results] of byAdx) {
    ...
    console.log(`  ADX=${adx} best: sma=${w.params.dailySmaPeriod} TRIX(${w.params.trixPeriod},sig=${w.params.signalPeriod}) | train: ${w.trainReturn.toFixed(2)}% MaxDD ${w.trainMaxDD.toFixed(1)}%`);
  }
  ...
  results.forEach((r, idx) => {
    ...
    console.log(`${String(idx + 1).padStart(2)}  ${r.profitPerDay >= 0 ? "+" : ""}${r.profitPerDay.toFixed(3)}  ${r.testReturn >= 0 ? "+" : ""}${r.testReturn.toFixed(2)}%  ${String(r.testTrades).padStart(6)}  ${r.testWinRate.toFixed(0)}%  ${r.testMaxDrawdown.toFixed(1)}%  ${calmar.toFixed(2)}   ${r.testSharpe.toFixed(2)}   sma=${r.params.dailySmaPeriod} adx=${r.params.dailyAdxMin} TRIX(${r.params.trixPeriod},sig=${r.params.signalPeriod}) stop=${r.params.stopAtrMult} rr=${r.params.rrRatio} stag=${r.params.stagnationH * 4}h`);
  });
