/**
 * Engine Combo Backtest - Test whether removing weak engines (GARCH, Range Expansion) improves results
 *
 * Engines: A(Daily Donchian), B(4h Supertrend), C(GARCH v2 MTF), D(Carry Momentum), E(Range Expansion)
 * Run: cd "/Users/jagmarques/Library/CloudStorage/OneDrive/Work/.TradingBot" && npx tsx scripts/bt-engine-combos.ts
 */
...
interface ComboConfig {
  label: string;
  engines: string[];         // which engines are active: "A","B","C","D","E"
  maxPositions: number;
  sizes: Record<string, number>; // engine -> margin size
}
...
const COMBOS: ComboConfig[] = [
  {
    label: "1. 5-eng baseline",
    engines: ["A","B","C","D","E"],
    maxPositions: 10,
    sizes: { A: 5, B: 3, C: 5, D: 5, E: 3 },
  },
  {
    label: "2. 3-eng (A+B+D)",
    engines: ["A","B","D"],
  ...
  {
    label: "5. 4-eng keepGARCH",
    engines: ["A","B","C","D"],
    maxPositions: 10,
    sizes: { A: 5, B: 3, C: 5, D: 5 },
  },
  {
    label: "6. 4-eng keepRange",
    engines: ["A","B","D","E"],
    maxPositions: 10,
    sizes: { A: 5, B: 3, D: 5, E: 3 },
  },
  {
    label: "7. 2-eng (B+D)",
    engines: ["B","D"],
...
console.log("  ENGINE COMBO BACKTEST - Drop Weak Engines (GARCH PF 1.02, Range Expansion PF 1.01)");
...
const best = sorted[0];
const bestCons = sorted2x[0];
const baseline = results[0]; // combo #1
...
Baseline (5-engine):           ${baseline.label}  ->  ${fmtPnl(baseline.fullStats.perDay)}/day, PF ${baseline.fullStats.pf}

Delta vs baseline (standard):  ${fmtPnl(best.fullStats.perDay - baseline.fullStats.perDay)}/day (${((best.fullStats.perDay / baseline.fullStats.perDay - 1) * 100).toFixed(1)}%)
Delta vs baseline (2x spread): ${fmtPnl(bestCons.conservative.perDay - baseline.conservative.perDay)}/day

Year consistency (best standard):
...
// Check if best is robust (positive in both standard and 2x)
if (best.fullStats.perDay > 0 && best.conservative.perDay > 0) {
  console.log(`VERDICT: ${best.label} is robust (positive under both cost models).`);
} else {
  console.log(`WARNING: Best config is not robust under doubled costs.`);
}

// Check if dropping engines helps
const threeEng = results[1]; // combo #2
if (threeEng.fullStats.perDay > baseline.fullStats.perDay) {
  console.log(`Dropping GARCH + Range Expansion IMPROVES results: +${fmtPnl(threeEng.fullStats.perDay - baseline.fullStats.perDay)}/day`);
} else {
  console.log(`Dropping GARCH + Range Expansion HURTS results: ${fmtPnl(threeEng.fullStats.perDay - baseline.fullStats.perDay)}/day`);
}
