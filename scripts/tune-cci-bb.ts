// CCI Trend + BB Squeeze pre-filter: only enter when BB width is in bottom 30th percentile (squeeze).
// Compare results vs tune-cci-deep.ts baseline (+0.416%/day, Sharpe 6.08).
// Walk-forward: 240d train / ~125d test. 0.29% round-trip, 10x leverage.
// Run: npx tsx scripts/tune-cci-bb.ts
...
interface Params { dailySmaPeriod: number; dailyAdxMin: number; cciPeriod: number; cciThreshold: number; stopAtrMult: number; rrRatio: number; stagnationH: number; }
...
interface H4PreInd { atr: (number | null)[]; cci: Record<number, (number | null)[]>; bbWidths: (number | null)[]; }
...
function precompute4h(candles: Candle[], cciPeriods: number[]): H4PreInd {
  ...
  return { atr: atrArr, cci, bbWidths: computeBBWidth(candles) };
}
...
const DAILY_ADX_VALS = [15, 18, 22];
const CCI_PERIOD_VALS = [5, 10, 14, 20, 30];
const CCI_THRESHOLD_VALS = [70, 80, 100, 120, 150];
  ...
  for (const dailyAdxMin of DAILY_ADX_VALS)
    for (const cciPeriod of CCI_PERIOD_VALS)
      for (const cciThreshold of CCI_THRESHOLD_VALS)
        PHASE1_GRID.push({ dailySmaPeriod, dailyAdxMin, cciPeriod, cciThreshold, stopAtrMult: 2.5, rrRatio: 3.5, stagnationH: 12 });

async function main() {
  const p2Count = STOP_ATR_VALS.length * RR_VALS.length * STAG_VALS.length;
  console.log(`=== tune-cci-bb.ts: CCI Trend + BB Squeeze filter, ${PAIRS.length} pairs, ${DAYS}d ===`);
  console.log(`Baseline (no filter): +0.416%/day, Sharpe 6.08`);
  ...
  console.log(`  Best Phase 1: sma=${best1.dailySmaPeriod} adx=${best1.dailyAdxMin} CCI(${best1.cciPeriod}) thresh=${best1.cciThreshold} | train: ${phase1[0].trainReturn.toFixed(2)}%`);
  ...
  results.forEach((r, i) => {
    const p = r.params;
    console.log(`${String(i + 1).padStart(2)}  ${r.profitPerDay >= 0 ? "+" : ""}${r.profitPerDay.toFixed(3)}  ${r.testReturn >= 0 ? "+" : ""}${r.testReturn.toFixed(2)}%    ${r.trainReturn >= 0 ? "+" : ""}${r.trainReturn.toFixed(2)}%   ${String(r.testTrades).padStart(6)}  ${r.testWinRate.toFixed(0)}%  ${r.testMaxDrawdown.toFixed(1)}%  ${r.testSharpe >= 0 ? " " : ""}${r.testSharpe.toFixed(2)}   sma=${p.dailySmaPeriod} adx=${p.dailyAdxMin} CCI(${p.cciPeriod}) thresh=${p.cciThreshold} stop=${p.stopAtrMult} rr=${p.rrRatio} stag=${p.stagnationH * 4}h`);
  });
