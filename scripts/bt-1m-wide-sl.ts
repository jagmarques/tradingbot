/**
 * Wider SL sweep with 1m realistic checking
 * Tests SL: 0.80-3.0%, trail variants, BE + trail, WR focus
 */
...
interface Config {
  ...
  trailA: number; trailD: number;
  bePct: number;
  maxHoldH: number; cdH: number;
}

function simulate(cfg: Config): { pnlDay: number; mdd: number; pf: number; wr: number; trades: number; calmar: number; slCount: number; trailCount: number } {
  ...
  for (const ts of timepoints) {
    ...
    for (let i = openPositions.length - 1; i >= 0; i--) {
      ...
      if (exitPrice > 0) {
        ...
        if (reason === "sl") { slCount++; cooldowns.set(`${pos.pair}:long`, ts + cfg.cdH * H); }
  ...
  return { pnlDay, mdd: mtmMaxDD, pf, wr, trades: totalTrades, calmar, slCount, trailCount };
}

const configs: { label: string; cfg: Config }[] = [];

// Wide SL sweep - focus on high WR configurations
for (const mc of [5, 7, 10]) {
  for (const sl of [
    { low: 0.008, high: 0.012, l: "0.8/1.2" },
    { low: 0.01, high: 0.015, l: "1.0/1.5" },
    { low: 0.012, high: 0.018, l: "1.2/1.8" },
    { low: 0.015, high: 0.02, l: "1.5/2.0" },
    { low: 0.02, high: 0.025, l: "2.0/2.5" },
    { low: 0.025, high: 0.03, l: "2.5/3.0" },
  ]) {
    for (const trail of [
      { a: 5, d: 3, l: "5/3" }, { a: 7, d: 3, l: "7/3" }, { a: 10, d: 5, l: "10/5" },
      { a: 10, d: 3, l: "10/3" }, { a: 15, d: 5, l: "15/5" }, { a: 15, d: 8, l: "15/8" },
      { a: 20, d: 8, l: "20/8" }, { a: 20, d: 10, l: "20/10" }, { a: 30, d: 10, l: "30/10" },
    ]) {
      for (const be of [0, 3, 5, 8]) {
        for (const cd of [2, 4]) {
          configs.push({
            label: `mc${mc} SL${sl.l} T${trail.l} BE${be}% cd${cd}h`,
            cfg: { mc, slLow: sl.low, slHigh: sl.high, trailA: trail.a, trailD: trail.d, bePct: be, maxHoldH: 120, cdH: cd },
          });
        }
      }
    }
...
console.log(`\nSweeping ${configs.length} wide-SL configs (1m)...\n`);

interface Result { label: string; pnlDay: number; mdd: number; pf: number; wr: number; trades: number; calmar: number; slCount: number; trailCount: number }
...
console.log(`${"Config".padEnd(55)} $/day   MDD    PF   WR%  Trades  SL% Trail% Calmar`);
for (const r of results.slice(0, 30)) {
  const slPct = (r.slCount / r.trades * 100).toFixed(0);
  const tpPct = (r.trailCount / r.trades * 100).toFixed(0);
  console.log(`${r.label.padEnd(55)} ${r.pnlDay.toFixed(2).padStart(5)}  ${r.mdd.toFixed(1).padStart(5)}  ${r.pf.toFixed(2).padStart(4)}  ${r.wr.toFixed(1).padStart(4)}  ${String(r.trades).padStart(5)}  ${slPct.padStart(3)}%  ${tpPct.padStart(4)}%  ${r.calmar.toFixed(4)}`);
}
...
console.log(`\n${"=".repeat(100)}`);
console.log("TOP 20 BY $/DAY (MDD < $50)");
console.log("=".repeat(100));
const safe = results.filter(r => r.mdd < 50);
