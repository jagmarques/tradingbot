// CCI ±threshold breakout with daily trend filter.
// CCI = (TP - SMA(TP)) / (0.015 * MeanDev(TP)). Entry when CCI crosses ±threshold in trend direction.
// Walk-forward: 240d train / ~125d test. 0.29% round-trip, 10x leverage.
// Run: npx tsx scripts/tune-cci-trend.ts
...
function runBacktest(candles4h: Candle[], pre4h: H4PreInd, dailyCandles: Candle[], preDaily: DailyPreInd, idxDailyAt: number[], p: Params, startIdx = 80, endIdx?: number): BacktestResult {
  ...
  for (let i = startIdx; i < end; i++) {
    ...
    if (pos !== null) {
      ...
      // Long: CCI crosses above threshold in uptrend
      // Short: CCI crosses below -threshold in downtrend
...
const CCI_PERIOD_VALS = [10, 14, 20];
const CCI_THRESHOLD_VALS = [80, 100, 120];
  ...
  for (const dailyAdxMin of DAILY_ADX_VALS)
    for (const cciPeriod of CCI_PERIOD_VALS)
      for (const cciThreshold of CCI_THRESHOLD_VALS)
        PHASE1_GRID.push({ dailySmaPeriod, dailyAdxMin, cciPeriod, cciThreshold, stopAtrMult: 2.0, rrRatio: 2.5, stagnationH: 9 });

async function main() {
  console.log(`=== tune-cci-trend.ts: CCI ±threshold breakout, ${PAIRS.length} pairs, ${DAYS}d ===`);
