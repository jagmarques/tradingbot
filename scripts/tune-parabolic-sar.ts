// Parabolic SAR: classic SAR crossover with daily trend filter.
// Walk-forward: 240d train / ~125d test. 0.29% round-trip, 10x leverage.
// Run: npx tsx scripts/tune-parabolic-sar.ts

import { ATR, ADX, PSAR } from "technicalindicators";

interface Candle { timestamp: number; open: number; high: number; low: number; close: number; volume: number; }
interface Params { dailySmaPeriod: number; dailyAdxMin: number; psarStep: number; psarMax: number; stopAtrMult: number; rrRatio: number; stagnationH: number; }
...
interface H4PreInd { atr: (number | null)[]; psar: Record<string, (number | null)[]>; }
...
function precompute4h(candles: Candle[], steps: number[], maxes: number[]): H4PreInd {
  const n = candles.length;
  ...
  const psar: Record<string, (number | null)[]> = {};
  for (const step of steps) {
    for (const max of maxes) {
      const key = `${step}_${max}`;
      const raw = PSAR.calculate({ high: highs, low: lows, step, max });
      const arr: (number | null)[] = new Array(n).fill(null);
      // align to end of input array
      raw.forEach((v, i) => { arr[n - raw.length + i] = v; });
      psar[key] = arr;
    }
  }
  return { atr: atrArr, psar };
}
...
function runBacktest(candles4h: Candle[], pre4h: H4PreInd, dailyCandles: Candle[], preDaily: DailyPreInd, idxDailyAt: number[], p: Params, startIdx = 80, endIdx?: number): BacktestResult {
  ...
  for (let i = startIdx; i < end; i++) {
    ...
    if (pos !== null) {
      ...
      const key = `${p.psarStep}_${p.psarMax}`;
      const psarArr = pre4h.psar[key];
      if (!psarArr) continue;
      const currSar = psarArr[i], prevSar = psarArr[i - 1];
      if (currSar === null || prevSar === null) continue;
      const currClose = c.close, prevClose = candles4h[i - 1].close;
      const dailyUptrend = dailyClose > dailySma, dailyDowntrend = dailyClose < dailySma;
      let dir: "long" | "short" | null = null;
      // Long: SAR flipped from above to below (was above prev close, now below curr close)
      if (dailyUptrend && prevSar > prevClose && currSar < currClose) dir = "long";
      // Short: SAR flipped from below to above
      if (dailyDowntrend && prevSar < prevClose && currSar > currClose) dir = "short";
      if (dir !== null && i + 1 < end) {
...
const PSAR_STEP_VALS = [0.005, 0.01, 0.015, 0.02, 0.03, 0.04, 0.06];
const PSAR_MAX_VALS = [0.08, 0.10, 0.15, 0.20, 0.25, 0.30, 0.40];
...
for (const dailySmaPeriod of DAILY_SMA_VALS)
  for (const dailyAdxMin of DAILY_ADX_VALS)
    for (const psarStep of PSAR_STEP_VALS)
      for (const psarMax of PSAR_MAX_VALS)
        PHASE1_GRID.push({ dailySmaPeriod, dailyAdxMin, psarStep, psarMax, stopAtrMult: 2.5, rrRatio: 3.0, stagnationH: 12 });

async function main() {
  const p2Count = STOP_ATR_VALS.length * RR_VALS.length * STAG_VALS.length;
  console.log(`=== tune-parabolic-sar.ts: ${PAIRS.length} pairs, ${DAYS}d ===`);
  ...
  for (const pair of PAIRS) {
    try {
      ...
      const pre4h = precompute4h(h4, [...new Set(PSAR_STEP_VALS)], [...new Set(PSAR_MAX_VALS)]);
  ...
  console.log(`  Best: sma=${best1.dailySmaPeriod} adx=${best1.dailyAdxMin} psar(step=${best1.psarStep},max=${best1.psarMax}) | train: ${phase1[0].trainReturn.toFixed(2)}%`);
  ...
  results.forEach((r, i) => {
    const p = r.params;
    console.log(`${String(i + 1).padStart(2)}  ${r.profitPerDay >= 0 ? "+" : ""}${r.profitPerDay.toFixed(3)}  ${r.testReturn >= 0 ? "+" : ""}${r.testReturn.toFixed(2)}%  ${String(r.testTrades).padStart(6)}  ${r.testWinRate.toFixed(0)}%  ${r.testMaxDrawdown.toFixed(1)}%  ${r.testSharpe >= 0 ? " " : ""}${r.testSharpe.toFixed(2)}   sma=${p.dailySmaPeriod} adx=${p.dailyAdxMin} psar(step=${p.psarStep},max=${p.psarMax}) stop=${p.stopAtrMult} rr=${p.rrRatio} stag=${p.stagnationH * 4}h`);
  });
