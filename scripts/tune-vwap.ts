...
function computeRollingVwap(candles: Candle[]): (number | null)[] {
  ...
  for (let i = VWAP_WINDOW - 1; i < n; i++) {
    let cumTpv = 0;
    let cumVol = 0;
    for (let j = i - VWAP_WINDOW + 1; j <= i; j++) {
...
// Phase 1: 3^4 = 81 combos (fixed rrRatio=2.0, stagnationH=12)
const DEVIATION_LONG = [-3.0, -4.0, -5.0];
const DEVIATION_SHORT = [2.0, 3.0, 4.0];
const TREND_CONFLICT = [2.0, 3.0, 5.0];
const STOP_ATR_MULTS = [1.25, 1.5, 2.0];

const PHASE1_GRID: VwapParams[] = [];
for (const deviationLongPct of DEVIATION_LONG) {
  for (const deviationShortPct of DEVIATION_SHORT) {
    for (const trendConflictPct of TREND_CONFLICT) {
      for (const stopAtrMult of STOP_ATR_MULTS) {
        PHASE1_GRID.push({
          deviationLongPct, deviationShortPct,
          trendConflictPct, stopAtrMult,
...
async function main() {
  console.log("=== tune-vwap.ts: Walk-Forward VWAP Engine Parameter Search ===");
  console.log(`Phase 1: ${PHASE1_GRID.length} combos x ${PAIRS.length} pairs (grid: deviation x conflict x stop)`);
  ...
  for (const params of PHASE1_GRID) {
    ...
    for (const pair of PAIRS) {
      ...
      const r = runBacktest(h1, pre1h, pre4h, idx4hAt, vwapSignal(params), params.stagnationH, params.stopAtrMult, params.rrRatio, 100, trainEnd);
  ...
  for (const rrRatio of RR_RATIOS_P2) {
    for (const stagnationH of STAGNATION_H_P2) {
      const params: VwapParams = { ...bestPhase1, rrRatio, stagnationH };
      let totalReturn = 0;
      for (const pair of PAIRS) {
        const { h1, pre1h, pre4h, idx4hAt, trainEnd } = candleMap[pair];
        const r = runBacktest(h1, pre1h, pre4h, idx4hAt, vwapSignal(params), params.stagnationH, params.stopAtrMult, params.rrRatio, 100, trainEnd);
  ...
  for (const { params, trainReturn } of top5Train) {
    ...
    for (const pair of PAIRS) {
      const { h1, pre1h, pre4h, idx4hAt, trainEnd } = candleMap[pair];
      const r = runBacktest(h1, pre1h, pre4h, idx4hAt, vwapSignal(params), params.stagnationH, params.stopAtrMult, params.rrRatio, trainEnd, h1.length);
  ...
  console.log("\n=== TOP 5 OUT-OF-SAMPLE RESULTS (VWAP Engine) ===");
  ...
  for (let rank = 0; rank < walkForwardResults.length; rank++) {
    ...
    const params = `dev ${p.deviationLongPct}/${p.deviationShortPct}% conflict ${p.trendConflictPct}% stop ${p.stopAtrMult}x rr ${p.rrRatio} stag ${p.stagnationH}h`;
    console.log(
  ...
  console.log(`VWAP_DEVIATION_LONG_PCT = ${bp.deviationLongPct}`);
  console.log(`VWAP_DEVIATION_SHORT_PCT = ${bp.deviationShortPct}`);
  console.log(`VWAP_TREND_CONFLICT_PCT = ${bp.trendConflictPct}`);
  console.log(`VWAP_STOP_ATR_MULTIPLIER = ${bp.stopAtrMult}`);
  console.log(`VWAP_REWARD_RISK_RATIO = ${bp.rrRatio}`);
