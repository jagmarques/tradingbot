// TRIX: triple-smoothed EMA rate-of-change crossover with signal line.
// Walk-forward: 240d train / ~125d test. 0.29% round-trip, 10x leverage.
// Run: npx tsx scripts/tune-trix.ts
...
interface Params { dailySmaPeriod: number; dailyAdxMin: number; trixPeriod: number; signalPeriod: number; stopAtrMult: number; rrRatio: number; stagnationH: number; }
...
// TRIX = ROC% of triple EMA; signal = SMA(TRIX, signalPeriod)
function computeTrix(closes: number[], period: number, signalPeriod: number): TrixSignal {
  ...
  // ema3[i] aligns to closes index: offset = (period-1) * 3
  const offset = (period - 1) * 3;
  // Build dense trix values aligned to closes indices
  const trixValues: number[] = [];
  const trixStartIdx: number[] = [];
  for (let i = 1; i < ema3.length; i++) {
    const prev = ema3[i - 1], curr = ema3[i];
    if (prev !== 0) {
      const closesIdx = offset + i;
      if (closesIdx < n) {
        const t = (curr - prev) / prev * 100;
        trixArr[closesIdx] = t;
        trixValues.push(t);
        trixStartIdx.push(closesIdx);
      }
    }
  }
  // Signal = SMA(trix, signalPeriod) — rolling average over the dense trix values
...
function precompute4h(candles: Candle[], trixPeriods: number[], signalPeriods: number[]): H4PreInd {
  ...
  for (const tp of trixPeriods)
    for (const sp of signalPeriods)
      trixMap[`${tp}_${sp}`] = computeTrix(closes, tp, sp);
...
function runBacktest(candles4h: Candle[], pre4h: H4PreInd, dailyCandles: Candle[], preDaily: DailyPreInd, idxDailyAt: number[], p: Params, startIdx = 80, endIdx?: number): BacktestResult {
  ...
  for (let i = startIdx; i < end; i++) {
    ...
    if (pos !== null) {
      ...
      const currTrix = ts.trix[i], prevTrix = ts.trix[i - 1];
      const currSignal = ts.signal[i], prevSignal = ts.signal[i - 1];
...
const SIGNAL_PERIOD_VALS = [3, 5, 7, 9, 12];
const STOP_ATR_VALS = [0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 4.0];
const RR_VALS = [1.5, 2.0, 2.5, 3.0, 3.5, 4.0, 5.0];
const STAG_VALS = [4, 6, 9, 12, 16, 24];
  ...
  for (const dailyAdxMin of DAILY_ADX_VALS)
    for (const trixPeriod of TRIX_PERIOD_VALS)
      for (const signalPeriod of SIGNAL_PERIOD_VALS)
        PHASE1_GRID.push({ dailySmaPeriod, dailyAdxMin, trixPeriod, signalPeriod, stopAtrMult: 2.5, rrRatio: 3.0, stagnationH: 12 });

async function main() {
  const p2Count = STOP_ATR_VALS.length * RR_VALS.length * STAG_VALS.length;
  console.log(`=== tune-trix.ts: TRIX Crossover, ${PAIRS.length} pairs, ${DAYS}d ===`);
  ...
  console.log(`  Best: sma=${best1.dailySmaPeriod} adx=${best1.dailyAdxMin} TRIX(${best1.trixPeriod}) Signal(${best1.signalPeriod}) | train: ${phase1[0].trainReturn.toFixed(2)}%`);
  ...
  const top5 = [...phase1, ...phase2].sort((a, b) => b.trainReturn - a.trainReturn).slice(0, 10);
  ...
  results.forEach((r, i) => {
    const p = r.params;
    console.log(`${String(i + 1).padStart(2)}  ${r.profitPerDay >= 0 ? "+" : ""}${r.profitPerDay.toFixed(3)}  ${r.testReturn >= 0 ? "+" : ""}${r.testReturn.toFixed(2)}%  ${String(r.testTrades).padStart(6)}  ${r.testWinRate.toFixed(0)}%  ${r.testMaxDrawdown.toFixed(1)}%  ${r.testSharpe >= 0 ? " " : ""}${r.testSharpe.toFixed(2)}   sma=${p.dailySmaPeriod} adx=${p.dailyAdxMin} TRIX(${p.trixPeriod}) Signal(${p.signalPeriod}) stop=${p.stopAtrMult} rr=${p.rrRatio} stag=${p.stagnationH * 4}h`);
  });
