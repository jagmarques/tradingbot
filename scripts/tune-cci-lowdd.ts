// CCI low-MaxDD sweep: top-3 per ADX (Calmar), ADX up to 30, MaxDD cap <22% on Phase 2, Phase 3 trail.
// Run: npx tsx scripts/tune-cci-lowdd.ts
...
interface Params { dailySmaPeriod: number; dailyAdxMin: number; cciPeriod: number; cciThreshold: number; stopAtrMult: number; rrRatio: number; stagnationH: number; trailThreshold: number; trailRetrace: number; }
...
interface H4PreInd { atr: (number | null)[]; cci: Record<number, (number | null)[]>; }

function computeCCI(candles: Candle[], period: number): (number | null)[] {
...
function precompute4h(candles: Candle[], cciPeriods: number[]): H4PreInd {
  const n = candles.length, closes = candles.map(c => c.close), highs = candles.map(c => c.high), lows = candles.map(c => c.low);
  ...
  const cci: Record<number, (number | null)[]> = {};
  for (const period of cciPeriods) cci[period] = computeCCI(candles, period);
  return { atr, cci };
}
...
function runBacktest(candles4h: Candle[], pre4h: H4PreInd, dailyCandles: Candle[], preDaily: DailyPreInd, idxDailyAt: number[], p: Params, startIdx = 80, endIdx?: number): BacktestResult {
  ...
  const cciArr = pre4h.cci[p.cciPeriod];
  if (!cciArr) return { trades: 0, wins: 0, totalReturn: 0, maxDrawdown: 0, tradePnlPcts: [], days: 0 };
  ...
  for (let i = startIdx; i < end; i++) {
    ...
    if (pos !== null) {
      ...
      const currCci = cciArr[i], prevCci = cciArr[i - 1];
      if (currCci === null || prevCci === null) continue;
      const dailyUptrend = dailyClose > dailySma, dailyDowntrend = dailyClose < dailySma;
      let dir: "long" | "short" | null = null;
      if (dailyUptrend && prevCci < p.cciThreshold && currCci >= p.cciThreshold) dir = "long";
      if (dailyDowntrend && prevCci > -p.cciThreshold && currCci <= -p.cciThreshold) dir = "short";
      if (dir !== null && i + 1 < end) {
...
const DAILY_ADX_VALS = [10, 14, 18, 22, 25, 30];  // extended — higher ADX = lower MaxDD
const CCI_PERIOD_VALS = [5, 10, 14, 20, 30];
const CCI_THRESHOLD_VALS = [70, 100, 120, 150, 200];
...
const MAXDD_CAP = 22;  // only promote Phase 2 combos with training MaxDD below this
...
for (const dailySmaPeriod of DAILY_SMA_VALS)
  for (const dailyAdxMin of DAILY_ADX_VALS)
    for (const cciPeriod of CCI_PERIOD_VALS)
      for (const cciThreshold of CCI_THRESHOLD_VALS)
        PHASE1_GRID.push({ dailySmaPeriod, dailyAdxMin, cciPeriod, cciThreshold, stopAtrMult: 2.5, rrRatio: 3.5, stagnationH: 12, trailThreshold: 5, trailRetrace: 2 });
...
async function main() {
  ...
  console.log(`=== tune-cci-lowdd.ts: CCI low-MaxDD (top-3/ADX Calmar, ADX 10-30, DD cap ${MAXDD_CAP}%, Phase 3 trail), ${PAIRS.length} pairs, ${DAYS}d ===`);
  console.log(`Phase 1: ${PHASE1_GRID.length} combos | Phase 2: top-3/ADX × ${p2Count} risk, cap MaxDD<${MAXDD_CAP}% | Phase 3: top-50 × ${p3Count} trail, top-20 tested`);
  ...
  for (const pair of PAIRS) {
    try {
      ...
      const pre4h = precompute4h(h4, [...new Set(CCI_PERIOD_VALS)]);
  ...
  console.log(`Phase 1 (${PHASE1_GRID.length} combos, Calmar-sorted, top-3 per ADX)...`);
  ...
  for (const [adx, results] of byAdx) {
    ...
    const w = top3[0];
    console.log(`  ADX=${adx} #1: sma=${w.params.dailySmaPeriod} CCI(${w.params.cciPeriod}) thresh=${w.params.cciThreshold} | train: ${w.trainReturn.toFixed(2)}% MaxDD ${w.trainMaxDD.toFixed(1)}%`);
  }

  console.log(`\nPhase 2: ${phase1Winners.length} P1 winners × ${p2Count} risk combos (trail locked 5/2, MaxDD cap ${MAXDD_CAP}%, Calmar-sorted, top-50)...`);
  ...
  let top50 = allPhase2
    .filter(r => r.trainReturn > 0 && r.trainMaxDD < MAXDD_CAP)
    .sort((a, b) => (b.trainReturn / b.trainMaxDD) - (a.trainReturn / a.trainMaxDD))
    .slice(0, 50);
  if (top50.length === 0) {
    console.warn(`  WARNING: No combos with trainMaxDD < ${MAXDD_CAP}%. Relaxing to <30%...`);
    top50 = allPhase2.filter(r => r.trainReturn > 0 && r.trainMaxDD < 30).sort((a, b) => (b.trainReturn / b.trainMaxDD) - (a.trainReturn / a.trainMaxDD)).slice(0, 50);
  }
  if (top50.length === 0) { console.error("Phase 2: no positive results"); process.exit(1); }
  console.log(`  Phase 2 done. ${top50.length} combos passed cap. Top #1: train ${top50[0].trainReturn.toFixed(2)}% MaxDD ${top50[0].trainMaxDD.toFixed(1)}%`);

  console.log(`\nPhase 3: ${top50.length} P2 winners × ${p3Count} trail combos, top-20 tested...`);
  ...
  results.forEach((r, idx) => {
    ...
    console.log(`${String(idx + 1).padStart(2)}  ${r.profitPerDay >= 0 ? "+" : ""}${r.profitPerDay.toFixed(3)}  ${r.testReturn >= 0 ? "+" : ""}${r.testReturn.toFixed(2)}%  ${String(r.testTrades).padStart(6)}  ${r.testWinRate.toFixed(0)}%  ${r.testMaxDrawdown.toFixed(1)}%  ${calmar.toFixed(2)}   ${r.testSharpe.toFixed(2)}   sma=${r.params.dailySmaPeriod} adx=${r.params.dailyAdxMin} CCI(${r.params.cciPeriod}) thresh=${r.params.cciThreshold} stop=${r.params.stopAtrMult} rr=${r.params.rrRatio} stag=${r.params.stagnationH * 4}h trail=${r.params.trailThreshold}/${r.params.trailRetrace}`);
  });
  ...
  console.log(`Deployed: +0.424%/day MaxDD 27.3%`);
  console.log(best.profitPerDay >= 0.30 && best.testMaxDrawdown < 27.3 ? "IMPROVEMENT — lower MaxDD" : best.profitPerDay >= 0.30 ? "VIABLE but no MaxDD improvement" : "NOT VIABLE");
}
