...
function sim(pairs:string[],maxDir:number,pdm:Map<string,D>,btc:D,s:number,e:number):{tr:T[],finalEq:number,minEq:number}{
  ...
  for(const t of st){const cl=new Set<string>();
    ...
    for(const[p,d]of pds){if(pos.has(p)||cl.has(p))continue;const bi=d.tm.get(t)??-1;if(bi<60)continue;
      ...
      const dc=[...pos.values()].filter(x=>x.dir===dir).length;if(dc>=maxDir)continue;
...
const ALL=["OPUSDT","WIFUSDT","ARBUSDT","LDOUSDT","TRUMPUSDT","DOTUSDT","ENAUSDT","DOGEUSDT","APTUSDT","LINKUSDT","ADAUSDT","WLDUSDT","XRPUSDT","SOLUSDT","BNBUSDT","kSHIBUSDT","TIAUSDT","NEARUSDT","kBONKUSDT","ONDOUSDT","HYPEUSDT","BTCUSDT"];
for(const p of ALL){const d=pre(p);if(d)pdm.set(p,d)}const btc=pdm.get("BTCUSDT")!;
const P21=ALL.filter(p=>p!=="BTCUSDT");

console.log("MaxDir  FinalEq   MinEq  MaxDD%  Trades  T/day  WR%    Train     Test");
console.log("-".repeat(80));

for(const md of [4,5,6,7,8]){
  const{tr,finalEq,minEq}=sim(P21,md,pdm,btc,S,E);
  ...
  console.log(`${String(md).padStart(6)}  $${finalEq.toFixed(0).padStart(7)}  $${minEq.toFixed(0).padStart(5)}  ${dd.toFixed(1).padStart(5)}%  ${String(tr.length).padStart(6)}  ${(tr.length/days).toFixed(1).padStart(5)}  ${(w/tr.length*100).toFixed(1).padStart(5)}  $${trnD.toFixed(2).padStart(7)}  $${tstD.toFixed(2).padStart(7)}`);
}
