// Donchian Channel Breakout: close breaks above/below N-bar high/low with ADX + daily trend filter.
// Entry on next bar open after signal. Walk-forward: 240d train / ~125d test.
// 0.29% round-trip, 10x leverage.
// Run: npx tsx scripts/tune-donchian.ts
...
interface Params { dailySmaPeriod: number; dailyAdxMin: number; donchianPeriod: number; stopAtrMult: number; rrRatio: number; stagnationH: number; }
...
interface H4PreInd { atr: (number | null)[]; donchian: Record<number, { upper: (number | null)[]; lower: (number | null)[] }>; }
...
function precomputeDonchian(candles: Candle[], periods: number[]): Record<number, { upper: (number | null)[]; lower: (number | null)[] }> {
  const n = candles.length;
  const result: Record<number, { upper: (number | null)[]; lower: (number | null)[] }> = {};
  for (const period of periods) {
    ...
    result[period] = { upper, lower };
  }
  return result;
}

function precompute4h(candles: Candle[], donchianPeriods: number[]): H4PreInd {
  const n = candles.length;
  const highs = candles.map((c) => c.high);
  const lows = candles.map((c) => c.low);
  const closes = candles.map((c) => c.close);
  ...
  const donchian = precomputeDonchian(candles, donchianPeriods);
  return { atr: atrArr, donchian };
}
...
function runBacktest(candles4h: Candle[], pre4h: H4PreInd, dailyCandles: Candle[], preDaily: DailyPreInd, idxDailyAt: number[], p: Params, startIdx = 80, endIdx?: number): BacktestResult {
  ...
  const dcData = pre4h.donchian[p.donchianPeriod];
  if (!dcData) return { trades: 0, wins: 0, totalReturn: 0, maxDrawdown: 0, tradePnlPcts: [], days: 0 };
  ...
  for (let i = startIdx; i < end; i++) {
    ...
    if (pos !== null) {
      ...
      // Use channel from bar i-1 (period ending at i-1): upper[i-1] and lower[i-1]
      const prevUpper = dcData.upper[i - 1];
      const prevLower = dcData.lower[i - 1];
      const currClose = c.close;
      const prevClose = candles4h[i - 1].close;
      if (prevUpper === null || prevLower === null) continue;

      const dailyUptrend = dailyClose > dailySma;
      const dailyDowntrend = dailyClose < dailySma;

      // Long: previous close was below upper channel, current close breaks above
      // Short: previous close was above lower channel, current close breaks below
      let dir: "long" | "short" | null = null;
      if (dailyUptrend && prevClose <= prevUpper && currClose > prevUpper) dir = "long";
      if (dailyDowntrend && prevClose >= prevLower && currClose < prevLower) dir = "short";
...
const DONCHIAN_PERIOD_VALS = [10, 15, 20, 30];
const STOP_ATR_VALS = [1.5, 2.0, 2.5, 3.0];
const RR_VALS = [2.5, 3.0, 3.5, 4.0];
const STAG_VALS = [6, 9, 12];

const ALL_DONCHIAN_PERIODS = [...new Set(DONCHIAN_PERIOD_VALS)];
...
for (const dailySmaPeriod of DAILY_SMA_VALS)
  for (const dailyAdxMin of DAILY_ADX_VALS)
    for (const donchianPeriod of DONCHIAN_PERIOD_VALS)
      PHASE1_GRID.push({ dailySmaPeriod, dailyAdxMin, donchianPeriod, stopAtrMult: 2.0, rrRatio: 3.0, stagnationH: 9 });

async function main() {
  console.log(`=== tune-donchian.ts: Donchian Channel Breakout, ${PAIRS.length} pairs, ${DAYS}d ===`);
  ...
  for (const pair of PAIRS) {
    try {
      ...
      const pre4h = precompute4h(h4, ALL_DONCHIAN_PERIODS);
  ...
  console.log(`  Best Phase 1: sma=${best1.dailySmaPeriod} adx=${best1.dailyAdxMin} donchian=${best1.donchianPeriod} | train: ${phase1[0].trainReturn.toFixed(2)}%`);
  ...
  results.forEach((r, i) => {
    const p = r.params;
    console.log(`${String(i + 1).padStart(2)}  ${r.profitPerDay >= 0 ? "+" : ""}${r.profitPerDay.toFixed(3)}  ${r.testReturn >= 0 ? "+" : ""}${r.testReturn.toFixed(2)}%    ${r.trainReturn >= 0 ? "+" : ""}${r.trainReturn.toFixed(2)}%   ${String(r.testTrades).padStart(6)}  ${r.testWinRate.toFixed(0)}%  ${r.testMaxDrawdown.toFixed(1)}%  ${r.testSharpe >= 0 ? " " : ""}${r.testSharpe.toFixed(2)}   sma=${p.dailySmaPeriod} adx=${p.dailyAdxMin} donchian=${p.donchianPeriod} stop=${p.stopAtrMult} rr=${p.rrRatio} stag=${p.stagnationH * 4}h`);
  });
  const best = results[0];
  console.log(`\nBest: ${best.profitPerDay >= 0 ? "+" : ""}${best.profitPerDay.toFixed(4)}%/day, Sharpe ${best.testSharpe.toFixed(2)}, trades ${best.testTrades}`);
  const p = best.params;
  console.log(`Optimal params: sma=${p.dailySmaPeriod} adx=${p.dailyAdxMin} donchian=${p.donchianPeriod} stop=${p.stopAtrMult} rr=${p.rrRatio} stag=${p.stagnationH * 4}h`);
  if (best.testSharpe > 1.0 && best.testTrades > 30 && best.profitPerDay > 0.25) {
    console.log(`VIABLE. Sharpe ${best.testSharpe.toFixed(2)} > 1.0, trades ${best.testTrades} > 30, %/day ${best.profitPerDay.toFixed(3)} > 0.25.`);
  } else { console.log(`NOT VIABLE (need Sharpe>1.0 AND >30 trades AND >0.25%/day). Got Sharpe=${best.testSharpe.toFixed(2)}, trades=${best.testTrades}, %/day=${best.profitPerDay.toFixed(3)}.`); }
}
