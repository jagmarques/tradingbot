/**
 * Stepped trail backtest: 22/6 -> 28/2 -> 33/1 vs flat 30/3 vs no trail.
 * GARCH-only, $9 margin, z=4.5/3.0, SL 3%, TP 7%, 72h hold, max 7 positions.
 * 23 pairs, 2023-01 to 2026-03. OOS: 2025-09+.
 * BTC filter: 1h EMA(9/21) for GARCH (NOT 4h EMA 12/21).
 *
 * Run: NODE_OPTIONS="--max-old-space-size=8192" npx tsx scripts/bt-stepped-final.ts
 */
...
// --------------- trail modes ---------------
type TrailMode = "none" | "flat30_3" | "stepped22";

// Stepped trail: returns the trail distance (%) given current peak
// Steps: >22 -> 6%, >28 -> 2%, >33 -> 1%
function steppedTrailDist(peakPct: number): number {
  if (peakPct >= 33) return 1;
  if (peakPct >= 28) return 2;
  if (peakPct >= 22) return 6;
  ...
  for (let t = FULL_START; t < FULL_END; t += MIN_5) {
    ...
    if (is5m && openPositions.length > 0) {
      for (let pi = openPositions.length - 1; pi >= 0; pi--) {
        ...
        } else if (trailMode === "stepped22") {
          const dist = steppedTrailDist(pos.peakPnlPct);
          if (dist > 0) {
            const currPct = pos.dir === "long"
              ? (bar.c / pos.entryPrice - 1) * LEV * 100
              : (pos.entryPrice / bar.c - 1) * LEV * 100;
...
const CONFIGS: Array<{ mode: TrailMode; label: string }> = [
  ...
  { mode: "stepped22", label: "Stepped 22/6->28/2->33/1" },
];
...
const DASH = "-".repeat(130);

console.log(`\n\n${SEP}`);
console.log("STEPPED TRAIL BACKTEST — GARCH-only $9 | z=4.5/3.0 | SL 3% | TP 7% | 72h | max 7 | 23 pairs | 2023-01 to 2026-03");
console.log(SEP);
console.log(
  "Config".padEnd(32) +
  "Trades".padStart(7) +
...
for (const r of results.slice(1)) {
  ...
  console.log(
    `  ${r.label.padEnd(30)}  $/day ${dPerDay >= 0 ? "+" : ""}${dPerDay.toFixed(3)}` +
...
if (results.length >= 3) {
  ...
  const stepped = results[2];
  const dPerDay = stepped.perDay - flat.perDay;
  const dOos    = stepped.oosPerDay - flat.oosPerDay;
  const dDD     = stepped.maxDD - flat.maxDD;
  console.log(`\nStepped 22/6->28/2->33/1 vs flat 30/3:`);
  console.log(
    `  $/day ${dPerDay >= 0 ? "+" : ""}${dPerDay.toFixed(3)}` +
    `  OOS ${dOos >= 0 ? "+" : ""}${dOos.toFixed(3)}` +
    `  MaxDD ${dDD >= 0 ? "+" : ""}${dDD.toFixed(1)}` +
    `  Trail# stepped=${stepped.trailExits} flat=${flat.trailExits}`
  );
}
