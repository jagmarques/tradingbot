/**
 * Trail sweep for z2.0/1.5 config: find distances that work on 1h bar closes
 * At 10x lev, 1% trail dist = 0.1% price, 2% = 0.2%, 3% = 0.3%
 * At 3x lev, 1% trail dist = 0.33% price, 2% = 0.67%, 3% = 1%
 * Need distances wide enough that a normal 1h bar close doesn't trigger
 */
...
function run(pairs: PD[], trail: {a:number;d:number}[], label: string) {
  ...
  for(const hr of hours) {
    for(let i=open.length-1;i>=0;i--) {
      ...
      if(xp>0){const rsp=isSL?pos.sp*1.5:pos.sp;const ex=pos.dir==="long"?xp*(1-rsp):xp*(1+rsp);const pnl=(pos.dir==="long"?(ex/pos.ep-1):(pos.ep/ex-1))*pos.not-pos.not*FEE*2;closed.push({pnl,reason});open.splice(i,1);if(reason==="sl")cd.set(`${pos.pair}:${pos.dir}`,hr+CD_H*H);}
    }
  ...
  console.log(`  ${label.padEnd(50)} ${String(closed.length).padStart(6)} ${(wr.toFixed(1)+"%").padStart(6)} ${pf.toFixed(2).padStart(5)} ${fmtPnl(pd).padStart(9)} $${maxDD.toFixed(0).padStart(3)} ${String(slN).padStart(6)} ${String(trN).padStart(6)}`);
}

function main() {
  console.log("=".repeat(110));
  console.log("  TRAIL SWEEP FOR z2.0/1.5 - find live-safe distances");
  console.log("  Price move per 1% lev PnL: 10x=0.1%, 5x=0.2%, 3x=0.33%");
  console.log("=".repeat(110));
  console.log("\n  Loading...");
  const pairs: PD[] = [];
  for (const n of ALL) {
  ...
  const hdr = `  ${"Trail Config".padEnd(50)} ${"Trd".padStart(6)} ${"WR%".padStart(6)} ${"PF".padStart(5)} ${"$/day".padStart(9)} ${"MDD".padStart(4)} ${"SL".padStart(6)} ${"Trail".padStart(6)}`;

  // Current (tight)
  console.log("--- CURRENT (tight, may be too aggressive for live) ---\n");
  console.log(hdr); console.log("  "+"-".repeat(95));
  run(pairs, [{a:3,d:1.5},{a:8,d:0.5},{a:20,d:0.5}], "CURRENT: 3/1.5 -> 8/0.5 -> 20/0.5");

  // ─── Moderate distances (1-3% range) ───
  console.log("\n--- MODERATE DISTANCES (1-3% trail dist) ---\n");
  console.log(hdr); console.log("  "+"-".repeat(95));
  run(pairs, [{a:3,d:2},{a:8,d:1},{a:20,d:1}], "3/2 -> 8/1 -> 20/1");
  run(pairs, [{a:3,d:2},{a:10,d:1.5},{a:25,d:1}], "3/2 -> 10/1.5 -> 25/1");
  run(pairs, [{a:5,d:3},{a:10,d:2},{a:20,d:1}], "5/3 -> 10/2 -> 20/1");
  run(pairs, [{a:5,d:3},{a:12,d:2},{a:25,d:1}], "5/3 -> 12/2 -> 25/1");
  run(pairs, [{a:5,d:2},{a:10,d:1},{a:20,d:1}], "5/2 -> 10/1 -> 20/1");
  run(pairs, [{a:5,d:2},{a:12,d:1.5},{a:25,d:1}], "5/2 -> 12/1.5 -> 25/1");
  run(pairs, [{a:5,d:2.5},{a:10,d:1.5},{a:20,d:1}], "5/2.5 -> 10/1.5 -> 20/1");
  run(pairs, [{a:3,d:1.5},{a:8,d:1},{a:20,d:1}], "3/1.5 -> 8/1 -> 20/1");
  run(pairs, [{a:3,d:2},{a:8,d:1.5},{a:20,d:1}], "3/2 -> 8/1.5 -> 20/1");

  // ─── Wide distances (3-5% range, very safe) ───
  console.log("\n--- WIDE DISTANCES (3-5% trail dist, very live-safe) ---\n");
  console.log(hdr); console.log("  "+"-".repeat(95));
  run(pairs, [{a:5,d:3},{a:15,d:2},{a:30,d:1.5}], "5/3 -> 15/2 -> 30/1.5");
  run(pairs, [{a:7,d:4},{a:15,d:3},{a:30,d:2}], "7/4 -> 15/3 -> 30/2");
  run(pairs, [{a:7,d:3},{a:15,d:2},{a:30,d:1}], "7/3 -> 15/2 -> 30/1");
  run(pairs, [{a:10,d:5},{a:20,d:3},{a:35,d:2}], "10/5 -> 20/3 -> 35/2");
  run(pairs, [{a:10,d:5},{a:25,d:3},{a:50,d:2}], "10/5 -> 25/3 -> 50/2");
  run(pairs, [{a:10,d:4},{a:20,d:2},{a:40,d:1}], "10/4 -> 20/2 -> 40/1");

  // ─── 2-stage simple (fewer edge cases) ───
  console.log("\n--- 2-STAGE SIMPLE ---\n");
  console.log(hdr); console.log("  "+"-".repeat(95));
  run(pairs, [{a:5,d:3},{a:15,d:1}], "5/3 -> 15/1");
  run(pairs, [{a:5,d:2},{a:15,d:1}], "5/2 -> 15/1");
  run(pairs, [{a:7,d:3},{a:20,d:1}], "7/3 -> 20/1");
  run(pairs, [{a:7,d:4},{a:20,d:2}], "7/4 -> 20/2");
  run(pairs, [{a:10,d:5},{a:25,d:2}], "10/5 -> 25/2");
  run(pairs, [{a:5,d:2},{a:20,d:1}], "5/2 -> 20/1");

  // ─── 4-stage gradual ───
  console.log("\n--- 4-STAGE GRADUAL ---\n");
  console.log(hdr); console.log("  "+"-".repeat(95));
  run(pairs, [{a:3,d:2},{a:7,d:1.5},{a:15,d:1},{a:30,d:1}], "3/2 -> 7/1.5 -> 15/1 -> 30/1");
  run(pairs, [{a:5,d:3},{a:10,d:2},{a:20,d:1.5},{a:40,d:1}], "5/3 -> 10/2 -> 20/1.5 -> 40/1");
  run(pairs, [{a:5,d:2.5},{a:10,d:2},{a:20,d:1},{a:35,d:1}], "5/2.5 -> 10/2 -> 20/1 -> 35/1");
  run(pairs, [{a:3,d:1.5},{a:7,d:1},{a:15,d:1},{a:30,d:0.5}], "3/1.5 -> 7/1 -> 15/1 -> 30/0.5");

  // ─── Price-move reference table ───
  console.log("\n--- REFERENCE: What trail distances mean in price ---");
  console.log("  Trail dist 1% lev PnL = price move:");
  console.log("    10x: 0.10%    5x: 0.20%    3x: 0.33%");
  console.log("  Trail dist 2% lev PnL = price move:");
  console.log("    10x: 0.20%    5x: 0.40%    3x: 0.67%");
  console.log("  Trail dist 3% lev PnL = price move:");
  console.log("    10x: 0.30%    5x: 0.60%    3x: 1.00%");
  console.log("  Trail dist 5% lev PnL = price move:");
  console.log("    10x: 0.50%    5x: 1.00%    3x: 1.67%");
  console.log("\n  For live safety, minimum 1% dist (0.1% price at 10x)");
  console.log("  Safe: 2%+ dist (0.2%+ price at 10x, 0.4%+ at 5x)");
}
