/**
 * Focused trail sweep: fixed $15 mc5 z1.5/1.0 SL0.15/0.30 cd2h
 * Tests many trail activation/distance combos
 */
...
type Trail = { a: number; d: number }[];

function simulate(trail: Trail): { pnlDay: number; mdd: number; pf: number; wr: number; trades: number; calmar: number; slPct: number; trailPct: number } {
  const openPositions: Position[] = [];
  let realizedPnl = 0, mtmPeak = 0, mtmMaxDD = 0;
  let totalTrades = 0, totalWins = 0, grossProfit = 0, grossLoss = 0;
  let slCount = 0, trailCount = 0;
  ...
  const slPctOut = totalTrades > 0 ? slCount / totalTrades * 100 : 0;
  const trailPctOut = totalTrades > 0 ? trailCount / totalTrades * 100 : 0;
  return { pnlDay, mdd: mtmMaxDD, pf, wr, trades: totalTrades, calmar, slPct: slPctOut, trailPct: trailPctOut };
}

// Trail configs to test - 3-stage combos
const activations1 = [1, 1.5, 2, 3, 4, 5];    // first stage activation
const distances1 = [0.3, 0.5, 0.8, 1.0, 1.5];  // first stage distance
const activations2 = [3, 5, 7, 10, 15];         // second stage activation
const distances2 = [0.2, 0.3, 0.5, 0.8, 1.0];  // second stage distance
const activations3 = [10, 15, 20, 30];          // third stage activation
const distances3 = [0.2, 0.3, 0.5];             // third stage distance

// Also test 2-stage and 1-stage trails
const configs: { trail: Trail; label: string }[] = [];

// 1-stage trails
for (const a of [1, 2, 3, 5, 7, 10]) {
  for (const d of [0.2, 0.3, 0.5, 0.8, 1.0, 1.5, 2.0]) {
    configs.push({ trail: [{ a, d }], label: `${a}/${d}` });
  }
}

// 2-stage trails
for (const a1 of [1, 2, 3, 5]) {
  for (const d1 of [0.5, 0.8, 1.0, 1.5]) {
    for (const a2 of [5, 10, 15, 20]) {
      if (a2 <= a1) continue;
      for (const d2 of [0.2, 0.3, 0.5]) {
        configs.push({ trail: [{ a: a2, d: d2 }, { a: a1, d: d1 }], label: `${a1}/${d1}-${a2}/${d2}` });
      }
    }
  }
}

// 3-stage trails (selective)
for (const a1 of activations1) {
  for (const d1 of distances1) {
    for (const a2 of activations2) {
      if (a2 <= a1) continue;
      for (const d2 of distances2) {
        if (d2 > d1) continue; // later stages should be tighter
        for (const a3 of activations3) {
          if (a3 <= a2) continue;
          for (const d3 of distances3) {
            if (d3 > d2) continue;
            configs.push({
              trail: [{ a: a3, d: d3 }, { a: a2, d: d2 }, { a: a1, d: d1 }],
              label: `${a1}/${d1}-${a2}/${d2}-${a3}/${d3}`,
            });
          }
        }
      }
...
console.log(`\nSweeping ${configs.length} trail configs...\n`);

interface Result { label: string; pnlDay: number; mdd: number; pf: number; wr: number; trades: number; calmar: number; slPct: number; trailPct: number }
const results: Result[] = [];

for (let i = 0; i < configs.length; i++) {
  if ((i + 1) % 50 === 0) process.stdout.write(`\r${i + 1}/${configs.length}`);
  const cfg = configs[i]!;
  const r = simulate(cfg.trail);
...
console.log(`${"Trail".padEnd(30)} $/day   MDD    PF   WR%  Trades  SL%  Trail% Calmar`);
for (const r of results.slice(0, 30)) {
  console.log(`${r.label.padEnd(30)} ${r.pnlDay.toFixed(2).padStart(5)}  ${r.mdd.toFixed(1).padStart(5)}  ${r.pf.toFixed(2).padStart(4)}  ${r.wr.toFixed(1).padStart(4)}  ${String(r.trades).padStart(5)}  ${r.slPct.toFixed(0).padStart(3)}%  ${r.trailPct.toFixed(0).padStart(4)}%  ${r.calmar.toFixed(4)}`);
}
...
console.log(`\n${"=".repeat(90)}`);
console.log("TOP 20 BY $/DAY (MDD < $30)");
console.log("=".repeat(90));
const safe = results.filter(r => r.mdd < 30);
...
console.log(`\n--- CURRENT CONFIG ---`);
const current = simulate([{ a: 10, d: 0.3 }, { a: 5, d: 0.3 }, { a: 2, d: 0.5 }]);
console.log(`2/0.5-5/0.3-10/0.3: $${current.pnlDay.toFixed(2)}/day, MDD $${current.mdd.toFixed(1)}, PF ${current.pf.toFixed(2)}, ${current.trades} trades, Calmar ${current.calmar.toFixed(4)}`);
