// Vortex HQ: picks top winner per ADX level (14-25), Phase 2 for each.
// Run: npx tsx scripts/tune-vortex-hq.ts
...
async function main() {
  ...
  console.log(`=== tune-vortex-hq.ts: Vortex HQ (ADX 14-25), ${PAIRS.length} pairs, ${DAYS}d ===`);
  ...
  for (const [adx, results] of byAdx) {
    ...
    console.log(`  ADX=${adx} best: sma=${w.params.dailySmaPeriod} Vortex(${w.params.vortexPeriod}) | train: ${w.trainReturn.toFixed(2)}%`);
  }
