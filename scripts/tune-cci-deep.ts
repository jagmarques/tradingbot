// CCI Trend deep sweep: wider CCI period and threshold search.
// Phase 1: 225 indicator combos. Phase 2: 80 risk/reward combos on best Phase-1 winner.
// Walk-forward: 240d train / ~125d test. 0.29% round-trip, 10x leverage.
// Run: npx tsx scripts/tune-cci-deep.ts
...
async function main() {
  ...
  console.log(`=== tune-cci-deep.ts: CCI Trend deep sweep, ${PAIRS.length} pairs, ${DAYS}d ===`);
  ...
  if (best.testSharpe > 1.0 && best.testTrades > 30) {
    console.log(`VIABLE. vs prev CCI +${prev}%/day: ${best.profitPerDay > prev ? `BEATS IT (+${(best.profitPerDay - prev).toFixed(3)}%/day)` : "below"}`);
