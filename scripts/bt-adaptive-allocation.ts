/**
 * Adaptive Capital Allocation Backtest
 * Test whether shifting capital toward currently-performing engines improves results.
 *
 * 5 engines: A(Donchian), B(Supertrend), C(GARCH), D(Carry), M(Momentum Confirm)
 * 6 allocation methods vs fixed baseline.
 *
 * Run: cd "/Users/jagmarques/Library/CloudStorage/OneDrive/Work/.TradingBot" && npx tsx scripts/bt-adaptive-allocation.ts
 */
...
const ALL_ENGINES = ["A","B","C","D","M"];
const ENGINE_NAMES: Record<string, string> = {
  A: "Donchian", B: "Supertrend", C: "GARCH", D: "Carry", M: "MomConfirm",
};
...
// ─── Allocation Methods ─────────────────────────────────────────────
type SizeFn = (engine: string, currentTime: number, closedTrades: Trade[]) => number;

function fixedSizeFn(sizes: Record<string, number>): SizeFn {
  return (engine: string) => sizes[engine] ?? 5;
}

function rollingSharpeWeightedFn(): SizeFn {
  let cachedSizes: Record<string, number> = {};
  let lastRecompute = 0;
  const RECOMPUTE_INTERVAL = 30 * DAY;
  const LOOKBACK = 30 * DAY;

  return (engine: string, currentTime: number, closedTrades: Trade[]) => {
    if (currentTime - lastRecompute >= RECOMPUTE_INTERVAL || lastRecompute === 0) {
      lastRecompute = currentTime;
      const sharpes: Record<string, number> = {};

      for (const eng of ALL_ENGINES) {
        const recent = closedTrades.filter(
          t => t.engine === eng && t.xt >= currentTime - LOOKBACK && t.xt < currentTime
        );
        if (recent.length < 3) { sharpes[eng] = 0; continue; }
        ...
        if (dpVals.length < 2) { sharpes[eng] = 0; continue; }
        const mean = dpVals.reduce((a, b) => a + b, 0) / dpVals.length;
        const std = Math.sqrt(dpVals.reduce((s, v) => s + (v - mean) ** 2, 0) / (dpVals.length - 1));
        sharpes[eng] = std > 0 ? (mean / std) * Math.sqrt(365) : 0;
      }

      // Allocate proportional to max(0, sharpe)
      const positives: Record<string, number> = {};
      let totalPositive = 0;
      for (const eng of ALL_ENGINES) {
        const v = Math.max(0, sharpes[eng]);
        positives[eng] = v;
        totalPositive += v;
      }
      ...
      if (totalPositive <= 0) {
        // Equal split
        for (const eng of ALL_ENGINES) cachedSizes[eng] = TOTAL_BUDGET / ALL_ENGINES.length;
      } else {
        for (const eng of ALL_ENGINES) {
          let raw = (positives[eng] / totalPositive) * TOTAL_BUDGET;
          raw = Math.max(SIZE_FLOOR, Math.min(SIZE_CAP, raw));
          cachedSizes[eng] = Math.round(raw * 100) / 100;
        }
      }
    }
    return cachedSizes[engine] ?? 5;
  };
}

function rollingPFWeightedFn(): SizeFn {
  let cachedSizes: Record<string, number> = {};
  let lastRecompute = 0;
  const RECOMPUTE_INTERVAL = 30 * DAY;
  const LOOKBACK = 30 * DAY;
  ...
  return (engine: string, currentTime: number, closedTrades: Trade[]) => {
    if (currentTime - lastRecompute >= RECOMPUTE_INTERVAL || lastRecompute === 0) {
      ...
      for (const eng of ALL_ENGINES) {
        ...
        if (recent.length < 3) { pfs[eng] = 0; continue; }

        const wins = recent.filter(t => t.pnl > 0);
        const losses = recent.filter(t => t.pnl <= 0);
        const gw = wins.reduce((s, t) => s + t.pnl, 0);
        const gl = Math.abs(losses.reduce((s, t) => s + t.pnl, 0));
        pfs[eng] = gl > 0 ? gw / gl : gw > 0 ? 3 : 0; // cap at 3 for infinite PF
      }

      // Weight by max(0, PF - 1) so PF < 1 gets zero weight
      const weights: Record<string, number> = {};
      let totalWeight = 0;
      for (const eng of ALL_ENGINES) {
        const v = Math.max(0, pfs[eng] - 1);
        weights[eng] = v;
        totalWeight += v;
      }
      ...
      if (totalWeight <= 0) {
        ...
        for (const eng of ALL_ENGINES) {
          let raw = (weights[eng] / totalWeight) * TOTAL_BUDGET;
          raw = Math.max(SIZE_FLOOR, Math.min(SIZE_CAP, raw));
          cachedSizes[eng] = Math.round(raw * 100) / 100;
        }
      }
...
function winStreakAdaptiveFn(): SizeFn {
  return (engine: string, _currentTime: number, closedTrades: Trade[]) => {
    const engTrades = closedTrades.filter(t => t.engine === engine);
    const last5 = engTrades.slice(-5);
    if (last5.length < 5) return 5; // default
    const winners = last5.filter(t => t.pnl > 0).length;
    const losers = last5.filter(t => t.pnl <= 0).length;
    if (winners >= 3) return 8;
    if (losers >= 3) return 2;
    return 5;
  };
}

function equityCurveFn(): SizeFn {
  return (engine: string, _currentTime: number, closedTrades: Trade[]) => {
    const engTrades = closedTrades.filter(t => t.engine === engine);
    if (engTrades.length < 20) return 5; // default until enough data

    // Cumulative PnL
    const cumPnl: number[] = [];
    let cum = 0;
    for (const t of engTrades) { cum += t.pnl; cumPnl.push(cum); }

    // 20-trade SMA of cumulative PnL
    const last20 = cumPnl.slice(-20);
    const smaVal = last20.reduce((a, b) => a + b, 0) / 20;
    const current = cumPnl[cumPnl.length - 1];

    return current > smaVal ? 7 : 3;
  };
}

function inverseVolatilityFn(): SizeFn {
  let cachedSizes: Record<string, number> = {};
  let lastRecompute = 0;
  const RECOMPUTE_INTERVAL = 30 * DAY;
  const LOOKBACK = 30 * DAY;
  ...
  return (engine: string, currentTime: number, closedTrades: Trade[]) => {
    if (currentTime - lastRecompute >= RECOMPUTE_INTERVAL || lastRecompute === 0) {
      ...
      if (totalInvVol <= 0) {
        ...
        for (const eng of ALL_ENGINES) {
          let raw = (invVols[eng] / totalInvVol) * TOTAL_BUDGET;
          raw = Math.max(SIZE_FLOOR, Math.min(SIZE_CAP, raw));
          cachedSizes[eng] = Math.round(raw * 100) / 100;
        }
      }
...
// ─── Run Backtest ───────────────────────────────────────────────────
function runBacktest(sizeFn: SizeFn, spreadMultiplier: number): Trade[] {
  ...
  function btcH4Ema20gt50(t: number): boolean {
    const h4i = getBarAtOrBefore(btcData.h4, t - H4, btcData.h4Map, H4);
    if (h4i < 0) return false;
    const e20 = btcData.h4Ema20[h4i], e50 = btcData.h4Ema50[h4i];
    return e20 !== null && e50 !== null && e20 > e50;
  }
  ...
  for (const dayT of dailyTimestamps) {
    ...
    for (const p of available) {
      ...
      const margin = sizeFn("A", dayT, trades);
      positions.set(key, {
    ...
    for (let h4Offset = 0; h4Offset < DAY; h4Offset += H4) {
      ...
      for (const p of available) {
        ...
        const margin = sizeFn("B", h4T, trades);
        positions.set(key, {
    ...
    for (let h1Offset = 0; h1Offset < DAY; h1Offset += H1) {
      ...
      for (const p of available) {
        ...
        const margin = sizeFn("C", h1T, trades);
        positions.set(key, {
    ...
    if (dayT - lastCarryRebalance >= 7 * DAY) {
      ...
      for (const pick of [...top3, ...bot3]) {
        ...
        const pd = pairDataMap.get(pick.pair)!;
        const di = pd.dailyMap.get(dayT);
        if (di === undefined) continue;
        ...
        const margin = sizeFn("D", dayT, trades);
        positions.set(key, {
    ...
    for (let h4Offset = 0; h4Offset < DAY; h4Offset += H4) {
      ...
      for (const p of available) {
        ...
        const margin = sizeFn("M", h4T, trades);
        positions.set(key, {
          pair: p, engine: "M", dir, ep, et: h4T, sl, tp: 0,
          margin, lev: 10, maxHold: 48 * H1, atr: 0, bestPnlAtr: 0,
        });
      }
    }
  }
...
function printStatsLine(label: string, s: Stats) {
  console.log(
    `${label.padEnd(24)} ${pad(String(s.trades), 6)} ${pad(String(s.pf), 6)} ${pad(String(s.sharpe), 7)} ` +
...
function printHeader() {
  console.log(
    `${"Method".padEnd(24)} ${pad("Trades", 6)} ${pad("PF", 6)} ${pad("Sharpe", 7)} ` +
...
const CONFIGS: AllocConfig[] = [
  {
    label: "1. Fixed baseline",
    sizeFn: () => fixedSizeFn({ A: 7, B: 5, C: 3, D: 7, M: 3 }),
  },
  {
    label: "2. Rolling Sharpe",
    sizeFn: rollingSharpeWeightedFn,
  },
  {
    label: "3. Rolling PF",
    sizeFn: rollingPFWeightedFn,
  },
  {
    label: "4. Win-streak",
    sizeFn: winStreakAdaptiveFn,
  },
  {
    label: "5. Equity curve",
    sizeFn: equityCurveFn,
  },
  {
    label: "6. Inverse volatility",
    sizeFn: inverseVolatilityFn,
  },
];

// ─── Main ───────────────────────────────────────────────────────────
console.log("=".repeat(115));
console.log("  ADAPTIVE CAPITAL ALLOCATION BACKTEST");
console.log("  5 engines: A(Donchian), B(Supertrend), C(GARCH), D(Carry), M(MomConfirm)");
...
console.log("  Budget: $25 total across 5 engines. Floor $2, cap $10.");
console.log("=".repeat(115));
...
for (const config of CONFIGS) {
  ...
  const sfn = config.sizeFn();
  const trades = runBacktest(sfn, 1.0);
  ...
  console.log("\n--- Engine Contribution ---");
  console.log(
    `${"Engine".padEnd(18)} ${pad("Trades", 6)} ${pad("PF", 6)} ${pad("WR", 7)} ` +
    `${pad("Total PnL", 11)} ${pad("$/day", 9)} ${pad("AvgPnl", 8)} ${pad("AvgSize", 8)}`
  );
  ...
  for (const eng of ALL_ENGINES) {
    ...
    const avgSize = et.length > 0 ? et.reduce((s, t) => s + t.margin, 0) / et.length : 0;
    console.log(
      `${(eng + ": " + ENGINE_NAMES[eng]).padEnd(18)} ${pad(String(et.length), 6)} ${pad(pf.toFixed(2), 6)} ` +
...
// ─── RANKED TABLE ───────────────────────────────────────────────────
console.log("\n\n" + "=".repeat(140));
console.log("  RANKED TABLE - All Allocation Methods (sorted by full-period $/day)");
console.log("=".repeat(140));

const sorted = [...results].sort((a, b) => b.fullStats.perDay - a.fullStats.perDay);

console.log(
  `${"#".padEnd(3)} ${"Method".padEnd(24)} ${pad("Trades", 6)} ${pad("PF", 6)} ${pad("Sharpe", 7)} ` +
...
// ─── DELTA vs BASELINE ──────────────────────────────────────────────
console.log("\n\n" + "=".repeat(120));
console.log("  DELTA vs FIXED BASELINE");
console.log("=".repeat(120));

const baseline = results[0];
console.log(
  `${"Method".padEnd(24)} ${pad("dPF", 7)} ${pad("dSharpe", 8)} ${pad("d$/day", 9)} ` +
  `${pad("dWR", 7)} ${pad("dMaxDD", 8)} ${pad("dTotalPnL", 12)} ${pad("Better?", 8)}`
);
...
for (let i = 1; i < results.length; i++) {
  const r = results[i];
  const s = r.fullStats;
  const b = baseline.fullStats;
  const dPf = s.pf - b.pf;
  const dSharpe = s.sharpe - b.sharpe;
  const dPerDay = s.perDay - b.perDay;
  const dWr = s.wr - b.wr;
  const dMaxDd = s.maxDd - b.maxDd;
  const dTotal = s.totalPnl - b.totalPnl;
  const better = dPerDay > 0 && dSharpe >= 0 ? "YES" : dPerDay > 0 ? "MAYBE" : "NO";

  console.log(
    `${r.label.padEnd(24)} ${pad((dPf >= 0 ? "+" : "") + dPf.toFixed(2), 7)} ` +
    `${pad((dSharpe >= 0 ? "+" : "") + dSharpe.toFixed(2), 8)} ` +
    `${pad(fmtPnl(dPerDay), 9)} ` +
    `${pad((dWr >= 0 ? "+" : "") + dWr.toFixed(1) + "%", 7)} ` +
    `${pad((dMaxDd >= 0 ? "+" : "-") + "$" + Math.abs(dMaxDd).toFixed(0), 8)} ` +
    `${pad(fmtPnl(dTotal), 12)} ` +
    `${pad(better, 8)}`
  );
}
...
const best = sorted[0];
const baselineS = baseline.fullStats;

console.log(`
Best by $/day:    ${best.label}  ->  ${fmtPnl(best.fullStats.perDay)}/day, PF ${best.fullStats.pf}, Sharpe ${best.fullStats.sharpe}
Baseline:         ${baseline.label}  ->  ${fmtPnl(baselineS.perDay)}/day, PF ${baselineS.pf}, Sharpe ${baselineS.sharpe}
Delta:            ${fmtPnl(best.fullStats.perDay - baselineS.perDay)}/day (${((best.fullStats.perDay / baselineS.perDay - 1) * 100).toFixed(1)}%)
...
Year consistency (baseline):
  2023: ${fmtPnl(baseline.yearStats.get(2023)!.perDay)}/day
  2024: ${fmtPnl(baseline.yearStats.get(2024)!.perDay)}/day
  2025: ${fmtPnl(baseline.yearStats.get(2025)!.perDay)}/day
  2026: ${fmtPnl(baseline.yearStats.get(2026)!.perDay)}/day
`);

if (best.label === baseline.label) {
  console.log("VERDICT: Fixed allocation remains the best. Adaptive methods do NOT improve results.");
} else if (best.fullStats.perDay > baselineS.perDay && best.fullStats.sharpe >= baselineS.sharpe) {
  console.log(`VERDICT: ${best.label} beats fixed allocation on both $/day and Sharpe. Consider implementing.`);
} else if (best.fullStats.perDay > baselineS.perDay) {
  console.log(`VERDICT: ${best.label} has higher $/day but lower Sharpe. Marginal improvement, test further.`);
} else {
  console.log("VERDICT: No adaptive method clearly beats fixed allocation.");
}
